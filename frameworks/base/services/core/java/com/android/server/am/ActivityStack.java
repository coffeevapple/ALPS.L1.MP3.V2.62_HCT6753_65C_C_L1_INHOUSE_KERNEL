...
*/
        ...
        /// M. BMW. [ALPS02194735] Restart if process in MinimaxRestartList hasn't clearup when restore
        boolean bNeedResume = true;
        bNeedResume = !MultiWindowProxy.isFeatureSupport() || (MultiWindowProxy.getInstance() != null 
                          && (!MultiWindowProxy.getInstance().matchMinimaxRestartList(next.packageName)
                              || (MultiWindowProxy.getInstance().matchMinimaxRestartList(next.packageName) 
                                  && !next.mPendingRestoreMax)));
        if (next.app != null && next.app.thread != null && bNeedResume) {
            ...
            /// M. BMW. [ALPS02194735] Cleanup before restart if process in MinimaxRestartList hasn't clearup
            if (MultiWindowProxy.isFeatureSupport() 
                ...
                if (!isHomeStack() && lastStack.isFloatingStack()) {
                    ...
                    if (appStack != null) {
                        ...
                        if (next != null && next.state == ActivityState.RESUMED) {
                            // M: [ALPS02147336]. No resumed, set mFloatingBackIsHome false
                            task.mFloatingBackIsHome = false;
                        } else {
                            task.mFloatingBackIsHome = true;
                        }
                    }
    ...
    private void removeActivityFromHistoryLocked(ActivityRecord r, String reason) {
        ...
        if (task != null && task.removeActivity(r)) {
            ...
            if (mStackSupervisor.isFrontStack(this) && task == topTask() &&
                ...
                /// M. [ALPS02127806]. Make sure rebuild window layer  @{
                if (MultiWindowProxy.isFeatureSupport()) {
                    mWindowManager.prepareAppTransition(AppTransition.TRANSIT_NONE, false);
                }
                /// @}
    ...
    final boolean moveTaskToBackLocked(int taskId) {
        ...
        if (task == tr && tr.isOverHomeStack() || numTasks <= 1 && isOnHomeDisplay()) {
            ...
            if (MultiWindowProxy.isFeatureSupport()) {
                // M. [ALPS02189483]. No need to move next floating stack to top, when moving floating stack to back
                if (isOverHome && !isFloatingStack()
                        && mStackSupervisor.moveNextFloatingStackToTopLocked(taskToReturnTo, null)) {
                    if (mResumedActivity != null) {
                        startPausingLocked(false, false, false, false);
                    }
                    return true;
