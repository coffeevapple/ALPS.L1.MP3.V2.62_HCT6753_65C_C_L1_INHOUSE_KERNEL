...
void ProgramAtlas::init() {
    ...
    if (property_get(PROPERTY_ENABLE_PROGRAM_BINARY, property, "0") > 0) {
        ALOGI("Get enable program binary service property (%s)", property);
        mEnableProgramBinaryService = atoi(property);
    }

    if (!mEnableProgramBinaryService) {
        ALOGW("Program binary service is not enabled.");
        return;
    }
