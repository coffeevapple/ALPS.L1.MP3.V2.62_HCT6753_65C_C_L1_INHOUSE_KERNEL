            ...
            if (SystemProperties.get(COMMON_SLOT_PROPERTY).equals("1")) {
                for (int j = (i+1); j < PROJECT_SIM_NUM; j++) {
                    ...
                    if (mCommonSlotResetDone &&
                            sIccId[i].equals(ICCID_STRING_FOR_NO_SIM) &&
                            !sIccId[i].equals(sIccId[j])) {
                        return false;
                    }
                }
            }
    ...
    public void handleMessage(Message msg) {
        switch (msg.what) {
            ...
            case EVENT_SIM_PLUG_IN: {
                index = getCiIndex(msg);
                logd("handleMessage : <EVENT_SIM_PLUG_IN> SIM" + (index + 1) + ": " +
                        mCollectSimState[index]);
                // NO SLOT when boot up -> Plug in so mCommonSlotResetDone is false!
                // To check it's mSimStateCount also
                if ((mCollectSimState[index] != 0) || mCommonSlotResetDone) {
