...
public class UtkService extends Handler implements AppInterface, IUtkService {
    ...
    private UtkService(CommandsInterface ci, UiccCardApplication ca, IccRecords ir,
        ...
        if (CdmaFeatureOptionUtils.isCdmaLteDcSupport()) {
            if (LTE_UTK_DBG) {
                mMutliSimType = getMutliSimType(ic);
...
//bip end
    ...
    private static int getMutliSimType(UiccCard ic) {
        IccCardApplicationStatus.AppType appType1 =
                IccCardApplicationStatus.AppType.APPTYPE_UNKNOWN;
        IccCardApplicationStatus.AppType appType2 =
                IccCardApplicationStatus.AppType.APPTYPE_UNKNOWN;
        UiccCardApplication ca = null;
        UiccCardApplication ca2 = null;

        ca = ic.getApplication(UiccController.APP_FAM_3GPP2);
        ca2 = ic.getApplication(UiccController.APP_FAM_3GPP);

        if (ca != null) {
            appType1 = ca.getType();
            UtkLog.d(LOG_TAG, " appType1 " + appType1);
        }
        if (ca2 != null) {
            appType2 = ca2.getType();
            UtkLog.d(LOG_TAG, " appType2 " + appType2);
        }

        if (IccCardApplicationStatus.AppType.APPTYPE_UNKNOWN == appType1) {
            return UTK_CARD_TYPE_UNKNOW;
        } else if ((IccCardApplicationStatus.AppType.APPTYPE_RUIM == appType1) ||
                    (IccCardApplicationStatus.AppType.APPTYPE_CSIM == appType1)) {
            if (IccCardApplicationStatus.AppType.APPTYPE_UNKNOWN == appType2) {
                return UTK_CARD_TYPE_UIM_ONLY;
            } else if ((IccCardApplicationStatus.AppType.APPTYPE_SIM == appType2) ||
                    (IccCardApplicationStatus.AppType.APPTYPE_USIM == appType2)) {
