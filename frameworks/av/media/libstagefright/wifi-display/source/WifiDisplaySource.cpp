...
///M: @{
      ...
      mWaitPlayRequest(false),
      mPlayRequestReceived(false),
      mCallbackOnConnected(false),
      mTestSessionID(0),
...
///M: export default configuration for WFD
    ...
    if (property_get("media.wfd.video-format", val, NULL)) {
        ...
        if(videoFormat >=0 && videoFormat <= 16){
            ...
            mSupportedSourceVideoFormats.setProfileLevel(VideoFormats::RESOLUTION_CEA, videoFormat, VideoFormats::PROFILE_CHP, VideoFormats::LEVEL_32);
        }
    }
    ...
    char wait[PROPERTY_VALUE_MAX];
    if (property_get("media.wfd.wait_play_req", wait, NULL)) {
        int value = atoi(wait);
        if(value == 1){
            ALOGI("media.wfd.wait_play_req:%s", wait);
            mWaitPlayRequest = true;
        }
    }
}
        ...
        case kWhatPlaybackSessionNotify:
        {
            ...
            if (what == PlaybackSession::kWhatSessionDead) {
                ...
                if (mClient != NULL) {
                    ...
                    if ((mWaitPlayRequest && mPlayRequestReceived) || !mWaitPlayRequest) {
                        ALOGI("mPlayRequestReceived, so callback onConnected");
                ...
                if ((mWaitPlayRequest && mPlayRequestReceived) || !mWaitPlayRequest) {
                    ...
                    ALOGI("deferring finishPlay() until play request");
                }
...
/// @}
    ...
    if (mClient != NULL && mPlaybackSessionEstablished && !mCallbackOnConnected) {
        ALOGI("callback on Connnected()");
        if (!mSinkSupportsVideo) {
            mClient->onDisplayConnected(
                    NULL,  // SurfaceTexture
                    0, // width,
