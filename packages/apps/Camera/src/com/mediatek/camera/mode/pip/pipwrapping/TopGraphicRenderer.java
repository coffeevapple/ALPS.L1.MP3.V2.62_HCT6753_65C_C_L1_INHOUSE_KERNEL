...
public class TopGraphicRenderer extends Renderer {
    ...
    public void draw(int preTex, final float[] preTexMtx, final float[] texReverseRotateMtx,
            final AnimationRect topRect, int rotation, boolean needFlip, boolean processCapture) {
        ...
        if (processCapture) {
            ...
            //TODO
            //according to sensor position, check landscape and portrait by difference compute method
            boolean needChange;
            if (Util.bottomGraphicIsMainCamera(getActivity())) {
                needChange = rotation != 90 && rotation != 180;
            } else {
                needChange = rotation == 90 || rotation == 180;
            }
            lowWidth = isLandScape ? (longer - shorter)
                    * (needChange ? (1f - topGraphicCropValue) : topGraphicCropValue)
                    / longer : 0;
            highWidth = isLandScape ? ((longer - shorter)
                    * (needChange ? (1f - topGraphicCropValue) : topGraphicCropValue) + shorter)
                    / longer
                    : 1;
