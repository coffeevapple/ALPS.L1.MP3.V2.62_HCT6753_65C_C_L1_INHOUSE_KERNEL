...
public class EventInfoFragment extends DialogFragment implements OnCheckedChangeListener,
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        if (Features.isHotKnotSupported()) {
            HotKnotHandler.hotKnotInit(this.getActivity());
        }
        /// @}
		
        ///M: Add a "share icon"&"hotknot icon". For Tablet only. @{
        if (mIsTabletConfig) {
            ...
            View d = mView.findViewById(R.id.hotknot);
            ///M:for the Multi_Window Option,
            ///when from multi-window to full-window and the Calendar is not focus @{
            if(d != null){
            ///@}
                d.setOnClickListener(new OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        HotKnotHandler.hotKnotSend(EventInfoFragment.this.getActivity(),EventInfoFragment.this);
                    }
                });
            }
        }
    ...
    //only for tablet
    private void updateHotknotMenu () {
	
        boolean hasEvent = !((mEventCursor == null) || (mEventCursor.getCount() == 0));
        View hotknot = mView.findViewById(R.id.hotknot);

        // for HotKnot.
        boolean supportHotKnot = HotKnotAdapter.getDefaultAdapter(mContext) != null;
		
        ///M:hotknot callback shouldn't be null
        if(hotknot == null){
           return;
        }
		
        if (hasEvent && supportHotKnot) {
            hotknot.setVisibility(View.VISIBLE);
            hotknot.setEnabled(true);
        } else {
            hotknot.setVisibility(View.GONE);
            hotknot.setEnabled(false);
        }
    }
