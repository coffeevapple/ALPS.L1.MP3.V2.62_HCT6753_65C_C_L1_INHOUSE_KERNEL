...
public class SIMEditProcessor extends SIMProcessorBase {
    ...
    private boolean isPhoneNumberInvaild() {
        ...
        if (!TextUtils.isEmpty(sPhone)) {
            ...
            if (!Pattern.matches(SIM_NUM_PATTERN, PhoneNumberUtils.extractCLIRPortion(sAfterPhone))) {
                mNumberInvalid = true;
            }
            if (setSaveFailToastText()) {
        ...
        if (!TextUtils.isEmpty(sOtherPhone)) {
            ...
            if (!Pattern.matches(SIM_NUM_PATTERN, PhoneNumberUtils.extractCLIRPortion(sAfterOtherPhone))) {
                mNumberInvalid = true;
            }
            if (setSaveFailToastText()) {
                LogUtils.i(TAG, "[isPhoneNumberInvaild]setSaveFailToastText,return true.");
            ...
            } else if (Phone.CONTENT_ITEM_TYPE.equals(mimeType)) {
{
    ...
    private boolean isSIMUpdateValuesInvalid() {
        if (TextUtils.isEmpty(mUpdateName) && TextUtils.isEmpty(mUpdatephone)) {
            ...
            if (!Pattern.matches(SIM_NUM_PATTERN, PhoneNumberUtils.extractCLIRPortion(mUpdatephone))) {
                mNumberInvalid = true;
            }
        }
    ...
    private boolean isUSIMUpdateValuesInvalid() {
        ...
        if (!TextUtils.isEmpty(mUpdateAdditionalNumber)) {
            if (!Pattern.matches(SIM_NUM_PATTERN, PhoneNumberUtils.extractCLIRPortion(mUpdateAdditionalNumber))) {
