...
public class SoundRecorder extends Activity implements SoundRecorderService.OnEventListener,
    ...
    /// M: [BUG ADD] Judge if float window case @{
    ...
    /// M: [BUG ADD] Judge if tablet project @{
    private boolean isTablet() {
        Resources res = getResources();
        if (res.getConfiguration().smallestScreenWidthDp >= 533 || isFloatWindow()) {
            return true;
        } else {
            return false;
        }
    ...
    private void setTimerViewTextSize(int time) {
        ...
        if ((time / TIME_BASE) >= textSizeChangeBoundary) {
            if (isTablet()) {
                mTimerTextView.setTextSize(TypedValue.COMPLEX_UNIT_DIP,
                                           getResources().getDimension(
                                               R.dimen.timerView_TextSize_Small));
            } else {
                mTimerTextView.setTextSize(
                    getResources().getDimension(
                        R.dimen.timerView_TextSize_Small));
            }
        } else {
            if (isTablet()) {
                mTimerTextView.setTextSize(TypedValue.COMPLEX_UNIT_DIP,
                                           getResources().getDimension(
                                               R.dimen.timerView_TextSize_Big));
            } else {
                mTimerTextView.setTextSize(
                    getResources().getDimension(
                        R.dimen.timerView_TextSize_Big));
