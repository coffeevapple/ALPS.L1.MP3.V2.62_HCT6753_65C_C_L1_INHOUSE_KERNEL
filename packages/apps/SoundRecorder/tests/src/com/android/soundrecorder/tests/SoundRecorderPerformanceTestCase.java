...
public class SoundRecorderPerformanceTestCase extends ActivityInstrumentationTestCase2<SoundRecorder> {
    ...
    public void testCase01_StartAndStopAndSaveButtonPerformance() throws Exception {
        ...
        Log.i(TAG, "[Performance test][SoundRecorder] recording start [" + mStartRecordingTime + "]");
        mSolo.clickOnView(mRecordButton);
        mSolo.sleep(RECORDING_TIME);
        long mStartStopTime = System.currentTimeMillis();
        Log.i(TAG, "[Performance test][SoundRecorder] recording stop start [" + mStartStopTime + "]");
        mSolo.clickOnView(mStopButton);
        mSolo.sleep(WAIT_FOR_RESPOND_TIME);
        long mStartSaveTime = System.currentTimeMillis();
        Log.i(TAG, "[Performance test][SoundRecorder] recording save start [" + mStartSaveTime + "]");
