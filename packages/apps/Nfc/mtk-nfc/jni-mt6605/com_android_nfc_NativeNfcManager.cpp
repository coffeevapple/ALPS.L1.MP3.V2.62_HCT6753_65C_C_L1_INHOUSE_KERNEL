...
static         bool     quick_enable_mode = FALSE;
static        bool     dta_quick_mode = FALSE;
...
static bool nfc_jni_mtc_mos_mode(struct nfc_jni_native_data * nat,bool on_off);
...
static int nfc_jni_initialize(struct nfc_jni_native_data *nat) 
{
    ...
    ALOGI("NFC Initialized");
        ...
        if (quick_enable_mode == FALSE) {
            ...
            nfc_jni_mtc_mos_mode(nat,false);
...
//        if (false == NFC_FULL_POWER_CARD_MODE) {
//            ALOGD(
//                    "[K5]Start NFC_FULL_POWER_CARD_MODE && De-Initialization\n");
//            //get NFC mode
...
//        }
        ...
        ALOGD("[QE] %s quick_enable = %d",__FUNCTION__,quick_enable_mode);
            ...
            if(TRUE == dta_quick_mode){
                ...
                result=nfc_jni_mtc_mos_mode(nat,true);
...
static bool nfc_jni_mtc_mos_mode(struct nfc_jni_native_data *nat, bool on_off){
    ...
    if(true == on_off){
        ...
        response = nfc_jni_mtc_mos_PNFC(TRUE);
        ALOGD("[QE]%s: Enter MTC_mos_mode result %d",__FUNCTION__,response);
        ...
        response = nfc_jni_mtc_mos_PNFC(FALSE);
    }
    return response;
}
/*******************************************************************************
...
static jboolean com_android_nfc_NfcManager_doDownload(JNIEnv *e, jobject o)
{
    ALOGD("com_android_nfc_NfcManager_doDownload()");
        ...
        if(quick_enable_mode == TRUE) {
            ...
            response = nfc_jni_mtc_mos_mode(nat,TRUE);
    ...
    jint mode, jboolean actNow)
{
    ...
    // Let NFC into card mode (MTCMOS) when screen off 
    if (nat->rDiscoverConfig.u1OpMode == MODE_IDLE && actNow == true)
    {
        nat->rDiscoverConfig.u1OpMode |= MODE_CARD;
        ALOGD("no nfc function, set card mode only");

        ALOGD("[QE] %s NFC Enter MTC Mos mos mode ... ",__FUNCTION__);
                result = nfc_jni_mtc_mos_mode(nat,TRUE);
