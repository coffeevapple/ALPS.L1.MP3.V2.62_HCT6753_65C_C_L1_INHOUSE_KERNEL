...
int sim_init(void);
int sim_hot_plug_entry(struct ftm_param *param, void *priv);
int sim_hot_plug_init(void);
...
static item_t sim_hot_plug_items_auto_mode[] = {
    item(-1, NULL),
};
...
static void *sim_hot_plug_detect_thread(void *priv)
{
    struct sim_factory *sim = (struct sim_factory*)priv;
    struct itemview *iv = sim->iv;
    char cmd_buf[BUF_SIZE] = {0};
    char rsp_buf[BUF_SIZE] = {0};
    int tobreak = 0, ate_count = 1, isEsims = 0, rsp_len = 0, tr = 0;
    ...
    while(1) {
        ...
        if (!sim->test_done) {
            ...
            /* Use USIMSMT to do SIM Hot Plug GPIO Level detect */
            LOGD(TAG "------Test Hot Plug GPIO Level (SIM%d) start------\n", sim->sim_id);
            if (sim->sim_id == SIM_ID_1) {
                strcpy(cmd_buf, "AT+USIMSMT=11\r\n");
            } else if (sim->sim_id == SIM_ID_2) {
                strcpy(cmd_buf, "AT+USIMSMT=12\r\n");
            }
            write(fd_at, cmd_buf, strlen(cmd_buf));
            LOGD(TAG "Send %s\n", cmd_buf);
            usleep(HALT_TIME);
            while (1) {
                ...
                read_ack(fd_at, rsp_buf, BUF_SIZE);
                //usleep(HALT_TIME);
                LOGD(TAG "Response of USIMSMT %s\n", rsp_buf);
                if (strstr(rsp_buf, "OK") != NULL) {
                    sprintf(sim->info + strlen(sim->info),
                    "%s%d: %s.\n", uistr_info_detect_sim, sim->sim_id, uistr_info_pass);
                    LOGD(TAG "Detect Hot Plug GPIO Level SIM%d: Pass.\n",sim->sim_id);
                    break;
                } else if (strstr(rsp_buf, "ERROR") != NULL) {
                    sprintf(sim->info + strlen(sim->info),
                    "%s%d: %s!!\n", uistr_info_detect_sim, sim->sim_id, uistr_info_fail);
                    LOGD(TAG "Detect Hot Plug GPIO Level SIM%d: Fail.\n",sim->sim_id);
                    break;
                } else {
                    LOGD(TAG "Detect Hot Plug GPIO Level SIM%d: Unknown.\n",sim->sim_id);
                }
            }
            LOGD(TAG "------Test Hot Plug GPIO Level  end------\n", sim->sim_id);
        }
    }
...
int sim_hot_plug_entry(struct ftm_param *param, void *priv) {
    ...
    LOGD(TAG "param->test_type:%d\n", param->test_type);
    iv->set_items(iv, sim_hot_plug_items_auto_mode, 0);
    iv->start_menu(iv,0);
    iv->redraw(iv);
    ...
    sim->exit_thread = false;
        ...
        LOGD("not open md1,md2,md5");
    #endif
    pthread_create(&sim->update_thread, NULL, sim_hot_plug_detect_thread, priv);
    // Test SIM1
    LOGD(TAG "detect SIM1 Hot Plug");
    //  strcpy(sim->info, "");
    memset(sim->info, 0, sizeof(sim->info) / sizeof(*(sim->info)));
    sprintf(sim->info + strlen(sim->info), "%s\n", uistr_sim_hot_plug_sim1_start);
    iv->redraw(iv);
    memset(sim->info, 0, sizeof(sim->info) / sizeof(*(sim->info)));
    sim->sim_id = SIM_ID_1;
    sim->test_done = false;
    while (strlen(sim->info) == 0) {
        LOGD (TAG "detect slot 1:enter");
        LOGD (TAG "sim_entry:sim->info:%s, lenth:%zu",sim->info,strlen(sim->info));
        LOGD(TAG "FTM_AUTO_ITEM detect SIM1 Hot Plug sim->info:%s, uistr_info_pass:%s strstr:%s",
                sim->info, uistr_info_pass, strstr(sim->info, uistr_info_pass));
        usleep(200000);
        if (strstr(sim->info, uistr_info_pass)) {
            passCount++;
        }
    ...
    usleep(1000000);
    LOGD (TAG "end redraw");
#if !defined(MTK_SIM_HOT_SWAP_COMMON_SLOT)
    // Test SIM2
    #if defined(GEMINI) || defined(MTK_GEMINI_3SIM_SUPPORT)|| defined(EVDO_DT_VIA_SUPPORT) || defined(EVDO_DT_SUPPORT) || defined(MTK_C2K_SUPPORT) || defined(FTM_SIM_USE_USIMSMT)
        //Detect SIM 2
        //  strcpy(sim->info, "");
        LOGD(TAG "detect SIM2 Hot Plug");
        memset(sim->info, 0, sizeof(sim->info) / sizeof(*(sim->info)));
        sprintf(sim->info + strlen(sim->info), "%s\n", uistr_sim_hot_plug_sim2_start);
        iv->redraw(iv);
        memset(sim->info, 0, sizeof(sim->info) / sizeof(*(sim->info)));
        sim->sim_id = SIM_ID_2;
        sim->test_done = false;
...
#else
    passCount++;
    LOGD (TAG "Do not test SIM2 in CODECK project!");
#endif
...
int sim_hot_plug_init(void)
{
    ...
    mod = ftm_alloc(ITEM_SIM_HOT_PLUG_PIN_DETECT, sizeof(struct sim_factory));
    if(!mod) {
        return -ENOMEM;
    }
    sim = mod_to_sim(mod);
    ...
    ret = ftm_register(mod, sim_hot_plug_entry, (void*)sim);
    if(ret) {
        LOGD(TAG "register sim_hot_plug_entry failed (%d)\n", ret);
    }
