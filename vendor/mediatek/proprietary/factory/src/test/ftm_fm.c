...
if(FTM_AUTO_ITEM == param->test_type)
{
    ...
    if (g_rssi >= rssi_th) {
		...
		snprintf(fmr->info+strlen(fmr->info), sizeof(fmr->info), uistr_info_fmr_pass, rssi_th);
		iv->redraw(iv);
    } else {
        fmr->mod->test_result = FTM_TEST_FAIL;
        LOGD("check fail\n");
		//sprintf(fmr->info+strlen(fmr->info), uistr_info_fmr_failed, rssi_th);
		snprintf(fmr->info+strlen(fmr->info), sizeof(fmr->info), uistr_info_fmr_failed, rssi_th);
		iv->redraw(iv);
    }
	fmr->exit_thd = true;
...
static void *fm_update_thread(void *priv)
{
    ...
    while (1) {
		...
		snprintf(fmr->info+strlen(fmr->info), sizeof(fmr->info), uistr_info_fmr_fail);
                iv->redraw(iv);
            ...
            else
            {
               ...
               ret = 0;
                ...
                else
                {
                     ...
                     snprintf(fmr->info+strlen(fmr->info), sizeof(fmr->info), uistr_info_fmr_rssi, rssi);
