...
int vibrator_init(void)
{
    ...
    mod = ftm_alloc(ITEM_VIBRATOR, sizeof(struct mVibrator));
    mvibrator_autotest = mod_to_mVibrator(mod);
    mvibrator_autotest->mod = mod;
	if (!mod)
		return -ENOMEM;

    ret = ftm_register(mod, vibrator_entry, (void*)mvibrator_autotest);
