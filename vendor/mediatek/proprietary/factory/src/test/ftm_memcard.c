...
static void mcard_update_info_multi_storage(struct mcard_array *ma, char *info,int first)
{
    ...
    return_data.memcard.sd2_total_size=0;
    return_data.memcard.sd2_free_size=0;
    ...
    if (mc_fix->avail && !mc_fix->mounted) {
        ...
        if (mc_fix->id > hot_plug_host && mc_removable->avail) {
            len = sprintf(mc_fix->dev_path, "/dev/block/mmcblk%d", 1);
            mc_fix->blocknum = 1;
        } else {
            len = sprintf(mc_fix->dev_path, "/dev/block/mmcblk%d", 0);
            mc_fix->blocknum = 0;
        }
    ...
    return_data.memcard.sd1_total_size=(unsigned int)(stat.f_blocks * stat.f_bsize >> 20);
    return_data.memcard.sd1_free_size=(unsigned int)(stat.f_bfree * stat.f_bsize >> 20);
    ...
    if (mc_removable->avail && !mc_removable->mounted) {
        ...
        if ((mc_removable->id < mc_fix->id && first) || (mc_removable->blocknum == 0 )) {
            len = sprintf(mc_removable->dev_path, "/dev/block/mmcblk%d", 0);
            mc_removable->blocknum = 0;
        } else {
            len = sprintf(mc_removable->dev_path, "/dev/block/mmcblk%d", 1);
            mc_removable->blocknum = 1;
        }
        LOGD("mc_removable\n");
    ...
    return_data.memcard.sd2_total_size=(unsigned int)(stat.f_blocks * stat.f_bsize >> 20);
    return_data.memcard.sd2_free_size=(unsigned int)(stat.f_bfree * stat.f_bsize >> 20);
...
static void mcard_update_info(struct mcard *mc, char *info)
{
	...
	mc->mntpnt = g_mountpoint2;
        ...
        if (fd < 0) {
            ...
            if (fd < 0) {
#ifdef MTK_GPT_SCHEME_SUPPORT
                sprintf(mc->dev_path, "/dev/block/platform/mtk-msdc.0/by-name/intsd");
