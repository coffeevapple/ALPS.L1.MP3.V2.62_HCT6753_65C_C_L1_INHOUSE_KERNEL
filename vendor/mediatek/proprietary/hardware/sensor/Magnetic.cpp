...
int MagneticSensor::enable(int32_t handle, int en)
{
	...
	m_mag_last_ts[Orientation]=0;
	m_mag_last_ts[MagneticField]=0;
...
int MagneticSensor::setDelay(int32_t handle, int64_t ns)
{
    ...
    if(ID_ORIENTATION == handle)
    {
		 ...
		 m_mag_delay[Orientation] = ns;
    }
    if(ID_MAGNETIC == handle)
    {
        strcpy(&input_sysfs_path[input_sysfs_path_len], "magdelay");
		m_mag_delay[MagneticField] = ns;
    }
        ...
        if (type == EV_ABS) {
            ...
            for (int j=0 ; count && mPendingMask && j<numSensors ; j++)
            {
                ...
                if (mPendingMask & (1<<j))
                {
					...
					//ALOGE("fwq-mag(%d)[%lld, %lld] ,delay=%lld\r\n",j,mPendingEvent[j].timestamp,m_mag_last_ts[j],m_mag_delay[j]);
           			float delta_mod= (float)(mPendingEvent[j].timestamp-m_mag_last_ts[j])/(float)(m_mag_delay[j]);
                    if (m_mag_delay[j] == 1000000000)
                        delta_mod = 0;
					//ALOGE("fwq m-delta_mod=%f \r\n",delta_mod);
					int loopcout=delta_mod;
					//ALOGE("fwq m-loopcout=%d \r\n",loopcout);

					if(loopcout>=1 && loopcout<100)
					{
					 	for(int i=0; i<loopcout; i++)
					 	{
					 		mPendingEvent[j].timestamp = time- (loopcout-i)*m_mag_delay[j];
							//ALOGE("fwq_n m[j]fack event [%lld ] \r\n",j,mPendingEvent[j].timestamp);
					...
					m_mag_last_ts[j] = mPendingEvent[j].timestamp;
            ...
            if (EV_REL == type && EVENT_TYPE_ORIENT_UPDATE == event->code)
            {
                mPendingMask |= 1<<Orientation;
            }
            else if (EV_REL == type && EVENT_TYPE_MAG_UPDATE == event->code)
            {
