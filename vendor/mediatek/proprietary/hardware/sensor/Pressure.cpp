    ...
    while (count && mInputReader.readEvent(&event)) {
        ...
        else if (type == EV_SYN)
        {
            ...
            if (mEnabled)
            {
                ...
                {
                    float delta_mod= (float)(mPendingEvent.timestamp-m_baro_last_ts)/(float)(m_baro_delay);
                    ...
                    if(loopcout>=1 && loopcout<100)
                    {
                        ...
                        {
                            mPendingEvent.timestamp = time- (loopcout-i)*m_baro_delay;
            ...
            m_baro_last_ts = mPendingEvent.timestamp;
        }
        else if (type != EV_ABS)
        {
            ALOGE("BarometerSensor: unknown event (type=%d, code=%d)",
                    type, event->code);
        }
        mInputReader.next();
    }
