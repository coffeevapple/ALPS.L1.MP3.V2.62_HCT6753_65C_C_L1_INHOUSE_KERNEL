...
int bands[RIL_NW_NUM] = RIL_NW_INIT_INT;
...
//            if (response[3] > 6)
    ...
    err = at_send_command_singleline(
              "AT+ECSRA=3",
              "+ECSRA:", &p_response, NW_CHANNEL_CTX);
    ...
    if (err == 0 && p_response->success != 0 && p_response->p_intermediates != NULL){	
        ...
        if (err >= 0)
        {
            ...
            if ( err >=0 )
            {
                asprintf(&ratStr, "%d", rat);
                LOGD("update property ril.nw.rat.detail to %s", ratStr );
                property_set("ril.nw.rat.detail", ratStr);
                if (NULL != ratStr) {
                    free(ratStr);
                    ratStr = NULL;
                }			
...
//Femtocell (CSG) feature END
    ...
    case RIL_REQUEST_TRIGGER_LTE_BG_SEARCH:
        requestTriggerLteBgSearch(data, datalen, t);
        break;
    /// @}
...
int rilNwUnsolicited(const char *s, const char *sms_pdu, RILChannelCtx* p_channel)
{
    ...
    /// M: [C2K][IR][MD-IRAT] URC for GMSS RAT changed. @}
    /// M: [C2K] AP IRAT feature. @{
    else if (strStartsWith(s, "+ECAMPON:")) {
        LOGD("ECAMPON URC:%s",s);
        onLteBgSearchStatus((char *)s, rid);
        return 1;
    }
    /// @}
    /// M: [C2K][SVLTE] Set the SVLTE RAT mode. @{
