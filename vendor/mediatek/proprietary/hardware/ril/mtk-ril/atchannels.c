                ...
                long long timeoutMsec, ATResponse **pp_outResponse, RILChannelCtx *p_channel)
{
    ...
    if(s_md_off == 1 && !strstr(command, "EPOF") && !strstr(command, "EPON") && !strstr(command, "ESIMAUTH") && !strstr(command, "EAUTH")) {
    ...
    RLOGD("AT send on %s, tid:%lu", p_channel->myName, p_channel->tid_myProxy);
        ...
        //[ALPS01482098][mtk06800] 10 minutes detection mechanism for AT commands not replying
            ...
            timeoutMsec = s_at_timeoutMsec;
            RLOGE("AT command timeoutMsec = %d", timeoutMsec);
            ...
            if (err == AT_ERROR_TIMEOUT && atoi(modemException) != 1) {
               LOG_ALWAYS_FATAL("AT command pending too long, assert!!! on %s, tid:%lu, AT cmd: %s, AT command timeout: %d", p_channel->myName, p_channel->tid_myProxy, sCurCmd, timeoutMsec);
            } else if (err == AT_ERROR_TIMEOUT && atoi(modemException) == 1) {
               LOG_ALWAYS_FATAL("Modem already exception, assert!!! on %s, tid:%lu, last AT cmd: %s, AT command timeout: %d", p_channel->myName, p_channel->tid_myProxy, sCurCmd, timeoutMsec);
            }
        #else
	    err = at_send_command_full_nolock(command, type, responsePrefix, smspdu, timeoutMsec, pp_outResponse, p_channel);
    ...
    RLOGD("after reset tid, response received on %s, tid:%lu", p_channel->myName, p_channel->tid_myProxy);
    pthread_mutex_unlock(&p_channel->commandmutex);
...
{
    RILChannelCtx *channel;
    int channelOffset = rid;

#if (SIM_COUNT >= 2)
    channelOffset = rid*RIL_CHANNEL_OFFSET;
#endif /* MTK_GEMINI */
