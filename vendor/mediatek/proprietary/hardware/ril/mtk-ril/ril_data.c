...
with current CID0 APN, not to do re-attach");
        ...
        if (strlen(iaProperty) == 0) {
            ...
            if (strlen(param->password) != 0) {
                strncpy(TEMP_IA_PASSWORD, param->password, sizeof(TEMP_IA_PASSWORD));
            }
            ...
            if (!definePdnCtx(defaultBearer, apn, get_protocol_type(param->protocol), 0,
...
void onAttachApnReset(void* param)
{
    ...
    RILChannelCtx* pChannel = getRILChannelCtx(RIL_DATA, rilid);

    char iaProperty[PROPERTY_VALUE_MAX*2] = {0};
    getIaCache(iaProperty);
    
    if (0 == strlen(iaProperty)) {
        char* iaTempProperty = alloca(PROPERTY_VALUE_MAX*2);
        memset(iaTempProperty, 0, PROPERTY_VALUE_MAX*2);
        getTempIa(iaTempProperty);
        if (0 < strlen(iaTempProperty)) {
            char iccidProperty[PROPERTY_VALUE_MAX] = {0};
            property_get(PROPERTY_ICCID_SIM[rilid], iccidProperty, "");

            int authType = 0;
            char* iccid = alloca(PROPERTY_VALUE_MAX);
            char* protocol = alloca(PROPERTY_VALUE_MAX);
            char* apn = alloca(PROPERTY_VALUE_MAX);
            char* username = alloca(PROPERTY_VALUE_MAX);
            int canHandleIms = 0;
            memset(iccid, 0, PROPERTY_VALUE_MAX);
            memset(protocol, 0, PROPERTY_VALUE_MAX);
            memset(apn, 0, PROPERTY_VALUE_MAX);
            memset(username, 0, PROPERTY_VALUE_MAX);

            //The format of IA property is "[ICCID],[Protocol],[APN],[Auth Type],[Username]"
            //Ex. "89860011010746583662,IP,internet,0,username"
            at_tok_nextstr(&iaTempProperty, &iccid);
            if (!at_tok_hasmore(&iaTempProperty)) {
                LOGE("IA: onAttachApnReset property parsing iccid error");
                goto finish;
            }
            at_tok_nextstr(&iaTempProperty, &protocol);
            if (!at_tok_hasmore(&iaTempProperty)){
                LOGE("IA: onAttachApnReset property parsing protocol error");
                goto finish;
            }
            at_tok_nextint(&iaTempProperty, &authType);
            if (!at_tok_hasmore(&iaTempProperty)){
                LOGE("IA: onAttachApnReset roperty parsing authType error");
                goto finish;
            }
            at_tok_nextstr(&iaTempProperty, &username);
            if (!at_tok_hasmore(&iaTempProperty)){
                LOGE("IA: onAttachApnReset property parsing username error");
                goto finish;
            }
            at_tok_nextint(&iaTempProperty, &canHandleIms);

            at_tok_nextstr(&iaTempProperty, &apn);
            LOGD("IA: onAttachApnReset property parsing result [%s, %s, %d, %s, %d, %s]",
                iccid, protocol, authType, username, canHandleIms, apn);

            if (strlen(iccid) > 0 && strlen(protocol) > 0) {
                if (strcmp(iccidProperty, iccid) == 0) {
                    char* password = alloca(PROPERTY_VALUE_MAX);
                    memset(password, 0, PROPERTY_VALUE_MAX);

                    RIL_Default_Bearer_VA_Config_Struct* defaultBearer = NULL;

                    LOGD("IA: onAttachApnReset cache is matched with current SIM");
                    ...
                    if (0 != strlen(TEMP_IA_PASSWORD))
                        strncpy(password, TEMP_IA_PASSWORD, PROPERTY_VALUE_MAX);
                    else
                        strncpy(password, "", PROPERTY_VALUE_MAX);
                        
                    if (definePdnCtx(defaultBearer, apn, get_protocol_type(protocol), 0, getAuthTypeInt(authType),
                            username, password, pChannel)) {
                        LOGE("IA: onAttachApnReset define attach APN success");

                        if (0 == strlen(TEMP_IA_PASSWORD))
                            setIaCache(iccid, protocol, authType, username, canHandleIms, apn);
                        else
                            memset(TEMP_IA_PASSWORD, 0, sizeof(TEMP_IA_PASSWORD));

                        setTempIa("", "", 0, "", 0, "");
                    } else {
                        LOGE("IA: onAttachApnReset define attach APN error");
                    }
                } else {
                    LOGD("IA: onAttachApnReset SIM is changed");
                }
            } else {
                LOGE("IA: onAttachApnReset invalid IA property");
            }
        } else {
            LOGE("IA: onAttachApnReset all IA empty error");
        }            
    } else {
        LOGD("IA: onAttachApnReset IA Cache isn't empty and will be set later");
    }
