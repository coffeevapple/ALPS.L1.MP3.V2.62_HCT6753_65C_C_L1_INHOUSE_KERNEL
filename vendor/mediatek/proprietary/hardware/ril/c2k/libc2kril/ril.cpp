...
static void onRequestUnsupportComplete(int32_t token);
...
static int
processCommandBuffer(void *buffer, size_t buflen, int owner) {
    Parcel p;
    status_t status;
    int32_t request;
    int32_t token;
...
/*        case 2011:
        ...
        case 4019:
            ...
            break;
        case 4020:
            request = RIL_REQUEST_SET_LTE_EARFCN_ENABLED;
            break;
        case 4021:
    ...
    if (request < 1 || request >= (int32_t)NUM_ELEMS(s_commands)) {
        ...
        onRequestUnsupportComplete(token);
...
static void dispatchWritePbToRuim(Parcel &p, RequestInfo *pRI) {
    ...
    s_callbacks.onRequest(pRI->pCI->requestNumber, &write_pb_info, sizeof(write_pb_info), pRI);
...
static int responseDataCallList(Parcel &p, void *response, size_t responselen)
{
    ...
    if (s_callbacks.version < 5) {
        ...
        for (i = 0; i < num; i++) {
            ...
            p.writeInt32(p_cur[i].cid);
            p.writeInt32(p_cur[i].active);
			p.writeInt32(p_cur[i].mtuSize);
            writeStringToParcel(p, p_cur[i].type);
            writeStringToParcel(p, p_cur[i].ifname);
            writeStringToParcel(p, p_cur[i].addresses);
            writeStringToParcel(p, p_cur[i].dnses);
            writeStringToParcel(p, p_cur[i].gateways);
            appendPrintBuf("%s[status=%d,retry=%d,cid=%d,%s,%d,%s,%s,%s,%s],", printBuf,
                p_cur[i].status,
...
static void onCommandsSocketClosed(int owner) {
    int ret;
    RequestInfo *p_cur;
...
//c+w
    if(count == 0){
        ...
        onCommandsSocketClosed(1);
        free(readbuf);
        readbuf = NULL;
        return;
    }
    processCommandBuffer(readbuf, count, 1);
    free(readbuf);
    readbuf = NULL;
}
...
static void processCommandsCallback(int fd, short flags, void *param) {
    ...
    for (;;) {
        ...
        if (ret == 0 && p_record == NULL) {
            ...
            processCommandBuffer(p_record, recordlen, 0);
        }
    }
...
static void listenCallback (int fd, short flags, void *param) {
    ...
    int fdCommand = accept(s_fdListen, (sockaddr *) &peeraddr, &socklen);
    ...
    ril_event_set (&s_commands_event, fdCommand, 1,
        processCommandsCallback, p_rs);

    rilEventAddWakeup (&s_commands_event);

    /// M: Assign s_fdCommand here, to avoid RIL_onUnsolicitedResponse reset and delete s_commands_event
    s_fdCommand = fdCommand;
    onNewCommandConnect();
}
...
RIL_onRequestComplete(RIL_Token t, RIL_Errno e, void *response, size_t responselen) {
    ...
    if (pRI->cancelled == 0) {
        ...
        if ((pRI->pCI->requestNumber >= RIL_REQUEST_SET_AUDIO_PATH) && (pRI->pCI->requestNumber <= RIL_REQUEST_SET_MUTE_FOR_RPC)) {
