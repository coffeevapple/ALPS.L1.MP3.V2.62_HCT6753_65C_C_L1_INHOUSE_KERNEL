...
/// M: [C2K][IR] Support SVLTE IR feature. @}
static void requestSetRegResume(void *data, size_t datalen, RIL_Token t);
static void requestEnableRegPause(void *data, size_t datalen, RIL_Token t);
...
static void requestSetLteEarfcnEnabled(void *data, size_t datalen, RIL_Token t);
...
static int isRegPauseEnable(void);
static void onEngModeInfoUpdate(char* urc);
static void onLteArfcnInfo(char* urc);
...
extern int rilNwMain(int request, void *data, size_t datalen, RIL_Token t)
{
    switch (request) {
        ...
        case RIL_REQUEST_SET_NETWORK_SELECTION_AUTOMATIC:
            at_send_command("AT+COPS=0", NULL, NW_CHANNEL_CTX);
            break;
        ...
        case RIL_REQUEST_QUERY_AVOID_SYS:
            ...
            break;
        case RIL_REQUEST_SET_LTE_EARFCN_ENABLED:
            requestSetLteEarfcnEnabled(data, datalen, t);
        break;
        /// M: [C2K][IR] Support CT 3g dual Mode card IR feature. @{
...
extern int rilNwUnsolicited(const char *s, const char *sms_pdu) {
    ...
    if (strStartsWith(s, "^SYSINFO:")) {
        ...
        case NET_TYPE_CDMA_ONLY:
            if (service_state_1x == 0) {
                ...
                s_unsol_msg_cache.service_state = 0;
                s_unsol_msg_cache.x1_service_state = 0;
            } else if (service_state_1x == 3) {//[ALPS02039951]
    ...
    else if (strStartsWith(s, "+VCPCOMPILE:"))
    {
        ...
        return 1;
    } else if (strStartsWith(s, "+ELTEARFCN:")) {
        LOGD("ELTEARFCN: %s", s);
        onLteArfcnInfo(s);
        return 1;
...
static void onLteArfcnInfo(char* urc)
{
    char *dup = NULL;
    int numOfArfcns;
    int arfcns[MAX_NUM_OF_ARFCNS + 1] = {-1};
    int i;
    char *line = NULL;
    int err;
    ...
    if (NULL == urc)
    {
        LOGE("ELTEARFCN input urc is NULL\r\n");
        return;
    }
    LOGD("ELTEARFCN input urc = \"%s\"\r\n", urc);

    dup = strdup(urc);
    if (NULL == dup)
    {
        LOGE("+ELTEARFCN: Unable to allocate memory");
        return;
    }
    ...
    err = at_tok_nextint(&line, &numOfArfcns);
    if (err < 0) {
        free(dup);
        return;
    }
    if ((numOfArfcns < MIN_NUM_OF_ARFCNS) || (numOfArfcns > MAX_NUM_OF_ARFCNS)) {
        LOGD("numOfArfcnss is invalid: %d", numOfArfcns);
        return;
    }
    LOGD("numOfArfcns is %d",numOfArfcns);

    arfcns[0] = numOfArfcns;
    for (i = 1; i < numOfArfcns + 1; i++) {
        err = at_tok_nextint(&line, &arfcns[i]);
        if (err < 0) {
            free(dup);
            return;
        }
        if ((arfcns[i] < 0) || (arfcns[i] > ARFCN_MAX)) {
            LOGD("arfcn is invalid: %d", arfcns[i]);
            return;
        }
        LOGD("arfcns[%d] = %d",i, arfcns[i]);
    }
    RIL_onUnsolicitedResponse(RIL_UNSOL_LTE_EARFCN_INFO, arfcns,
            (numOfArfcns + 1) * sizeof(int));
    free(dup);
    return;
}
    ...
    if (onOff == 0 && getRadioState() != RADIO_STATE_OFF) {
        ...
        if (getRadioState() != RADIO_STATE_RUIM_READY) {
            if (isRadioOn() != 1) {
				...
				/// M:[C2K] add for DSDA to allow data request after modem on.
				allowDataRequest();
...
static void requestBasebandVersion(void *data, size_t datalen, RIL_Token t)
{
    ...
    char * line_cp = NULL;
    char * response_string9 = NULL;
    char * response_string10 = NULL;
    ...
    err = at_send_command_singleline("AT+VCPCOMPILE?", "+VCPCOMPILE", &p_response_cp, NW_CHANNEL_CTX);
    if ((err != 0) || (p_response_cp->success == 0)) {
        goto error;
    }

    line_cp = p_response_cp->p_intermediates->line;
    err = at_tok_start(&line_cp);
    if (err < 0) {
        goto error;
    }
    err = at_tok_nextstr(&line_cp, &response_string9);
    if (err < 0) goto error;
    err = at_tok_nextstr(&line_cp, &response_string10);
    if (err < 0) goto error;
    ...
    #else
    asprintf(&response, "%s, 20%s/%s/%s %s:%s", response_string11, response_string6,response_string4,response_string5,response_string7,response_string8);
...
static void requestSetLteEarfcnEnabled(void *data, size_t datalen, RIL_Token t)
{
    ...
    LOGD("%s: value is %d", __FUNCTION__, value);
    asprintf(&cmd, "AT+ELTEARFCN=%d", value);
    err = at_send_command(cmd, &p_response, NW_CHANNEL_CTX);
    free(cmd);
    if ((err < 0) || (p_response->success == 0))
    {
