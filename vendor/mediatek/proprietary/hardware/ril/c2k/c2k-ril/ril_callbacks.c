...
{
    VIATEL_CHANNEL_AT,
#ifndef MTK_C2K_DATA_PPP_SUPPORT
    VIATEL_CHANNEL_AT2,
    VIATEL_CHANNEL_AT3
#endif
};
...
int inemergency = 0;
static char * operators_map[][3] = {{"46003", "CHINA TELECOM",    "CT"}, \
                                   {"46005",  "CHINA TELECOM",    "CT"}, \
                                   {"46000",  "CHINA MOBILE",     "CMCC"}, \
                                   {"46001",  "CHINA UNICOM",     "CU"}, \
                                   {"310004", "Verizon Wireless", "VZ"}, \
...
void getChannelPath(int channelId, char *driverPath, char *devicePath) {
    int atChannelCnt = 0;
    int atResetCnt = 0;
    int sleepTime = 10; /*s*/
    ...
    char atpath[32];
    for (;;) {
        err = waitForAdjustPortPath(channelId, driverPath, atpath);
        if (err < 0) {
            //LOGE("adjust port error, continue");
        ...
        LOGD("atpath = %s", atpath);
        if (strStartsWith(atpath, "/dev")) {
            devicePath = atpath;
            break;
        }
    }
}
#endif /* #ifndef FLASHLESS_SUPPORT */

void initATChannelPathes()
{
#ifndef FLASHLESS_SUPPORT
    DIR *dirp;
    char *sysfsdrvpath = NULL;
    if((dirp = opendir(SYSFS_USB_DRIVER_OLD)) != 0) {
        sysfsdrvpath = SYSFS_USB_DRIVER_OLD;
        closedir(dirp);
    } else if ((dirp = opendir(SYSFS_USB_DRIVER_NEW)) != 0) {
        sysfsdrvpath = SYSFS_USB_DRIVER_NEW;
        closedir(dirp);
    }
#endif /* FLASHLESS_SUPPORT */
    ...
    {
#ifndef FLASHLESS_SUPPORT
        getChannelPath(s_mux_fd_property_list[i], sysfsdrvpath, s_mux_path[i]);
        sysfsdrvpath = NULL;
#else /* FLASHLESS_SUPPORT */
...
static void *mainLoop(void *param)
{
    ...
    /**
     * In statusd mode, RILD is enabled by statusd after CBP "+VPUP" has been received.
     * So, here could be considered as the cbp powered on indicator roughly.
     */
    sendPincode();
#endif /* USE_STATUSD */
...
static void reportUSBDisconn()
{
    ...
    if(getRadioState() != RADIO_STATE_UNAVAILABLE)
    {
         ...
         if(!callStatus.isDataCall && (s_PcModemState != PCMODEM_DISCONNECT))
         {
            ...
            RIL_Data_Call_Response_v6 *datacall_response = NULL;
            char name[PROPERTY_VALUE_MAX];
            datacall_response = (RIL_Data_Call_Response_v6 *) alloca(
                    sizeof(RIL_Data_Call_Response_v6));
            memset(datacall_response, 0, sizeof(RIL_Data_Call_Response_v6));
            ...
            RIL_onUnsolicitedResponse(RIL_UNSOL_DATA_CALL_LIST_CHANGED,
                    datacall_response, sizeof(RIL_Data_Call_Response_v6));
...
int waitForAdjustPortPath(int interface_num, char *sysfsdrv, char devpath[])
{
    ...
    char path[64];
    dirp = opendir(sysfsdrv);
    if(!dirp)
    {
        //LOGE("waitForAdjustPortPath open %s error  errno = %d",sysfsdrv, errno);
        return -1;
    }
    do {
        n = 0;
        dirent = readdir(dirp);
        if(!dirent) {
            //LOGE("waitForAdjustPortPath  break line %d  errno = %d",__LINE__,errno);
            closedir(dirp);
            return -1;
        }
        ...
        if(dirent->d_name[n++] != '.')
            continue;
        if(interface_num == AT_INTERFACE)
        {
            if(dirent->d_name[n++] != '2')
                continue;
        }
        else if (interface_num == DATA_INTERFACE)
        {
            if(dirent->d_name[n++] != '0')
                continue;
        }
        else
        {
            LOGE("interface error");
            closedir(dirp);
            return -1;
        }
        //form new path
        memset(path, 0, sizeof(path));
        strcat(path, sysfsdrv);
        strcat(path, dirent->d_name);
        closedir(dirp);
        LOGD("waitForAdjustPortPath path = %s",path);
        //open this path
        dirp = opendir(path);
        if(!dirp)
        {
            LOGE("can't open path %s",path);
            return -1;
        }
        do {
            n = 0;
            ...
            if(!dirent)
            {
                //LOGE("waitForAdjustPortPath error line %d errno = %d", __LINE__, errno);
                closedir(dirp);
                return -1;
            }
            if(strncmp(dirent->d_name, "ttyUSB", strlen("ttyUSB")))
                continue;
            //we got it, finally
            memset(devpath, 0, sizeof(devpath));
            strcat(devpath, "/dev/");
            strcat(devpath, dirent->d_name);
            LOGD("devpath adjust to %s\n", devpath);
            closedir(dirp);
            return 0;
        } while(1);
        break;
...
int isC2KWorldPhoneP2Support() {
    ...
    char property_value[PROPERTY_VALUE_MAX] = { 0 };
    property_get("ro.mtk.c2k.slot2.support", property_value, "0");
