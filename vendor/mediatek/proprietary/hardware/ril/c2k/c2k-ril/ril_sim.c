        ...
        if (strStartsWith(s, "+UIMST:")) {
            ...
            if ((strcmp(plugstate, "true")) && (!strcmp(pinstate, "true"))) {
             ...
             if ((CARD_NOT_INSERTED == cardState) || (UNKOWN_CARD == cardState)
                ...
                if ((mode != 0) && (cardState == CARD_NOT_INSERTED) && !isSvlteSupport()) {
                    ...
                    if (strcmp("1", ct3gRoaming2) == 0) {
                        ...
                        property_set(PROPERTY_ICCID_SIM[getCdmaSocketSlotId() - 1], "N/A");
                        RLOGD("Reset %s to N/A", PROPERTY_ICCID_SIM[getCdmaSocketSlotId() - 1]);
                    }
                ...
                property_get(PROPERTY_ICCID_SIM[getCdmaSocketSlotId()-1], svlteIccidStatus, "");
                RLOGD("svlteIccidStatus:%s", svlteIccidStatus);
                if (isSvlteSupport() && (!strcmp(svlteIccidStatus,"N/A"))) {
		            setIccidProperty(CLEAN_ICCID, NULL);
                    property_set(PROPERTY_ICCID_SIM[getCdmaSocketSlotId()-1], "");
                    RLOGD("Reset %s to NULL", PROPERTY_ICCID_SIM[getCdmaSocketSlotId()-1]);
                }
...
static void requestSimOpenNFCChannel(void *data, size_t datalen, RIL_Token t)
{
    ...
    c = (char *)data;
    LOGD("requestOpenNFCChannel data is %s",c);
...
static void requestSimCloseNFCChannel(void *data, size_t datalen, RIL_Token t)
{
    ...
    LOGD("the sessionid is %d", sessionid);
...
static void requestSimTransmitChannel(void *data, size_t datalen, RIL_Token t)
{
    ...
    LOGD("requestSimTransmitChannel entering ....  %s",(char*)data);

    memset(&sr, 0, sizeof(sr));
    p_args = (RIL_SIM_APDU *)data;

    if((p_args->data == NULL) ||(strlen(p_args->data) == 0)) {
        LOGD("%s p3:%02x", __FUNCTION__, p_args->p3);
        ...
        if(p_args->p3 < 0) {
            asprintf(&cmd, "AT+CGLA=%d,%d,\"%02x%02x%02x%02x\"",
                            ...
                            p_args->p1, p_args->p2);
            LOGD("%s, p3:%02x, command:%s", __FUNCTION__, p_args->p3, cmd);
            err = at_send_command_singleline(cmd, "+CGLA:", &p_response, SIM_CHANNEL_CTX);
            free(cmd);
           ...
           sr.simResponse[len - 4] = '\0';
            ...
            LOGD("requestCGLA_OpenChannelWithSw err = %d", err);
        }
    } else {
    ...
    LOGD("p_args->instruction:%02x, sr.sw1:%02x", p_args->instruction, sr.sw1);
    // end sim toolkit session if 90 00 on TERMINAL RESPONSE
...
fetch:
    ...
    if(err != RIL_E_SUCCESS) {
        LOGD("%s, requestSimChannelAccess return error",__FUNCTION__);
        goto error;
    }
 ...
 *return to framework:SUCCESS or GENERIC_FAILURE
 */
static void requestSimopenNFCChannelWithSw(void *data, size_t datalen,
        RIL_Token t) {
    int err = 0;
    ATResponse *p_response = NULL;
    char* cmd = NULL;
    ...
    LOGD("requestSimopenNFCChannelWithSw entering ....  %s", (char*) data);
    ...
    if (err == RIL_E_SUCCESS) {
        ...
        LOGD("%s:NFCchannel is:%d", __FUNCTION__, NFCchannel);
...
//                               8, 0x00, 0x70, 0x80, NFCchannel);
    ...
    status[1] = sr.sw1;
    status[2] = sr.sw2;
    RLOGD("requestSimopenNFCChannelWithSw, Pass, channel = %d %02x, %02x", status[0], status[1], status[2]);
    RIL_onRequestComplete(t, RIL_E_SUCCESS, status, 3*sizeof(int));
    return;

    error: srResponse[0] = (char) '0';
    srResponse[1] = (char) '0';
    srResponse[2] = (char) '\0';
    ...
    status[1] = sr.sw1;
    status[2] = sr.sw2;
    RLOGD("requestSimopenNFCChannelWithSw, Fail, channel = %d %02x, %02x", status[0], status[1], status[2]);
...
{
    LOGE("%s:channel:%d, length:%d, cla:%d, ins:%d, p1:%d, p2:%d, p3:%d", __FUNCTION__, NFCchannel, length, cla, ins, p1, p2, p3);
    int err = 0;
    ATResponse *p_response = NULL;
    char *line = NULL;
    char *cmd = NULL;
    ...
    srResponse = (char *)malloc(sizeof(char) * 600);
redo:
    *len = 0;
    ...
    if(sr->sw1 == 0x61) {
        ...
        if (sr->sw1 == 0x62 && sr->sw2 == 0x83) {
            LOGD("requestCGLA_OpenChannelWithSw: select file deactivated!!");
        } else {
...
static void readAllUnreadSmsOnME(void) {
    ...
    err = at_send_command_multiline ("AT^HCMGL=0", "^HCMGL:", &p_response, getChannelCtxbyId(MISC_CHANNEL));
    if ((err < 0) || (p_response->success == 0))
    {
    ...
    if (indexCount > 0) {
        err = at_send_command_singleline("AT+VCGMM", "+VCGMM", &p_cbp_version_response, getChannelCtxbyId(MISC_CHANNEL));
