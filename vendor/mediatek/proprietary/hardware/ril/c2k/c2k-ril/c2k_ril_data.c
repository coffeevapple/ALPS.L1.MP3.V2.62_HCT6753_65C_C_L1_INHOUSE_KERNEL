...
void fillDataCallResponse(int interfaceId, RIL_Data_Call_Response_v6* response);
void dumpDataResponse(RIL_Data_Call_Response_v6* response, const char* reason);
...
void requestSetupDataCall(void * data, size_t datalen, RIL_Token t)
{
	...
	RIL_Data_Call_Response_v6* response = (RIL_Data_Call_Response_v6*) calloc(1,
	...
	int *activatedCidList = (int *) calloc(1, nPdnInfoSize * sizeof(int)); // Acivated cids of PDN connections.
...
void requestDataAttach(void* data, size_t datalen, RIL_Token t)
{
	...
	ATResponse *p_response = NULL;
	int isDataAttach = ((int *) data)[0];
	LOGD("[RILData_C2K_Util] requestDataAttach isDataAttach= %x", isDataAttach);

	asprintf(&cmd, "AT+CGATT=1");
	err = at_send_command(cmd, &p_response, DATA_CHANNEL_CTX);

	if (isATCmdRspErr(err, p_response)) {
		LOGD("[RILData_C2K_Util] requestDataAttach fail with CGATT, err = " + err);
	}
	AT_RSP_FREE(p_response);
	free(cmd);
}
...
void allowDataRequest() {
    if (isSvlteSupport()) {
        ...
        int isDataOn = getDataEnable();
        int defaultSim = getDefaultDataSim();
        LOGD("[RILData_C2K] SVLTE allowDataRequest: isDataOn = %d, curSim = %d, defaultSim = %d.",
            isDataOn, curSim, defaultSim);
        if (defaultSim == curSim || (defaultSim == 0 && curSim == get4GCapabilitySlot() && isC2kSimInsert())) {
            RIL_onUnsolicitedResponse(RIL_UNSOL_DATA_ALLOWED, NULL, 0);
        }
    } else {
        // DT support, allow data when device boot.
...
void requestOrSendDataCallList(RIL_Token t)
{
	...
	char *out = NULL;
	int i = 0;
	RIL_Data_Call_Response_v6* responsesOfActive = NULL;
	int activeCount = 0;
	RIL_Data_Call_Response_v6* responses = (RIL_Data_Call_Response_v6*) calloc(1,
			MAX_CCMNI_NUMBER * sizeof(RIL_Data_Call_Response_v6));
	...
	if (activeCount > 0) {
		responsesOfActive = (RIL_Data_Call_Response_v6*) calloc(1,
				activeCount * sizeof(RIL_Data_Call_Response_v6));
...
void fillDataCallResponse(int interfaceId, RIL_Data_Call_Response_v6* response)
{
	...
	for (i = 0; i < nPdnInfoSize; i++) {
		if (pPdnInfo[i].interfaceId == interfaceId) {
            ...
            response->mtuSize = pPdnInfo[i].mtu;
		}
	}
...
void freeDataResponse(RIL_Data_Call_Response_v6* response)
{
