...
#ifdef RIL_SHLIB
extern "C" void RIL_onUnsolicitedResponseSocket(int unsolResponse, void *data,
...
static RIL_RadioState callOnStateRequest(RIL_SOCKET_ID rid)
{
...
static void dispatchRadioCapability(Parcel &p, RequestInfo *pRI){
    ...
    string8 = strdupReadString(p);
    strncpy(rc.logicalModemUuid, string8, MIN(strlen(string8), MAX_UUID_LENGTH));
    if (string8) {
        free(string8);
    }
...
static void onCommandsSocketClosed(RIL_SOCKET_ID socket_id) {
    ...
    if (socket_id == RIL_SOCKET_1) {
        ...
        //If SIM_COUNT >=2:It's MSIM case and need to update right command channel
#if (SIM_COUNT >= 2) 
            pRI->cid = RIL_CMD2_2;
#if (SIM_COUNT >= 4)
        if (socket_id == RIL_SOCKET_4)
...
static int handleSpecialRequestWithArgs(int argCount, char** args){
    ...
    if (1 == argCount)
    {
        ...
        if (at_tok_hasmore(&line)) {
            ...
            if (err < 0 || slotId >= RIL_SOCKET_NUM) {
...
extern "C"
void RIL_onUnsolicitedResponseSocket(int unsolResponse, void *data,
                                size_t datalen, RIL_SOCKET_ID socket_id)
{
