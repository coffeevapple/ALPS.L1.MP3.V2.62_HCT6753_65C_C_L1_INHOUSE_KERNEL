...
handlePanoImgCallBack(MVOID* const puJpegBuf, int u4SrcWidth, int u4SrcHeight)
{
    ...
    BasObj->captureDoneCallback(MTK_CAMERA_MSG_EXT_DATA_AUTORAMA
                                , 2
                                , (uintptr_t)pMPImageInfo.imageBuf
...
handleMotionTrackImgCallBack(MVOID* const puJpegBuf, int u4SrcWidth, int u4SrcHeight)
{
    ...
    MINTPTR trueJpegBuf = ((MINTPTR*) puJpegBuf)[4];
    MY_LOGD("[handleJpegCallBack] + integer(%p,%p,%p) (puJpgBuf, u4SrcWidth, u4SrcHeight) = (%p, %d , %d)", ((MINTPTR*) puJpegBuf)[0], ((MINTPTR*) puJpegBuf)[1], ((MINTPTR*) puJpegBuf)[2], trueJpegBuf, u4SrcWidth, u4SrcHeight);

    if ((((MINTPTR*) puJpegBuf)[0] == 3) || /* Callback on intermediate data */
        ((((MINTPTR*) puJpegBuf)[0] == 2) && /* Callback with no blended image */
         (((MINTPTR*) puJpegBuf)[2] == 0)))
    {
        if ((((MINTPTR*) puJpegBuf)[0] == 2) && (((MINTPTR*) puJpegBuf)[2] == 0))
        {
            /* Callback with no blended image */
            pmem = BasObj->mpCamMsgCbInfo->mRequestMemory(-1, sizeof(MUINT32[5]), 1, NULL);
            ...
            MY_LOGD("[performCallback] + byte(%d,%d,%d,%d)", pCBData[0], pCBData[1], pCBData[2], pCBData[3]);
        }
        else
        {
            /* Callback on intermediate data */
            pmem = BasObj->mpCamMsgCbInfo->mRequestMemory(-1, sizeof(MUINT32[2]) + ((MINTPTR*) puJpegBuf)[1], 1, NULL);
            pCBData = reinterpret_cast<uint32_t*>(pmem->data);
            pCBData[0] = MTK_CAMERA_MSG_EXT_DATA_MOTIONTRACK;
            pCBData[1] = 3;
            memcpy((void*) &(pCBData[2]) , (void*) ((MINTPTR*) puJpegBuf)[2], ((MINTPTR*) puJpegBuf)[1]);

#ifdef MOTIONTRACK_DEBUG
            char sourceFiles[80];
            sprintf(sourceFiles, "/sdcard/intermediatedata");
            dumpBufToFile((char *)sourceFiles, (uint8_t*)&(pCBData[2]), ((MINTPTR*) puJpegBuf)[1]);
#endif
        ...
        // Capture sound callback:
        BasObj->mpCamMsgCbInfo->mDataCb(
                MTK_CAMERA_MSG_EXT_DATA,
                pmem,
                0,
                NULL,
                BasObj->mpCamMsgCbInfo->mCbCookie
                );
    ...
    pmem = BasObj->mpCamMsgCbInfo->mRequestMemory(-1, ((sizeof(MUINT32[6]) + u4ExifHeaderSize + u4JpegSize) + 0x3) & ~0x3, 1, NULL);
    pCBData = reinterpret_cast<uint32_t*>(pmem->data);

    pCBData[0] = MTK_CAMERA_MSG_EXT_DATA_MOTIONTRACK;
    pCBData[1] = ((MINTPTR*) puJpegBuf)[0];
    pCBData[2] = ((MINTPTR*) puJpegBuf)[1];
    pCBData[3] = ((MINTPTR*) puJpegBuf)[2];
    memcpy((void*) &(pCBData[5]) , puExifHeaderBuf, u4ExifHeaderSize);
    memcpy((void*) (((MINTPTR) &(pCBData[5])) + u4ExifHeaderSize), (void*) jpegBuf->getBufVA(0), u4JpegSize);
...
handleMAVImgCallBack(MVOID* const puJpegBuf, int u4SrcWidth, int u4SrcHeight)
{
    ...
    for (MUINT8 i = (BasObj->mShotNum - 1); i > 0 ; i--) {  	  
        ...
        MY_LOGD("MAV JPG: %d Adr %p size %d", i, pMPImageInfo[i].imageBuf, pMPImageInfo[i-1].imageSize);
