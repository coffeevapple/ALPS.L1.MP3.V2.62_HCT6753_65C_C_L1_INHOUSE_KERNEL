...
using namespace std;
//
#define DEFAULT_SIZE        (0xFF7E)
#define APPN_SIZE           (DEFAULT_SIZE + 0x02) // 128-aligned
#define APPN_COUNT          (4)
    ...
    **********************************************************************************/
    {
        ...
        if ( ::strcmp(make, "0") != 0 ) {
            ...
            ::strncpy((char*)exifApp1Info->strMake, (const char*)make, 32);
        }
        // [Model]
        if ( ::strcmp(model, "0") != 0 ) {
            ::memset(exifApp1Info->strModel, 0, 32);
            ::strncpy((char*)exifApp1Info->strModel, (const char*)model, 32);
        }
    }
...
updateDbgExif()
{
    ...
    MUINT8* pDst            = (MUINT8*)mpOutputExifBuf + getStdExifSize() + getThumbnailSize();
...
)
{
    ...
    {
        MY_LOGW("ID_%s(0x%04x) did not exist", mpDebugIdMap->stringFor(dbgModuleID).string(), dbgModuleID);
        MUINT8* pDbgModuleInfo = new MUINT8[DEFAULT_SIZE];
        ::memset(pDbgModuleInfo, 1, sizeof(MUINT8)*DEFAULT_SIZE);
        mpBaseExif->exifAppnMake(dbgAppn, *ppuAppnBuf, pDbgModuleInfo, DEFAULT_SIZE, &appnReturnSize);
        *ppuAppnBuf += appnReturnSize;
