...
)
{
    ...
    for (size_t i = 0; i < pMyUserGraph->size(); i++) {
        ...
        if  ( ! found ) {
            found = MTRUE;
            MY_LOGD_IF(0, "0-indegree user: set acquire fence:%d", getAcquireFence());
            err = pMyUserGraph->setAcquireFence(i, getAcquireFence());
        }
        else {
            MINT fence = ::dup(getAcquireFence());
            MY_LOGW("another 0-indegree user; need dup acquire fence:%d->%d", getAcquireFence(), fence);
