/*======================================================================*
 ...
 AND ANY RELATED DOCUMENTATION IS AT THE RECIPIENTS SOLE RISK:
...
#define POSTPRO_PATCH_OUTPROC_PLAY_SAMPLES(a, fmt, buf, bsize, rate, count) \
    if (((fmt == AUDIO_FORMAT_PCM_16_BIT) || (fmt == AUDIO_FORMAT_PCM_32_BIT))    && SRS_DoOutput()){ \
        SRS_Processing::ProcessOut(SRS_Processing::AUTO, a, buf, bsize, rate, count, fmt); \
    }
#else
#define POSTPRO_PATCH_OUTPROC_PLAY_SAMPLES(a, fmt, buf, bsize, rate, count) \
    if (((fmt == AUDIO_FORMAT_PCM_16_BIT) || (fmt == AUDIO_FORMAT_PCM_32_BIT))    && SRS_DoOutput()){ \
...
#define POSTPRO_PATCH_OUTPROC_PLAY_ROUTE(a, para, val) \
    if (SRS_DoOutput()){ \
        SRS_LOG("SRS_Processing - OutNotify_Route: a = %d, val = %d\n", a, val); \
