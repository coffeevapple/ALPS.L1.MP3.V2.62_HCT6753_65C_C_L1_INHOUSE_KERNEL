...
static int get_meta_com_id(void)
{
  ...
  s = read(fd, (void *)&com_id, sizeof(char));
  close(fd);
...
static int queue_com_triggers_action()
{
     ...
     * Please refernce meta_inti.rc and factory_init..rc
     */
    if(meta_com_id && (boot_mode == MT_META_BOOT || boot_mode == MT_FACTORY_BOOT || boot_mode == MT_ATE_FACTORY_BOOT))
    {
