...
int main(int argc, char **argv)
{
    ...
    __android_log_print(ANDROID_LOG_INFO, "ip-up-cdma", "suffix Version: %s", VIA_SUFFIX_VERSION);
    ...
    //To the original code of DataConnection.java we should set net.<ifname>.dnsX,
    //When we set the props to net.<ifname>.dnsX,
    //MobileDataStateTracker should set mDnsPropNames to net.<mInterfaceName>.dnsX
    ...
    /*
    sprintf(tmp,"/system/bin/route add default gw %s dev %s",iplocal,ifname);
    __android_log_print(ANDROID_LOG_INFO, "ip-up-cdma", "----%s\n", tmp);
    system(tmp);
    */

    property_set("net.cdma.linkup", iplocal ? "yes" : "no");
    property_set("net.cdma.pppd_via.terminated", "no");

    if(iplocal)
    {
        __android_log_print(ANDROID_LOG_INFO, "ip-up-cdma", "Cdma ppp link UP OK, IP=%s", iplocal);
    }
    else
    {
        __android_log_print(ANDROID_LOG_INFO, "ip-up-cdma", "Cdma ppp link UP error!");
    }
