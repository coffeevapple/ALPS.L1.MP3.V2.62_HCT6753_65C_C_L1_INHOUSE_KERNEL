...
case TRANSACTION_writeImsi1: {
    ...
    case TRANSACTION_writeIccID1: {
        ...
        data.enforceInterface(descriptor);
        int len = data.readInt32();
        XLOGD("writeIccID1 len  = %d\n", len);
        if (len == -1) { // array is null
            reply->writeInt32(0);
        } else {
            char buff[len];
            data.read(buff, len);
            XLOGD("writeIccID1 buff  = %s\n", buff);
            reply->writeInt32(writeIccID1(buff, len));
        }
        XLOGI("writeIccID1 done\n");
        return NO_ERROR;
    }
        break;
    case TRANSACTION_writeIccID2: {
        ...
        data.enforceInterface(descriptor);
        int len = data.readInt32();
        XLOGD("writeIccID2 len  = %d\n", len);
        if (len == -1) { // array is null
            reply->writeInt32(0);
        } else {
            char buff[len];
            data.read(buff, len);
            XLOGD("writeIccID2 buff  = %s\n", buff);
            reply->writeInt32(writeIccID2(buff, len));
        }
        XLOGI("writeIccID2 done\n");
        return NO_ERROR;
    }
        break;
    ...
    case TRANSACTION_getMacAddr: {
        ...
        XLOGI("getMacAddr done\n");
        return NO_ERROR;
    }
        break;

    case TRANSACTION_readSelfRegisterFlag: {
        XLOGI("readSelfRegisterFlag\n");
        data.enforceInterface(descriptor);
        int size = 0;
        char * ret = readSelfRegisterFlag(size);
        XLOGD("readSelfRegisterFlag = %s\n", ret);
        if (ret == NULL) {
            reply->writeInt32(-1);
        } else {
            reply->writeInt32(size);
        ...
        XLOGI("readSelfRegisterFlag done\n");
        return NO_ERROR;
    }
        break;
    case TRANSACTION_setSelfRegisterFlag: {
        XLOGI("setSelfRegisterFlag\n");
        data.enforceInterface(descriptor);
        int len = data.readInt32();
        XLOGD("setSelfRegisterFlag len  = %d\n", len);
        if (len == -1) { // array is null
            reply->writeInt32(0);
        } else {
            char buff[len];
            data.read(buff, len);
            XLOGD("setSelfRegisterFlag buff  = %s\n", buff);
            reply->writeInt32(setSelfRegisterFlag(buff, len));
        }
        XLOGI("setSelfRegisterFlag done\n");
        return NO_ERROR;
    }
        break;
	default:
...
char * DmAgent::readIccID1(int & size) {
    ...
    if (fd == -1) {
        ...
        read(fd, buff, size);
        close(fd);
        XLOGD("the readIccID1 buffer = %s\n", buff);
        return buff;
    }
}

bool DmAgent::writeIccID1(char * iccID, int size) {
    XLOGD("the ICCID1 want to save is %s\n", iccID);
...
bool DmAgent::writeIccID2(char * iccID, int size) {
    XLOGD("the ICCID2 want to save is %s\n", iccID);
    if (iccID == NULL || size == 0 || size > MAX_FILE_SIZE) {
        return 0;
    }
...
char * DmAgent::readSelfRegisterFlag(int & size) {
    ...
    if (dm_fd == -1) {
        ...
        read(dm_fd, buff, size);
        close(dm_fd);
        XLOGD("the readSelfRegisterFlag buffer = %s\n", buff);
        return buff;
    }
}

bool DmAgent::setSelfRegisterFlag(char * flag, int size) {
    XLOGD("the SelfRegister flag want to save is %s\n", flag);
    if (flag == NULL || size == 0 || size > MAX_FILE_SIZE) {
        return 0;
    }
...
int main(int argc, char *argv[]) {
    ...
    if (ret < 0) {
        XLOGE("Create RECOVERY_FOLDER error: [%d]\n", errno);
    } else {
        XLOGD("RECOVERY_FOLDER created end.");
    }
#endif
