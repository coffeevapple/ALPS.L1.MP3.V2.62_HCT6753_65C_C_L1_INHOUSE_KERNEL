	...
	private_pubkey_authenticator_t *this, message_t *message)
{
	...
	if (status != SUCCESS) 
	{
		...
		property_get("net.wo.statuscode", propvalue, "0");		
		DBG1(DBG_IKE, "property_get with ('net.wo.statuscode', '%s')", propvalue);	
		if ((strcmp(propvalue, "0") == 0) && (property_set("net.wo.statuscode", "1101") != 0)) {
			DBG1(DBG_IKE, "property_set fail with ('net.wo.statuscode', '1101')");	
		}
	}
#endif
	return status;
