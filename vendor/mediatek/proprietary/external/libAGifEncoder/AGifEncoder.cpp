...
bool AGifEncoder::setEncodeFile(const char file[])
{
   ...
   IMG_DBG("AGIFE:: setEncodeFile stream %p, L:%d!!\n", fGifStream ,__LINE__);   
...
{
   IMG_DBG("AGIFE:: setEncodeStream stream %p, w h %d %d , L:%d!!\n", stream, fWidth, fHeight , __LINE__);   
...
#if 1  
  if(fGifStream == NULL){    
    IMG_DBG("AGIFE::skip!!null stream, data %p, buf %p, size %x, total %x, L:%d!!\n", data,buffer,size, fWriteSize,__LINE__);
  }else if(size>0){
...
bool AGifEncoder::encodeBuffer(unsigned char *src_addr)
{
   ...
   IMG_DBG("AGIFE:: appendImage addr %p, time %d L:%d!!\n",src_addr, fPostTime, __LINE__); 
