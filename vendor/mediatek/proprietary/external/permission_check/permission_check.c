...
static int check_decrypt_ready(void)
{
    ...
    if(strcmp(property_val, "") == 0){
   		...
   		return 0;
			...
			property_get("vold.encryption.type", property_val, NULL);
			while(strcmp(property_val, "") == 0){
				property_get("vold.encryption.type", property_val, NULL);
			}
			LOGD("vold.encryption.type=%s\n",property_val);		
			if(strcmp(property_val, "default") == 0){
				LOGD("auto decrypt\n");
				wait_decrypt_done();
				wait_nvram_ready();
   			return 0;		
			}else if(strcmp(property_val, "pin") == 0){
				LOGD("pin encrypted!!\n");
				return 1;
			}else if(strcmp(property_val, "password") == 0){
				LOGD("password encrypted!!\n");
				return 1;
			}else if(strcmp(property_val, "pattern") == 0){
				LOGD("pattern encrypted!!\n");
				return 1;
			}
  	}
