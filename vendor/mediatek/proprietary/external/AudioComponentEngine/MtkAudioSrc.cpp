...
ACE_ERRID MtkAudioSrc::SetParameter(uint32_t paramID, void *param)
{
    ...
    if(mState == ACE_STATE_OPEN && Curparam != SRC_PAR_SET_INPUT_SAMPLE_RATE) //Only input sampling rate could be update during process.
    ...
    switch(paramID)
    {
        case SRC_PAR_SET_PCM_FORMAT:
        {
            ...
            mBlisrcParam.PCM_Format = Curparam;
            break;
        }
        case SRC_PAR_SET_INPUT_SAMPLE_RATE:
        {
            mBlisrcParam.in_sampling_rate = Curparam;
            break;
        }
        case SRC_PAR_SET_OUTPUT_SAMPLE_RATE:
        {
            mBlisrcParam.ou_sampling_rate = Curparam;
        ...
        {
            mBlisrcParam.in_channel = Curparam;
            break;
        }
        case SRC_PAR_SET_OUTPUT_CHANNEL_NUMBER:
        {
            mBlisrcParam.ou_channel = Curparam;
            break;
        }
        default:
            return ACE_INVALIDE_PARAMETER;
...
ACE_ERRID MtkAudioSrc::GetParameter(uint32_t paramID, void *param)
{
    ...
    switch (paramID)
    {
        ...
        {
            Curparam = (long)mBlisrcParam.PCM_Format;
            break;
        }
        case SRC_PAR_GET_INPUT_SAMPLE_RATE:
        {
            Curparam = (long)mBlisrcParam.in_sampling_rate;
            break;
        }
        case SRC_PAR_GET_OUTPUT_SAMPLE_RATE:
        {
            Curparam = (long)mBlisrcParam.ou_sampling_rate;
            break;
        }
        case SRC_PAR_GET_INPUT_CHANNEL_NUMBER:
        {
            Curparam = (long)mBlisrcParam.in_channel;
            break;
        }
        case SRC_PAR_GET_OUTPUT_CHANNEL_NUMBER:
        {
            Curparam = (long)mBlisrcParam.ou_channel;
            break;
        }
        default:
            return ACE_INVALIDE_PARAMETER;
...
ACE_ERRID MtkAudioSrc::MultiChannel_Open(void)
{
    ALOGD("+%s()\n",__FUNCTION__);
    Mutex::Autolock _l(mLock);
    if(mState != ACE_STATE_INIT)
    ...
    int ret;
#if defined(HAVE_SWIP)
    ret = Blisrc_MultiChannel_GetBufferSize( &mInternalBufSize,
                            &mTempBufSize,
                            &mBlisrcParam);
...
ACE_ERRID MtkAudioSrc::MultiChannel_ResetBuffer(void)
{
    ALOGD("+%s()\n",__FUNCTION__);
    Mutex::Autolock _l(mLock);
    if(mState != ACE_STATE_OPEN)
...
ACE_ERRID MtkAudioSrc::MultiChannel_Process(void *pInputBuffer,      /* Input, pointer to input buffer */
                     uint32_t *InputSampleCount,        /* Input, length(byte) of input buffer */ 
                                                        /* Output, length(byte) left in the input buffer after conversion */ 
                     void *pOutputBuffer,               /* Input, pointer to output buffer */
                     uint32_t *OutputSampleCount)       /* Input, length(byte) of output buffer */ 
