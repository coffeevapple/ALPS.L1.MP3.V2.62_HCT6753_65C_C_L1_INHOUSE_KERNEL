...
#define I2C_ADDRESS   0x68  //cruson modify TFA9890 SLAVE ADDRESS  0x6c->0x68
...
int tfa9887_init(int sRate)
{
	...
	/* use the generic slave address for optimizations */
      ...
      /* create handle */
      err = Tfa98xx_Open(I2C_ADDRESS + 2, &handle);
	    assert(err == Tfa98xx_Error_Ok);
        g_handle = handle;
      coldStartup(handle);
  ...
  calibrateDone = 0;
	...
	waitCalibration(handle, &calibrateDone);
      if (calibrateDone)
      {
         ...
         ALOGI("Calibration value is %2.2f ohm\n", re25);
...
void tfa9887_SpeakerOn(void)
{
  ...
  Tfa98xx_Error_t err = Tfa98xx_Error_Ok;
	Tfa98xx_handle_t handle = g_handle;
		...
		printf("%s (RIGHT) return value is %d.\n", __func__, err);
		assert(err == Tfa98xx_Error_Ok);
...
//set samplerate at tfa9887 powerdown
...
EXPORT_SYMBOL(tfa9887_deinit);
EXPORT_SYMBOL(tfa9887_eqset);
EXPORT_SYMBOL(tfa9887_SpeakerOff);
EXPORT_SYMBOL(tfa9887_SpeakerOn);
EXPORT_SYMBOL(tfa9887_setSamplerate);
