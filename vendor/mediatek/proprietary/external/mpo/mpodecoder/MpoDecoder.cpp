...
SkBitmap* MpoDecoder::getImageBitmap(JNIEnv* env, 
                                     int imageIndex,jobject options,SkBitmap::Allocator* javaalloctor)
{
    ...
    //decode buffer
    SkBitmap* bitmap = decodeBuffer(env, options, &memStream,javaalloctor);
...
SkBitmap* MpoDecoder::decodeBuffer(JNIEnv* env, jobject options, 
                                   SkStreamRewindable* stream,SkBitmap::Allocator* javaalloctor) {
    int sampleSize = 1;
    int preferSize = 0;
    int postproc = 0;
    int postprocflag = 0;
	...
	decoder->setSkipWritingZeroes(1);
	decoder->setAllocator(javaalloctor);
    }

    SkBitmap* bitmap = new SkBitmap;

	SkImageDecoder::Result result = decoder->decode(stream, bitmap, colortype, decodeMode);
    if (result == SkImageDecoder::kFailure) {
        XLOGE("SkImageDecoder-decode() returned kFailure");
        return NULL;
    }
...
bool MpoDecoder::decodeBounds()
{
    ...
    SkImageDecoder::Result result = decoder->decode(&memStream, bitmap, colortype, decodeMode);
    if (result == SkImageDecoder::kFailure) {
        XLOGE("SkImageDecoder-decode() returned kFailure");
        free(jpegBuffer);
        return false;
