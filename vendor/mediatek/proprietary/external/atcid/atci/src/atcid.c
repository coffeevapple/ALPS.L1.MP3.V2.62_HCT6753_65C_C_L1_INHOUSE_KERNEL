...
static int getBootMode(void) {
    int mode = -1;
    char buf[BOOT_MODE_STR_LEN + 1] = {0};
    int bytes_read = 0;
    int res = 0;
    ...
    if (fd != -1) {
        ...
        LOGATCI(LOG_ERR, "Failed to open boot mode file %s", BOOT_MODE_INFO_FILE);
    }
    return mode;
}
...
int main(int argc,char *argv[]) {
    ...
    LOGATCI(LOG_INFO, "Boot mode = %d", bootMode);
    if(bootMode == BOOT_MODE_ADV_META) {
        LOGATCI(LOG_INFO, "advanced meta mode, stop atcid-daemon");
        exit(0);
    }
    ...
    strcpy(serial.devicename[1],TTY_GS4);
#else
