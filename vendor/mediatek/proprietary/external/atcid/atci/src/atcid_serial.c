    ...
    if (RIL_TYPE == dataType) {
        if (connc_socket == SOCKET_MD2) {
            ...
            s_fdRild_command = socket_local_client(SOCKET_NAME_RILD2,
                                                   ...
                                                   SOCK_STREAM);
        } else if (connc_socket == SOCKET_MD3) {
            LOGATCI(LOG_DEBUG, "single modem mode and try to connect socket 3.");
            s_fdRild_command = socket_local_client(SOCKET_NAME_RILD3,
                                                   ANDROID_SOCKET_NAMESPACE_RESERVED,
                                                   SOCK_STREAM);
