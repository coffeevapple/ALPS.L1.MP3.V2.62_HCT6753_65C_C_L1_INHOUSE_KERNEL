...
int nfc_cmd_handler(char* cmdline, ATOP_t at_op, char* response)
{
    ...
    int  ret = 0;
#ifdef MTK_NFC_QUICK_SUPPORT
    ret = quickEnable_cfg_check();
    ALOGD("[QE]NFC Quick Enable:%d", ret);
    if(ret == 1) // At Command not support NFC Quick Enable
    {

    	sprintf(response,"\r\n\nATCommand not Support NFC Quikc Enable!!\r\n"
    			"Please turn off NFC Quick Enable!!\r\n"
    			"1.set [/system/etc/nfc.cfg] QUICK_ENABLE:0 & reboot !\n");
