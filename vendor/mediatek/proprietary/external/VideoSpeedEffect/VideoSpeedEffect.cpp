...
#define VIDEOSPEEDEFFECT_USE_XLOG
#ifdef VIDEOSPEEDEFFECT_USE_XLOG
...
public:
	...
	{
		ALOGD("receive MPEG4Writer notify:msg(%d),ext1(%d),ext2(%d)");
...
private:
	...
	public:	
		...
		VideoFpsControlTable(Type tableStartPoint,int32_t entryNum,Type stepInterval){
			...
			ALOGI("createFpsControlTable,table [%lld-%lld],entry num [%d],",mTableStartPoint,mTableMaxPoint,mTableNumEntry);
		...
		FpsControlEntry* findEntry(Type accessPoint){
			...
			for(i = 0; i< mTableNumEntry; i++){
				...
				if(accessPoint > item.mEntryStartPoint && accessPoint <= item.mEntryStopPoint){
					...
					item.mEntryStartPoint,item.mEntryStopPoint,item.mHasFrame);
			...
			if(i == mTableNumEntry){
				ALOGW("can not find entry contain [%lld]",accessPoint);
			}
			return entry;
		...
		status_t update(Type tableStartPoint,int32_t entryNum,Type stepInterval){
			...
			mTableMaxPoint = lastPoint;
			mTableNumEntry = entryNum;
			ALOGI("update control table,table [%lld-%lld],entry num [%d],",mTableStartPoint,mTableMaxPoint,mTableNumEntry);
			return OK;
		}
...
status_t VideoSpeedEffect::startSaveSpeedEffect(int srcFd,int64_t srcOffset, int64_t srcLength, int dstFd) {
	...
	//get src file format
	sp<DataSource> dataSource = new FileSource(srcFd, srcOffset, srcLength);
	...
	mLooper->start();
	sp<AMessage> progressNotify = new AMessage('prog', id());
	progressNotify->post();
...
status_t VideoSpeedEffect::notify(int msg, int ext1, int ext2) {
	sp<AMessage> notify = new AMessage('notf',id());
	...
	for(List<SpeedEffectParam>::iterator i = mpSpeedEffectParamList->begin(); i != mpSpeedEffectParamList->end(); i++){
		if(timeUs >= (*i).mStartPos && timeUs <= (*i).mEndPos){
			...
			if(!((*i).mMeta->findInt32(kKeyFirstVideoSectBuf,&isFirstSpeedEffectBuffer)))
			{
				...
				ALOGI("video,SpeedEffectIfNeed,first orig video timestamp= (%d) of this section,mLastOrigTimestamp=%lld ",timeUs,mLastOrigTimestamp);
				mSectionFirstOrigTimestamp = timeUs;
				mLastDelta += timeUs - mLastOrigTimestamp;
				mSectinoFirstNewTimestamp = mLastMaintainTimestamp +  mLastDelta;
				buffer->meta_data()->setInt64(kKeyTime, mSectinoFirstNewTimestamp);
				ALOGI("video,SpeedEffectIfNeed,first new video timestamp= (%d) of this section ",mSectinoFirstNewTimestamp);
				//for A/V sync
