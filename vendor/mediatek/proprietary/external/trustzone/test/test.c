...
#define remove_releasecm_test
#define remove_chunkmem_test
...
int do_do_C_D_test (pid_t pid, uint32_t id, uint32_t test_size)
{
    ...
    if (ret != TZ_RESULT_SUCCESS)
    {
        printf("    [%d] do D Error: 0x%x, %d\n", id, (uint32_t) param[0].value.a, ret);
        error_exit_1 ("do_do_C_D_test");
    }  
...
int do_ion_test (pid_t pid, uint32_t id, UREE_SESSION_HANDLE mem_session, int check_add_mem)
{
	...
	result = 0;
        ...
        ret = check_add_mem_test ((int *) ion_buf, (int) param[2].value.a, 0, ION_BUF_LEN, (int) ION_BUF_LEN / sizeof(unsigned int));
