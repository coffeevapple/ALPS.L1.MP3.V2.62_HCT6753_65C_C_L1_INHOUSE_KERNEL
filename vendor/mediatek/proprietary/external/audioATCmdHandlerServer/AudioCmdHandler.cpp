...
ACHStatus AudioCmdHandler::DLCustSPHLPBKParamToNV(void *pParam)
{
    ...
    SXLOGD("AudioCmdHandler::DLCustSPHLPBKParamToNV() in");
    ...
    SXLOGD("DLCustSPHLPBKParamToNV- GetNBSpeechLpbkParamFromNVRam read size=%d", size);
    if (size != sizeof(AUDIO_CUSTOM_SPEECH_LPBK_PARAM_STRUCT))
    {
        SXLOGD("DLCustSPHLPBKParamToNV Up load from NVRAM fail, structure size=%d,read_size=%d", sizeof(AUDIO_CUSTOM_SPEECH_LPBK_PARAM_STRUCT), size);
        free(pSPHLPBKPara);
        pSPHLPBKPara = NULL;
        return ACHFailed;
    }
    ...
    for (int i = 0; i < 48; i++)
    {
        SXLOGD("LPBK_STRUCT[%d] =%d", i, ((unsigned short *)pSPHLPBKPara) + i);
    }

    write_size = SetNBSpeechLpbkParamToNVRam(pSPHLPBKPara);
    if (write_size != sizeof(AUDIO_CUSTOM_SPEECH_LPBK_PARAM_STRUCT))
    {
        SXLOGD("DLCustSPHHACParamToNV Down load to NVRAM fail, structure size=%d,writed_size=%d", sizeof(AUDIO_CUSTOM_SPEECH_LPBK_PARAM_STRUCT), write_size);
        free(pSPHLPBKPara);
        pSPHLPBKPara = NULL;
        return ACHFailed;
    }
...
ACHStatus AudioCmdHandler::ULCustSPHLPBKParamFromNV(void *pParam, int *len)
{
    ...
    SXLOGD("AudioCmdHandler::ULCustSPHLPBKParamFromNV() in");
    AUDIO_CUSTOM_SPEECH_LPBK_PARAM_STRUCT *pSPHLPBKPara = (AUDIO_CUSTOM_SPEECH_LPBK_PARAM_STRUCT *)malloc(sizeof(AUDIO_CUSTOM_SPEECH_LPBK_PARAM_STRUCT));
    ...
    if (size != sizeof(AUDIO_CUSTOM_SPEECH_LPBK_PARAM_STRUCT))
    {
        SXLOGD("ULCustSPHLPBKParamFromNV Up load from NVRAM fail, structure size=%d,read_size=%d", sizeof(AUDIO_CUSTOM_SPEECH_LPBK_PARAM_STRUCT), size);
        free(pSPHLPBKPara);
        pSPHLPBKPara = NULL;
        return ACHFailed;
    }
    ...
    SXLOGD("ULCustSPHLPBKParamFromNV the data sent to PC is %d", dataLen);
    *((int *)pParam) = dataLen;
    *len = 2 * (dataLen + sizeof(int));
...
ACHStatus AudioCmdHandler::DLCustSPHParamToNV(void *pParam, int block)
{
	...
	else if(block == 3) 
	{
		 ...
		 {
			 SXLOGD("AUD_SPH_AFTER_FOUR_MODE_PARAM_STRUCT[%d]=%d", i, *((unsigned short *)pFourModeCustomPara + i));
		 }
...
ACHStatus AudioCmdHandler::DLCustSPHWBParamToNV(void *pParam, int block)
{
	...
	else if(block == 2) 
	{
		...
		for (int i = 0; i < SPH_MODE_NUM*SPH_ENHANCE_PARAM_NUM ;  i++)
		{
			SXLOGD("AUD_SPH_AFTER_FOUR_MODE_PARAM_STRUCT[%d]=%d", i, *((unsigned short *)pFourCustomPara + i));
		}
...
void AudioCmdHandler::printGainInfo(PCDispTotolStru *dispStru, int block)
{
	if(block == 1)
	{
		SXLOGD("PCDispTotolStru->itemNum: 0x%x", dispStru->itemNum);
		for(int i=0; i<16; i++)
		{
			SXLOGD("PCDispTotolStru->DispItem[%d].strType is %s", i, dispStru->DispItem[i].strType);
			SXLOGD("PCDispTotolStru->DispItem[%d].level is 0x%x", i, dispStru->DispItem[i].level);
			SXLOGD("PCDispTotolStru->DispItem[%d].subItemNum is 0x%x", i, dispStru->DispItem[i].subItemNum);
			for(int j=0; j<MAX_SUB_ITEM_NUM; j++)
			{
				SXLOGD("PCDispTotolStru->DispItem[%d].subItem[%d].outputDevice is %s", i, j, dispStru->DispItem[i].subItem[j].outputDevice);
				SXLOGD("PCDispTotolStru->DispItem[%d].subItem[%d].AnalogPoint is 0x%x", i, j, dispStru->DispItem[i].subItem[j].AnalogPoint);
			}
		}
	}
	else if(block == 2)
	{
		SXLOGD("Disp Info For Mic");
		SXLOGD("dispStru->DispMic.strType: %s", dispStru->DispMic.strType);
		SXLOGD("dispStru->DispMic.subItem: 0x%x", dispStru->DispMic.subItemNum);
		for(int j=0; j<24; j++)
		{
			SXLOGD("dispStru->DispMic.subItem[j]:%s", dispStru->DispMic.subItem[j]);
		}
		
		SXLOGD("dispStru->gainRangeNum: 0x%x", dispStru->gainRangeNum);
		for(int k=0; k<MAX_GAIN_POINT_NUM; k++)
		{
			SXLOGD("Audio Buffer Gain/Voice Buffer Gain/Mic Gain/Ext AMP1 Gain/Ext AMP2 Gain/Ext AMP3 Gain/Ext AMP4 Gain/Side Tone Gain");
			SXLOGD("%d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d", dispStru->gainRange[k][0], dispStru->gainRange[k][1], dispStru->gainRange[k][2], dispStru->gainRange[k][3],
...
{
	int i=0, j=0;
	SXLOGD("OutputDevice: GAIN_OUTPUT_EARPIECE,GAIN_OUTPUT_HEADSET,GAIN_OUTPUT_SPEAKER,GAIN_OUTPUT_DOCK");
	...
	if(block == 1)
	{
		for(i=0; i<12; i++)
		{
			...
			for(j=0; j<3; j++)
			{
				SXLOGD("Digtal Gain: 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x", 
				...
				SXLOGD("Analog Gain: 0x%x", ptr->stream[j].analog);
				SXLOGD("Amp Gain: 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x", 
	...
	{
		SXLOGD("OutputDevice: GAIN_OUTPUT_EARPIECE,GAIN_OUTPUT_HEADSET,GAIN_OUTPUT_SPEAKER,GAIN_OUTPUT_DOCK, and Every Gain Step");
		for(i=0; i<4; i++)
		{
			for(j=0; j<18; j++)
			{
				SXLOGD("SpeechWB Digtal Gain: 0x%x", gainTable->speechWB.speech[i][j].digital);
				SXLOGD("SpeechWB Analog Gain: 0x%x", gainTable->speechWB.speech[i][j].analog);
				SXLOGD("Amp Gain: 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x", 
		...
		SXLOGD("stream Type is sideToneWB");
		SXLOGD("sideToneWB Gain: 0x%x, 0x%x, 0x%x, 0x%x", gainTable->sideToneWB.sidetone[0], gainTable->sideToneWB.sidetone[1], gainTable->sideToneWB.sidetone[2], gainTable->sideToneWB.sidetone[3]);

		SXLOGD("stream Type is mic");
		SXLOGD("mic Gain: 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x", 
...
{
    SXLOGD("getGainInfoForDisp() in, pParam is 0x%x", (int)pParam);
	...
	if(block == 0)
	{
		SXLOGD("Get all PCDispTotolStru Structure");
		//TBD
	}
	else if(block == 1)  
	{
		SXLOGD("First get GainInfo for PC UI display");
		GainInfoFlag = true;
	
		SXLOGD("Get itemNum and 20 DispItems");
	...
	{
		SXLOGD("Get Mic Structure and gainRange");
		...
		SXLOGD("get GainInfo for PC UI display Done");
		GainInfoFlag = false;
	}
	else if(block == 3)
	{
/*
		SXLOGD("Pass next 4 DispItems and Analog Gain Range");
		for(int i=0; i<4; i++)
		{
		...
		SXLOGD("sizeof(pGainInfoParam->gainRange) is %d", sizeof(pGainInfoParam->gainRange));
...
*/
	...
	{
		SXLOGE("The value of block: %d is invailed", block);
		return ACHFailed;
	}
    ...
    *len = 2 * (size + sizeof(int));
	...
	SXLOGD("getGainInfoForDisp() out, size is %d, len is %d", size, *len);
    return ACHSucceeded;
}
...
ACHStatus AudioCmdHandler::ULCustGainParamFromNV(void *pParam, int *len, int block)
{
	...
	SXLOGD("ULCustGainParamFromNV() in");
    if (pParam == NULL)
    {
        return ACHFailed;
    }
    ...
    SXLOGD("ULCustGainParamFromNV ,LGE Gain Table size=%d", sizeof(AUDIO_GAIN_TABLE_STRUCT));
    size = GetAudioGainTableParamFromNV(pGainTableParam);  //TBD
    if (size != sizeof(AUDIO_GAIN_TABLE_STRUCT))
    {
        SXLOGD("ULCustGainParamFromNV Up load from NVRAM fail, structure size=%d,read_size=%d", sizeof(AUDIO_GAIN_TABLE_STRUCT), size);
	...
	if(block ==0)
	{
		SXLOGD("Get all AUDIO_GAIN_TABLE_STRUCT Structure");
		//TBD
	}
	else if(block == 1)
	{
		SXLOGD("Get 12 streamType items");
	...
	{
		SXLOGD("Get Speech/SideTone/Mic items");
	...
	*len = 2 * (size + sizeof(int));
	...
	SXLOGD("ULCustGainParamFromNV() out, size is %d, len is %d", size, *len);
	return ACHSucceeded;
}

ACHStatus AudioCmdHandler::DLCustGainParamToNV(void *pParam, int block)
{
	SXLOGD("AudioCmdHandler::DLCustGainParamToNV() in");
	int size = 0;
	int dataLen = 0;
	...
	SXLOGD("DLCustGainParamToNV ,LGE Gain Table size=%d", sizeof(AUDIO_GAIN_TABLE_STRUCT));
	size = GetAudioGainTableParamFromNV(pGainTableParam);  //TBD
	if (size != sizeof(AUDIO_GAIN_TABLE_STRUCT))
	{
		SXLOGD("DLCustGainParamToNV Upload from NVRAM fail, structure size=%d,read_size=%d", sizeof(AUDIO_GAIN_TABLE_STRUCT), size);
		if (pGainTableParam) { free(pGainTableParam); }
		pGainTableParam = NULL;
		return ACHFailed;
	}
//	printCustGainTable(pGainTableParam, block);
	...
	else if(block == 1)
	{
        ...
        if (dataLen != size)
        {
            SXLOGE("DLCustGainParamToNV 12 streamType data miss !! dataLen=%d, size=%d", dataLen, size);
            if (pGainTableParam) { free(pGainTableParam); }
            return ACHFailed;
        }
	...
	{
		SXLOGD("Set Speech/SideTone/Mic items");
        ...
        if (dataLen != size)
        {
            SXLOGE("DLCustGainParamToNV  Speech/SideTone/Mic items data miss !! dataLen=%d, size=%d", dataLen, size);
            if (pGainTableParam) { free(pGainTableParam); }
            return ACHFailed;
        }
    ...
    if (size != sizeof(AUDIO_GAIN_TABLE_STRUCT))
    {
        SXLOGD("DLCustDualMicParamToNV down load to NVRAM fail, structure size=%d,write_size=%d", sizeof(AUDIO_GAIN_TABLE_STRUCT), size);
        if (pGainTableParam) { free(pGainTableParam); }
        pGainTableParam = NULL;
        return ACHFailed;
    }
	...
	SXLOGD("DLCustGainParamToNV() out");
	return ACHSucceeded;
}

ACHStatus AudioCmdHandler::getBtNrecInfoForDisp(void *pParam, int *len)
{
    SXLOGD("getBtNrecInfoForDisp() in, pParam is 0x%x", (int)pParam);
    if (pParam == NULL)
    {
        return ACHFailed;
    }
    ...
    *len = 2 * (size + sizeof(int));
	...
	SXLOGD("getBtNrecInfoForDisp() out, size is %d, len is %d", size, *len);
    return ACHSucceeded;
}
...
ACHStatus AudioCmdHandler::DLCustBtNrecParamToNV(void * pParam)
{
    ...
    SXLOGD("AudioCmdHandler::DLCustBtNrecParamToNV() in");
    ...
    SXLOGD("DLCustBtNrecParamToNV- GetAudioBTGainParamFromNV read size=%d", size);
    if (size != sizeof(AUDIO_BT_GAIN_STRUCT))
    {
        SXLOGD("DLCustBtNrecParamToNV Up load from NVRAM fail, structure size=%d,read_size=%d", sizeof(AUDIO_BT_GAIN_STRUCT), size);
        free(pBTGainPara);
        pBTGainPara = NULL;
        return ACHFailed;
    }
...
//Print
	 ...
	 if (*((int *)pParam) != size)
	 {
		 SXLOGE("DLCustBtNrecParamToNV miss data !!, received size = %d", *((int *)pParam));
		 free(pBTGainPara);
		 pBTGainPara = NULL;
		 return ACHFailed;
	 }
    ...
    {
        SXLOGD("DLCustSPHHACParamToNV Down load to NVRAM fail, structure size=%d,writed_size=%d", sizeof(AUDIO_BT_GAIN_STRUCT), write_size);
        free(pBTGainPara);
        pBTGainPara = NULL;
        return ACHFailed;
    }
...
ACHStatus AudioCmdHandler::ULCustBtNrecParamFromNV(void * pParam,int * len)
{
    ...
    SXLOGD("AudioCmdHandler::ULCustBTNRECParamFromNV() in");
    AUDIO_BT_GAIN_STRUCT *pBTGainPara = (AUDIO_BT_GAIN_STRUCT *)malloc(sizeof(AUDIO_BT_GAIN_STRUCT));
    ...
    if (size != sizeof(AUDIO_BT_GAIN_STRUCT))
    {
        SXLOGD("ULCustBTNRECParamFromNV Up load from NVRAM fail, structure size=%d,read_size=%d", sizeof(AUDIO_BT_GAIN_STRUCT), size);
        free(pBTGainPara);
        pBTGainPara = NULL;
        return ACHFailed;
    }
//Print
    ...
    SXLOGD("ULCustBTNRECParamFromNV the data sent to PC is %d", dataLen);
    *((int *)pParam) = dataLen;
    *len = 2 * (dataLen + sizeof(int));
...
ACHStatus AudioCmdHandler::startRecorder(const char *filePath, int recordAudioSource)
{
    ...
    if (m_MediaRecorderClient.get() == NULL)
    {
    	SXLOGD("new MediaRecorder()");
        m_MediaRecorderClient = new MediaRecorder();
		SXLOGD("m_MediaRecorderClient.get() is: 0x%x", m_MediaRecorderClient.get());
