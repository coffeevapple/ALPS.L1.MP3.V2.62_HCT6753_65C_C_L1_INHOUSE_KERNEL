...
class AudioCmdHandler : public MediaPlayerListener
{
    public:
		...
		ACHStatus DLCustSPHLPBKParamToNV(void *pParam);
		ACHStatus ULCustSPHLPBKParamFromNV(void *pParam, int *len);
	...
	ACHStatus findACFFO(unsigned short cmdType, void *pParam, int *len);
		...
		ACHStatus getGainInfoForDisp(void *pParam, int *len, int block);
		void printGainInfo(PCDispTotolStru *dispStru, int block);
		void printCustGainTable(AUDIO_GAIN_TABLE_STRUCT *gainTable, int block);
		ACHStatus ULCustGainParamFromNV(void *pParam, int *len, int block);
		ACHStatus DLCustGainParamToNV(void *pParam, int block);
		ACHStatus getBtNrecInfoForDisp(void *pParam, int *len);
		ACHStatus DLCustBtNrecParamToNV(void *pParam);
		ACHStatus ULCustBtNrecParamFromNV(void *pParam, int *len);
#endif
