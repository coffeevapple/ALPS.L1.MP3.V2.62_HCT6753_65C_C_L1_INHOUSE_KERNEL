...
static void DoGetGainInfoForDisp(AudioCmdParam &audioCmdParams, void *pParam, AudioCmdHandler *pAudioHandle)
{
    ...
    if (audioCmdParams.param1 == 0)  //get data
    {
		...
		{
			SXLOGD("block number:%d is invailed", audioCmdParams.param2);
			ret = ACHFailed;
		}
		else
		{
			SXLOGD("pGainForDispInfo is 0x%x", (int)pGainForDispInfo);
        ...
        if (ret == ACHSucceeded)
        {
			for(int i=0; i< 96; )
			{
				SXLOGD("0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x",(char *)*((char *)pGainForDispInfo+i), (char *)*((char *)pGainForDispInfo+i+1), (char *)*((char *)pGainForDispInfo+i+2), (char *)*((char *)pGainForDispInfo+i+3), (char *)*((char *)pGainForDispInfo+i+4), (char *)*((char *)pGainForDispInfo+i+5), (char *)*((char *)pGainForDispInfo+i+6), (char *)*((char *)pGainForDispInfo+i+7));
				i += 8;
			}
    ...
    {
		SXLOGE("audioCmdParams.param1:%d is invailed", audioCmdParams.param1);
