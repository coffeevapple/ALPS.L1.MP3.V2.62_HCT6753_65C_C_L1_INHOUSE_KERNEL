    ...
    while (true)
    {
        ...
        switch ((Audio_Dump_type_t)SecondCommand)
        {
            case AUDIO_DUMP_OUT_STREAMOUT:
                sprintf(keyInputFileName, "AUDIO_DUMP_OUT_STREAMOUT=%d", bDumpFlg);
                break;
            case AUDIO_DUMP_OUT_FLINGER:
                sprintf(keyInputFileName, "AUDIO_DUMP_OUT_FLINGER=%d", bDumpFlg);
                break;
            case AUDIO_DUMP_IN_STREAMIN:
                sprintf(keyInputFileName, "AUDIO_DUMP_IN_STREAMIN=%d", bDumpFlg);
                break;
            case AUDIO_DUMP_OUT_TRACK:
                sprintf(keyInputFileName, "AUDIO_DUMP_OUT_TRACK=%d", bDumpFlg);
                break;                
            default:
                bBreak = true;
                printf("Exit\n");
...
void ListDumpAudioDataState(void)
{
    ...
    mValue = AudioSystem::getParameters(0, String8("AUDIO_DUMP_OUT_STREAMOUT"));
    printf("%s\n", mValue.string());

    mValue = AudioSystem::getParameters(0, String8("AUDIO_DUMP_OUT_FLINGER"));
    printf("%s\n", mValue.string());

    mValue = AudioSystem::getParameters(0, String8("AUDIO_DUMP_IN_STREAMIN"));
    printf("%s\n", mValue.string());

    mValue = AudioSystem::getParameters(0, String8("AUDIO_DUMP_OUT_TRACK"));
    printf("%s\n\n\n", mValue.string());     
}
...
int main()
{
    ...
    while (true)
    {
        printf("please enter audio command\n"
               ...
               "  15: set speaker monitor upper bound\n"
               "  16: set speaker monitor lower bound\n"
#if 0
               "  5: set afe register\n"
               "  6: set pmic register\n"
#endif
               "  other digital num: exit\n");
/*
