...
public class CustomAccountRemoteViews {
    ...
    public void configureView() {
        if (mData != null) {
            ...
            if (mData.size() > MOSTACCOUNTNUMBER) {
                for (int i = 0; i < mData.size() && i < MOSTACCOUNTNUMBER - 1; i++) {
                ...
                mOtherAccounts = mData.subList(MOSTACCOUNTNUMBER - 1, mData.size());
                AccountInfo otherAccount = generateOtherAccount();
                if (hasActiveAccount(mOtherAccounts)) {
                    otherAccount.setActiveStatus(true);
                } else {
                    otherAccount.setActiveStatus(false);
                }

                configureAccount(RESOURCE_ID[MOSTACCOUNTNUMBER - 1], otherAccount);
    ...
    private AccountInfo generateOtherAccount() {
        ...
        AccountInfo otherAccount = new AccountInfo(com.mediatek.R.drawable.other_accounts_icon, other_accounts, null,
                otherIntent);
        return otherAccount;
    }
