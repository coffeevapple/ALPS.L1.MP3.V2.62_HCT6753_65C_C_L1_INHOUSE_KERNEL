...
public class ImageViewEx extends ImageView {
    ...
    private Bitmap resizeAndRotateImage(Uri uri) {
        ...
        if (imageWidth > MAX_WIDTH || imageHeight > MAX_HEIGHT) {
            Log.e(TAG, "<resizeAndRotateImage> Image need resize: "
        ...
        try {
            ...
            if (input != null) {
                try {
                    finalImage = BitmapFactory.decodeStream(input, null,
                            options);
                } catch (OutOfMemoryError ex) {
                ...
                try {
                    finalImage = Bitmap.createScaledBitmap(
                                    finalImage,
                                    (bound.width / scaleFactor) > 0 ? (bound.width / scaleFactor) : 1,
                                    (bound.height / scaleFactor) > 0 ? (bound.height / scaleFactor) : 1,
                                    false);
                } catch (OutOfMemoryError ex) {
                }
        ...
        try {
            ...
            while (true) {
                ...
                Log.v(TAG, "<gifAnimation> sleep for " + frameDuration
                        + " ms for frame " + mCurrGifFrame + " //this=" + self);
                if (mAbort) {
