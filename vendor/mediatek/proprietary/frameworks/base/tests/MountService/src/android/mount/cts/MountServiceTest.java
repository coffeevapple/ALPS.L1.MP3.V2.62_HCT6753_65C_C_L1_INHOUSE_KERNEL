...
public class MountServiceTest extends InstrumentationTestCase {
    ...
    /*public void testIsSomeStorageEmulated() {
        // StorageManager.isSomeStorageEmulated()
        waitForAllStorageMounted();
        if (!SystemProperties.get(PROP_2SDCARD_SWAP).equals("1")  && !SystemProperties.get(PROP_SHARED_SDCARD).equals("1")) {
            assertTrue(!Environment.isSomeStorageEmulated());
        } else if (!SystemProperties.get(PROP_2SDCARD_SWAP).equals("1")  && SystemProperties.get(PROP_SHARED_SDCARD).equals("1")) {
            assertTrue(Environment.isSomeStorageEmulated());
        } else if (SystemProperties.get(PROP_2SDCARD_SWAP).equals("1")  && !SystemProperties.get(PROP_SHARED_SDCARD).equals("1")) {
            assertTrue(!Environment.isSomeStorageEmulated());
...
/*
    public void testFormatDefault() {
        ...
        try {
            context = mContext.createPackageContext("system", 
                Context.CONTEXT_INCLUDE_CODE | Context.CONTEXT_IGNORE_SECURITY);
        } catch (NameNotFoundException e) {
        }
        if(context == null) {
            fail("testFormatDefault context null!");
            return;
        }
        ...
        Button YesBtn = (Button)activity.findViewById(AlertDialog.BUTTON_POSITIVE);
