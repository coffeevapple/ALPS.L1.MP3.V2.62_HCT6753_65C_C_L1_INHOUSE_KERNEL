    ...
    **/
    ...
    private void prepareSystemServerSrvInitList() {
        ...
        if (mContext.getPackageManager().hasSystemFeature(PackageManager.FEATURE_BLUETOOTH) &&
            SystemProperties.getBoolean("config.disable_bluetooth", false) == false)
        {
        ...
        if (!SystemProperties.get("system_init.startmountservice").equals("0")) {
        ...
        Slog.v(TAG, "ro.mtk_bsp_package:" + SystemProperties.get("ro.mtk_bsp_package").equals("1"));
        ...
        if (false == SystemProperties.get("ro.mtk_bsp_package").equals("1")
            ...
            mSrvInitList.add(Context.AUDIO_PROFILE_SERVICE);
        }
        if (mContext.getPackageManager().hasSystemFeature(PackageManager.FEATURE_USB_HOST) ||
            mContext.getPackageManager().hasSystemFeature(PackageManager.FEATURE_USB_ACCESSORY))
        {
            mSrvInitList.add(Context.USB_SERVICE);
        }
        mSrvInitList.add(Context.SERIAL_SERVICE);
        if (mContext.getPackageManager().hasSystemFeature(PackageManager.FEATURE_BACKUP)) {
        ...
        /*if(SystemProperties.get("ro.mtk_agps_app").equals("1") && SystemProperties.get("ro.mtk_gps_support").equals("1")) {
