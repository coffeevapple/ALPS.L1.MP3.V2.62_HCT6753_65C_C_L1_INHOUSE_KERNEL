...
/**
 * Default Implementation for IOmaSettingHelper.
 */
...
public class DefaultOmaSettingHelper implements IOmaSettingHelper {
    ...
    public Map<String, String> setSettingHeader(Context context, Uri uri, Map<String, String> headers) {
        if (LOG) Xlog.v(TAG, "setSettingHeader: with uri=" + uri);
        if (isOMAEnabled() && uri != null) {
    ...
    protected boolean isOMAEnabled() {
        boolean enabled = SystemProperties.getBoolean("ro.mtk_omacp_support", false) ||
                          SystemProperties.getBoolean("ro.mtk_dm_app", false);
