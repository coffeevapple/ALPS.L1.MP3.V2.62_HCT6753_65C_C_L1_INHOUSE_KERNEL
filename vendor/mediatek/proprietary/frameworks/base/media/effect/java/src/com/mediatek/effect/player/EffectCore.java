    ...
    public synchronized SurfaceTexture graphRun() {
        Runnable EffectThread = new Runnable() {
            public void run() {
                ...
                try {
                    ...
                    e.printStackTrace();
                    // notify AP "process()=false"
                    mTool.log('v', "graphRun() mProcessDone:" + mProcessDone);
                    if (null != mProcessDone) {
                        submit(new Runnable() {
                            @Override
                            public void run() {
                                Thread.currentThread().setName("[" + mTool.getID() +
                                    "] OnProcessDone Call Back (error)");
                                mTool.log('e', "graphRun() notify AP process()=false");
                                if (null != mProcessDone) {
                                    mProcessDone.onProcessDone(null);
                                }
                            }
                        });
                    }
        ...
        if (mSourceFilter != null) {
            while (true) {
                ...
                if (waitCount >= 100 || false == isGraphRunning) {
