...
public class MediaEncoderOutFilter extends Filter {
    ...
    public void prepare(FilterContext context) {
        ...
        // for Debug dump
        // mProgram = new ShaderProgram(context, mInvertShader);
        // mProgram.setHostValue("factor", (float) mFactor / 10.0f);
    ...
    public synchronized void process(FilterContext context) {
        ...
        // for Debug dump
        // mFactor++;
        // mProgram.setHostValue("factor", (float) (mFactor % 10) / 10.0f);

        // Process
        mProgram.process(input, mScreen);

        // for Debug dump
        /*Bitmap bb = input.getBitmap();
        Bitmap cc = MyUtility.generateSmallImage(bb, 0.1f);
        bb.recycle();
        mTool.saveBitmap(cc, "/storage/sdcard0/Screen" + mFactor + "_" + mTimestampNs + ".png");
        cc.recycle();*/
        ...
        if (isSubmitFirstClose == true && isRunFirstClose == false) {
            ...
            // specific handling on android L
            // notify MediaRecorder to stop and wait EOS
            // MediaRecorder shell make sure all the swapped buffers to be encodered.
            try {
                mTool.log('w', "call MediaRecorder.stop()");
                mMediaRecorder.stop();
    ...
    private synchronized void stopRecording(FilterContext context) {
        ...
        try {
            ...
            mTool.log('e', "stopRecording() MediaRecorder.stop() failed!");
            // ignored the exception for error handling that effect library can keep processing
            // throw new MediaRecorderStopException("MediaRecorder.stop() failed!", e);
        }
