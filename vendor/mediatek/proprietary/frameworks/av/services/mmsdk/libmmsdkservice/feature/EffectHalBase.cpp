...
start()
{
    ...
    ALOGD("[%s]: uid=%d, mState=%d", __FUNCTION__, mUid, mState);
...
status_t
EffectHalBase::
setOutputSurfaces(android::Vector<android::sp<android::IGraphicBufferProducer> > surfaces)
{
    FUNCTION_LOG_START;
    status_t ret = OK;
...
addInputFrame(const sp<IImageBuffer> frame, const sp<EffectParameter> parameter)
{
    ...
    ret = addInputFrameImpl(frame, parameter);
...
status_t
EffectHalBase::
addInputFrameDone(EffectResult result, const sp<EffectParameter> parameter, android::status_t state)
{
    FUNCTION_LOG_START;
    status_t ret = state;
...
//#if 0   //TTT3
    ...
    if(listener != 0)
    {
        if(ret == OK)
        {
            listener->onInputFrameProcessed(NULL, parameter, result);
        }
        else
        {
            listener->onFailed(NULL, result);
...
addOutputFrame(const sp<IImageBuffer> frame, const sp<EffectParameter> parameter)
{
    ...
    // action
    ret = addOutputFrameImpl(frame, parameter);
...
status_t
EffectHalBase::
addOutputFrameDone(EffectResult result, const sp<EffectParameter> parameter, android::status_t state)
{
    FUNCTION_LOG_START;
    status_t ret = state;
    ...
    if(listener != 0)
    {
        if(ret == OK)
        {
            listener->onOutputFrameProcessed(NULL, parameter, result);
        }
        else
        {
            listener->onFailed(NULL, result);
