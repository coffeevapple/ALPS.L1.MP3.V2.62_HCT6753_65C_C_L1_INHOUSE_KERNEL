...
public class MusicTestUtils {
    ...
    private static final String SONGS_LESS_THAN_TRHEE = "There are less than three songs or the system performance is bad, please push more or reboot phone to make music pass test";
    ...
    public static void invokeDeclaredMethod(Activity activity, String methodName, Class[] classes, Object[] para) {
        try {
            Method method = activity.getClass().getDeclaredMethod(methodName, classes);
            method.setAccessible(true);
            method.invoke(activity, para);
            Log.d(TAG, "invokeDeclaredMethod: methodName = " + methodName + ", para length = " + para.length);
     ...
     */
    public static LocalActivityManager getDeclaredLocalActivityManager(Activity activity, String string) {
        try {
            Field field = activity.getClass().getDeclaredField(string.toString());
            field.setAccessible(true);
    ...
    public static boolean checkErrorViewState(View errorView, int expectState) {
        ...
        Log.e(TAG, "checkSdcardState: expectState = " + expectState + ", actualState = " + errorView.getVisibility());
        return !isTimeOut;
    }
    ...
    public static boolean isMoreThanThreeSongs(Context context) {
        ...
        // Requery the songs number again for 6 times to makesure there are really no songs in Music.
        // If after 6 times requery there are no songs but we sure there are some songs in sdcard, maybe
    ...
    public static void deleteAllPlaylist(Activity activity) {
        ...
        try {
            if (c != null && c.moveToFirst()) {
                do {
                    Uri deleteUri = ContentUris.withAppendedId(MediaStore.Audio.Playlists.EXTERNAL_CONTENT_URI, c.getInt(0));
                    Log.d(TAG, "deleteAllPlaylist: deleteUri = " + deleteUri + ", playlist = " + c.getString(1)
    ...
    public static void deleteAllUnknownTracks(Activity activity) {
        Cursor c = activity.getContentResolver().query(
                ...
                MediaStore.Audio.Artists.ARTIST + "=?" + " OR " + MediaStore.Audio.Albums.ALBUM + "=?",
        ...
        if (c != null) {
            try {
                ...
                do {
                    Uri deleteUri = ContentUris.withAppendedId(MediaStore.Audio.Media.EXTERNAL_CONTENT_URI, c.getInt(0));
                    activity.getContentResolver().delete(deleteUri, null, null);
                    try {
        ...
        if (storageManager != null) {
            ...
            if (volumePath != null) {
                ...
                for (int i = 0; i < length; i++) {
                    ...
                    Log.d(TAG, "hasMountedSDcard: path = " + volumePath[i] + ",status = " + status);
