...
public class NetworkModeSelectHelper {
    ...
    public NetworkModeSelectHelper(Instrumentation ins, SoloDecorator solo) {
        mInst = ins;
        mSolo = solo;
        initSimMap();
    }
...
//      List<SimInfoRecord> simList = SimInfoManager.getInsertedSimInfoList(this);
    ...
    private boolean open3GSwitch() {
        ...
        Log.d(TAG, "mIs3GSwitchManualChangeAllowed: " + mIs3GSwitchManualChangeAllowed);
        ...
        Log.d(TAG, "mManualAllowedSlot: " + slotId);
        int id = currentActivity.getResources().getIdentifier("com.android.phone:string/enable_3G_service", null, null);
        log("get str :" + currentActivity.getString(id));
        mSolo.clickOnText(currentActivity.getString(id));
        mInst.waitForIdleSync();
    ...
    public boolean isStateInService(int slotId) {
        boolean isInService = false;
        int waitTimesLeft = CONFIG_WAIT_SECONDS; // check every second, at most 1 minute
        ...
        while (!isInService && serviceState != null) {
            isInService = (serviceState.getState() == ServiceState.STATE_IN_SERVICE);
            if (isInService || waitTimesLeft == 0) {
                break;
            } else {
                waitTimesLeft--;
                mSolo.sleep(TIME_ONE_SECOND);
                log("isInService = " + isInService + ", waitTimesLeft = " + waitTimesLeft);
            }
        }

        log("isStateInService()... isInService: " + isInService);
        return isInService;
    }
