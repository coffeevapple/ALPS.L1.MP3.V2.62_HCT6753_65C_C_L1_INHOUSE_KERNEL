...
public class ConfigActivity extends Activity {
    ...
    private abstract class SettingView extends FrameLayout {
        ...
        public void send() {
            if (setting.label.equals("ims_roaming_mode")) {
                SystemProperties.set("persist.sys.ims.roaming", getValue());
            }
            if (setting.label.equals("Authentication")) {
    ...
    private class SettingSingleSelectView extends SettingView {
        ...
        protected void setValue(String value) {
            Xlog.d(TAG, "setValue " + setting.label + ", " + value);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Xlog.i(TAG, "sub id " + subId);
        int phoneId = SubscriptionManager.getPhoneId(subId);
        Xlog.i(TAG, "phone id " + phoneId);
        int phoneCount = TelephonyManager.getDefault().getPhoneCount();
        Xlog.i(TAG, "phone count " + phoneCount);
    ...
    private String parseCommandResponse(String data) {
        Xlog.d(TAG, "raw data: " + data);
        Pattern p = Pattern.compile("\\+ECFGGET:\\s*\".*\"\\s*,\\s*\"(.*)\"");
        Matcher m = p.matcher(data);
        while (m.find()) {
            String value = m.group(1);
            Xlog.d(TAG, "value: " + value);
            return value;
        }
        Xlog.e(TAG, "wrong format: " + data);
