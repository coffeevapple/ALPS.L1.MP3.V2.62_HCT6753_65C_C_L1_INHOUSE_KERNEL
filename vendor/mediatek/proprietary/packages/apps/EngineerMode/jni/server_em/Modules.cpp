...
static void exec_cmd(RPCClient* msgSender) {
    ...
    do {
        ...
        if (ret < 0) {
            XLOGD("Receive fail; error:%s", strerror(errno));
            msgSender->PostMsg(ERRORINFO(RECIEVE_FAIL));
            return;
        }
        recv_count += ret;
    } while (recv_count < len);
    XLOGD("exec_cmd:%s", cmd_buff);
    ...
    if (fpipe == NULL) {
        XLOGE("exec_cmd popen fail, error:%s", strerror(errno));
    ...
    while (!feof(fpipe)) {
        ...
        if (ret < 0) {
            XLOGD("fread error:", strerror(errno));
...
out:
    ...
    pthread_mutex_unlock(&exec_lock);
    if (ret != 0) {
        XLOGD("abnormal status code:%d, cmd exit code:%d, errno:%d, %s", ret, WEXITSTATUS(ret), errno, strerror(errno));
