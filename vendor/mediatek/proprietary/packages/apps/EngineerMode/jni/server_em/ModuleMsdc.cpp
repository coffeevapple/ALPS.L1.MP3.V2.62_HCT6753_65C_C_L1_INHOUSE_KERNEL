...
{
	if(hostNum > MSDC_MAX_HOST_MUM || clkPU >7 || clkPD >7 || cmdPU >7 || cmdPD >7 || datPU >7 || datPD >7 || rstPU > 7 || rstPD > 7 || dsPU > 7 || dsPD > 7 ||hopBit >3 || hopTime > 5)
	{
	...
	XLOGD("set: clk_pu=%d,  clk_pd = %d, cmd_pu=%d,  cmd_pd = %d, dat_pu=%d,  dat_pd = %d, rst_pu = %d, rst_pd = %d, ds_pu = %d, ds_pd = %d\r\n",
...
int ModuleMsdc::getCurrentInternal(int hostNum, int opcode)
{
	...
	command.ds_pu_driving = 0xF;
	command.ds_pd_driving = 0xF;
	command.rst_pu_driving = 0xF;
    ...
    if(opcode == 0){
	  ...
	  if (chipid >= MTK_6589_SUPPORT) {
	      if (l_clkIdx_pu != 0xF && l_cmdIdx_pu != 0xF && l_datIdx_pu != 0xF)
	  	  {
	  	      XLOGD("6589 success: clk_pu=%d,  clk_pd = %d, cmd_pu=%d,  cmd_pd = %d, dat_pu=%d,  dat_pd = %d, rst_pu = %d, rst_pd = %d, ds_pu = %d, ds_pd = %d\r\n", 
	  				l_clkIdx_pu, l_clkIdx_pd, l_cmdIdx_pu, l_cmdIdx_pd, l_datIdx_pu, l_datIdx_pd, l_rstIdx_pu, l_rstIdx_pd, l_dsIdx_pu, l_dsIdx_pd);
	  				return ( ( l_dsIdx_pu << 28) | (l_rstIdx_pu << 24) |(l_datIdx_pd << 20) | (l_datIdx_pu << 16) | (l_cmdIdx_pd << 12) | (l_cmdIdx_pu << 8) | (l_clkIdx_pd << 4) | l_clkIdx_pu);
	  	  } else {
		      XLOGD("error: clk_pu=%d,  clk_pd = %d, cmd_pu=%d,  cmd_pd = %d, dat_pu=%d,  dat_pd = %d, rst_pu = %d, rst_pd = %d, ds_pu = %d, ds_pd = %d\r\n",
	  				l_clkIdx_pu, l_clkIdx_pd, l_cmdIdx_pu, l_cmdIdx_pd, l_datIdx_pu, l_datIdx_pd, l_rstIdx_pu, l_rstIdx_pd, l_dsIdx_pu, l_dsIdx_pd);
		      return -1;
	      }
	  }	else {
...
int ModuleMsdc::setSd30ModeInternal(int hostNum, int sd30Mode, int sd30MaxCurrent, int sd30Drive, int sd30PowerControl)
{
	...
	if(hostNum > MSDC_MAX_HOST_MUM || sd30Mode > 6 || sd30MaxCurrent > 3 || sd30Drive > 3 || sd30PowerControl > 1)
	{
