                ...
                if (mSimCount >= 1) {
                    for (int i = 1; i <= mSimCount; i++) {
                        if (mContactCheckTypes[i]) {
                            params.add(mSimInfoList.get(i - 1).getDisplayName().toString());
                            MyLogger.logD(CLASS_TAG, " mDisplayName is "
                                    + mSimInfoList.get(i - 1).getDisplayName());
                        }
                    }
                }
...
//                showProgress(0, null);
                ...
                if (path == null) {
                    ...
                    MyLogger.logE(CLASS_TAG, "unkown error");
                    Bundle b = new Bundle();
...
//                    showDialog(DialogID.DLG_CREATE_FOLDER_FAILED, b);
        ...
        if (simInfo != null) {
            ...
            if (mSimInfoList != null) {
                MyLogger.logI(CLASS_TAG, "[mSimInfoList]===>mSimInfoList size = "
                        + mSimInfoList.size());
                for (SubscriptionInfo tsimInfo : mSimInfoList) {
                    int simId = tsimInfo.getSimSlotIndex();
                    if (isSIMLocked(simId) && isSimCardSelected(tsimInfo.getSimSlotIndex())) {
    ...
    private class InitPersonalDataTask extends AsyncTask<Void, Void, Long> {
        ...
        protected Long doInBackground(Void... arg0) {
            ...
            boolean isnotWifiOnly = cm.isNetworkSupported(ConnectivityManager.TYPE_MOBILE);

            MyLogger.logD(CLASS_TAG, "isnotWifiOnly = " + isnotWifiOnly);
            if (!isnotWifiOnly) {
            ...
            for (int i = 0; i < num; i++) {
                ...
                switch (types[i]) {
                    ...
                    case ModuleType.TYPE_MESSAGE:
                        ...
                        MyLogger.logD(CLASS_TAG, "countSMS = " + countSMS + "countMMS" + countMMS);
                        if (countMMS != 0) {
