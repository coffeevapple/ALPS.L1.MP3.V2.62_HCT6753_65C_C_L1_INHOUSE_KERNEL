...
public class BookmarkBackupComposer extends Composer {
    ...
    public boolean init() {
        ...
        mCursor = mContext.getContentResolver().query(
                BrowserContract.Bookmarks.CONTENT_URI, null,
                BrowserContract.Bookmarks.IS_FOLDER + "=0 and " + where, null,
                null);
    ...
    protected boolean implementComposeOneEntity() {
        ...
        if (mCursor != null && !mCursor.isAfterLast()) {
            ...
            String title = mCursor.getString(mCursor
                    .getColumnIndex(BrowserContract.Bookmarks.TITLE));
            String url = mCursor.getString(mCursor.getColumnIndex(BrowserContract.Bookmarks.URL));
            MyLogger.logD(CLASS_TAG, "implementComposeOneEntity:" + "title-" + title + ", url-"
                    + url);
            mCursor.moveToNext();
            ...
            File file = new File(mFolderFile.getAbsolutePath() + File.separator
                    + title + FILE_EXT);
            if (!file.exists()) {
                try {
                    file.createNewFile();
