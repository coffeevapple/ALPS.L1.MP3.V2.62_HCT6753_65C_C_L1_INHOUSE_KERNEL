...
final class HttpManager {
                    ...
                    public void handleMessage(Message msg) {
                        switch (msg.what) {
                            ...
                            case SERVER_VERSION_ERROR_TOAST:
                                Toast.makeText(mContext, R.string.server_version_error,
    ...
    void notifyNewVersion() {
        ...
        if (Util.getUpdateType() == Util.UPDATE_TYPES.OTA_UPDATE_ONLY) {
            ...
            Util.setAlarm(mContext, AlarmManager.RTC_WAKEUP, Calendar.getInstance()
                    .getTimeInMillis() + Util.REFRESHTIME,
                    Util.Action.ACTION_REFRESH_TIME_OUT);

            if (mHandler != null) {
                mHandler.sendMessage(mHandler
                        .obtainMessage(SystemUpdateService.MSG_NOTIFY_QUERY_DONE));
    ...
    int writeFile(HttpResponse response, long currSize) {
        ...
        if (mDownloadInfo.getDLSessionStatus() == DownloadInfo.STATE_DOWNLOADING) {
            ...
            Util.setAlarm(mContext, AlarmManager.RTC, Calendar.getInstance().getTimeInMillis()
                    + AUTO_DL_TIME,
        ...
        if (mHandler != null) {
            ...
            case HTTP_RESPONSE_VERSION_DELETE:
                ...
                case HTTP_RESPONSE_NETWORK_ERROR:
                    if (mDownloadInfo.getDLSessionStatus() != DownloadInfo.STATE_QUERYNEWVERSION) {
                        mHandler.sendMessage(mHandler
                                .obtainMessage(SystemUpdateService.MSG_NETWORKERROR));
                    } else {
                        mHandler.sendMessage(mHandler
                                .obtainMessage(SystemUpdateService.MSG_NOTIFY_QUERY_DONE));
                    }
            ...
            case HTTP_RESPONSE_DELTA_DELETE:
                    mHandler.sendMessage(mHandler
                            .obtainMessage(SystemUpdateService.MSG_DELTADELETED));
                    break;
            ...
            case HTTP_RESPONSE_UNZIP_ERROR:
                case HTTP_RESPONSE_UNZIP_CKSUM:
                    mHandler.sendMessage(mHandler
                            .obtainMessage(SystemUpdateService.MSG_UNZIP_ERROR));
                    break;

                case HTTP_FILE_NOT_EXIST:

                    mHandler.sendMessage(mHandler
                            .obtainMessage(SystemUpdateService.MSG_FILE_NOT_EXIST));
                break;
            default:

                    mHandler.sendMessage(mHandler
                            .obtainMessage(SystemUpdateService.MSG_UNKNOWERROR));
            }
        }
    }
    ...
    void onDownloadImage() {
        ...
        BasicNameValuePair deltaId = new BasicNameValuePair(
                mDownloadInfo.getFullPkgFlag() ? "versionId" : "deltaId",
    ...
    void onDownloadComplete() {
        ...
        if (!Util.checkIfTopActivity(mContext, OtaPkgManagerActivity.CLASS_NAME)) {
            ...
            reminder.putExtra(ForegroundDialogService.DLG_ID,
