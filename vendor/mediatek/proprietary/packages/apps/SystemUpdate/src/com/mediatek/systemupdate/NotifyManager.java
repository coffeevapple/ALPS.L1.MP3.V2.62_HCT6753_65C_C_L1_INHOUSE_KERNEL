...
class NotifyManager {
    ...
    void showDownloadingUnknownPer() {
        ...
        setNotificationProgress(
                R.drawable.stat_download_downloading, contentTitle, 0, false, true);
    ...
    void showDownloadingNotificaton(String version, int currentProgress, boolean isOngoing) {
        ...
        mNotificationType = NOTIFY_DOWNLOADING;
        String contentTitle = mNotificationContext.getString(R.string.app_name);
        setNotificationProgress(R.drawable.stat_download_downloading, contentTitle,
                currentProgress, isOngoing, false);
    }

    private void setNotificationProgress(int iconDrawableId, String contentTitle,
            int currentProgress, boolean isOngoing,
            boolean isUnknown) {
        if (mNotificationBuilder == null) {
            ...
            mNotificationBuilder.setAutoCancel(true).setOngoing(isOngoing)
    ...
    private void configAndShowNotification(int iconDrawableId, CharSequence contentTitle,
        ...
        mNotificationBuilder.setAutoCancel(autoCancle).setContentTitle(contentTitle)
                .setContentText(contentText)
                .setSmallIcon(iconDrawableId).setWhen(System.currentTimeMillis())
                .setOngoing(isOngoing)
                .setContentIntent(getPendingIntenActivity(intentFilter));

        Notification notification = new Notification.BigTextStyle(mNotificationBuilder).bigText(
                contentText).build();
