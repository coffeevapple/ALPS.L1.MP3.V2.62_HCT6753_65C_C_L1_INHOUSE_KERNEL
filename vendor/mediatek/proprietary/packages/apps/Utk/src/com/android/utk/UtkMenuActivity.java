...
public class UtkMenuActivity extends ListActivity {
    ...
    private final BroadcastReceiver mIRStateChangeReceiver = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            if ((SvlteRatController.INTENT_ACTION_START_SWITCH_ROAMING_MODE.equals(intent.getAction())) ||
                (SvlteRatController.INTENT_ACTION_FINISH_SWITCH_ROAMING_MODE.equals(intent.getAction()))) {
                //Query RAT Mode
                ...
                if (phoneId == mPhoneId) {
                    LteDcPhoneProxy lteDcPhoneProxy = (LteDcPhoneProxy) PhoneFactory.getPhone(mPhoneId);
    ...
    private void register4GDataModeObserver() {
        ...
        Uri uri = Settings.Global.getUriFor(Settings.Global.LTE_ON_CDMA_RAT_MODE);
