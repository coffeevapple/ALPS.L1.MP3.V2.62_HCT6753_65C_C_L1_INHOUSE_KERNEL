...
public class LoaderService extends Service {
    ...
    private int mLoadProduct = LOAD_PRODUCT_AUTO;
    ...
    private static final String SAVING_OPERA_PACKGAGE_NAME = "com.opera.max.global";
    ...
    private static final int SUSPEND_REQUEST_LAUNCH_OPERAMAX = 3;
    ...
    private final ILoaderService.Stub binder = new ILoaderService.Stub() {
        ...
        public void startSaving() {
            ...
            SystemProperties.set("persist.service.operamax", "1");
        ...
        public void launchOperaMAX() {
            ...
            if (savingService != null) {
                ...
                bindService(getSavingServiceIntent(), savingServiceConnection, Context.BIND_AUTO_CREATE);
                suspendRequest = SUSPEND_REQUEST_LAUNCH_OPERAMAX;
            }
        }
        ...
        public void setCompressLevel(int level) {
            if (savingService != null) {
                try {
                    savingService.setCompressLevel(level);
                } catch (RemoteException e) {
                }
            }
            else {
        ...
        public int getCompressLevel() {
            ...
            if (savingService != null) {
                try {
                    level = savingService.getCompressLevel();
                } catch (RemoteException e) {
                }
            }
            else {
    ...
    private void selectLoadProduct() {
        boolean oupeng = SDKUtils.isPackageInSystem(this, SAVING_OUPENG_PACKGAGE_NAME);
        boolean operamax = SDKUtils.isPackageInSystem(this, SAVING_OPERA_PACKGAGE_NAME);

        if (oupeng && operamax) {
            mLoadProduct = LOAD_PRODUCT_AUTO;
        } else if (oupeng) {
            mLoadProduct = LOAD_PRODUCT_OUPENG;
        } else if (operamax) {
            mLoadProduct = LOAD_PRODUCT_OPERA;
        } else {
            mLoadProduct = LOAD_PRODUCT_AUTO;
        }
        ...
        if ((DEBUG & DEBUG_SAVING_MODE) != 0) {
            ...
            Log.d(TAG, "mLoadProduct: " + mLoadProduct);
        }
    ...
    private void runSuspendRequest() {
        switch (suspendRequest) {
            ...
            case SUSPEND_REQUEST_STOP_SAVING:
                ...
                break;
            case SUSPEND_REQUEST_LAUNCH_OPERAMAX:
                try {
                    if ((DEBUG & DEBUG_SAVING_MODE) != 0) {
                        Log.d(TAG, "lauchOperaMax in runSuspendRequest in runSuspendRequest");
                    }
                    savingService.launchOperaMAX();
                } catch (RemoteException e) {
                    Log.e(TAG, "lauchOperaMax RemoteException in runSuspendRequest");
                }
                break;
