...
public class RegisterService extends Service {
    ...
    public void onCreate() {
        ...
        if (isSingleLoad()) {
            ...
            Log.d(SUB_TAG, "Single SIM load.");
        } else {
            mSlotList = Const.UIM_ID_LIST;
            Log.d(SUB_TAG, "Dual SIM load.");
        }
    ...
    private boolean isUimAvailable(int uimId) {
        ...
        if (mTelephonyManager.hasIccCard(uimId)) {
            ...
            if (subId == null || subId[0] < 0) {
                Log.e(SUB_TAG, "[isUimAvailable] getSubId invalid!");
                return false;
            }
            ...
            if (TelephonyManager.PHONE_TYPE_CDMA == phoneType) {
                String netwOperator = mTelephonyManager.getNetworkOperatorForSubscription(subId[0]);
                ...
                if (Const.OPERATOR_CT.equals(netwOperator)
                        ...
                        || Const.OPERATOR_CT_4G.equals(netwOperator)) {
                    ...
                    if (netwOperator.equals(simOperator)
                            || (netwOperator.equals(Const.OPERATOR_CT)
                                    && simOperator.equals(Const.OPERATOR_CT_4G))
                            || (netwOperator.equals(Const.OPERATOR_CT_4G)
                                    && simOperator.equals(Const.OPERATOR_CT))) {
                        return true;
                    }
                }
            }
    ...
    private String[] getSavedImsi() {
        ...
        String[] imsiArr = new String[mSlotList.length];
        try {
    ...
    private void writeEsnToUim() {
        ...
        try {
            byte[] writeResult = iTel.iccExchangeSimIOExUsingSlot(0, fileId, writeCommand, 0, 0,
                    sEsnLength, path, pEsnReverse, null);
    ...
    public String getCurrentLogonImsi() {
        ...
        if (mTelephonyManager != null) {
            ...
            if (currentUim != Const.UIM_NONE) {
                int[] subId = SubscriptionManager.getSubId(currentUim);
                if (subId == null || subId[0] == -1) {
                    Log.e(SUB_TAG, "[getCurrentLogonImsi] getSubId invalid!");
                    return null;
                }
                currentLogonImsi = mTelephonyManager.getSubscriberId(subId[0]);
    ...
    private String[] getImsiFromUim() {
        if (mTelephonyManager != null) {
            ...
            int length = mSlotList.length;
            String[] imsiArr = new String[length];
            for (int i = 0; i < length; ++i) {
                int uimId = mSlotList[i];
                if (isUimAvailable(uimId)) {
                    int[] subId = SubscriptionManager.getSubId(uimId);
                    if (subId == null || subId[0] == -1) {
                        Log.e(SUB_TAG, "[getImsiFromUim] getSubId invalid!");
                        return null;
                    }
                    imsiArr[i] = mTelephonyManager.getSubscriberId(subId[0]);
                    /// M: To avoid JE.
                    if (imsiArr[i] == null) {
                        Log.e(SUB_TAG, "[getImsiFromUim] Get IMSI null! subId[0]: " + subId[0]);
    ...
    private String getEsnFromUim() {
        ...
        ITelephony iTel = ITelephony.Stub.asInterface(
                ServiceManager.getService(Context.TELEPHONY_SERVICE));
        try {
            byte[] readResult = iTel.iccExchangeSimIOExUsingSlot(getCurrentLogonUim(), fileId,
                    readCommand, 0, 0, 8, path, null, null);
            if (readResult != null && readResult.length > 2) {
                int realLength = 0;
                if (readResult[0] == 0x04) { /// M: pESN, 04abababab
                    Log.d(SUB_TAG, "getEsnFromUim(), pESN saved in UIM.");
                    realLength = readResult.length - 5;
                } else { /// M: MEID, 07ababababababab
                    Log.d(SUB_TAG, "getEsnFromUim(), MEID saved in UIM.");
                    realLength = readResult.length - 2;
                }
            ...
            Log.e(SUB_TAG, "Read failed!" + e.getMessage());
            e.printStackTrace();
        }
    ...
    private boolean isImsiSame() {
        ...
        for (int i = 0; i < imsiFromUim.length; i++) {
            if (!imsiFromUim[i].equals(imsiFromDevice[i])) {
                Log.d(SUB_TAG, "Can't find the same IMSI from device:" + imsiFromUim[i]);
                return false;
            }
        }
    ...
    private boolean isEsnSame() {
        String deviceEsn = null;

        String uimSavedEsn = getEsnFromUim();
        if (uimSavedEsn != null && uimSavedEsn.length() == 8) { /// M: pEsn
            deviceEsn = meidToEsn(getDeviceId());
        } else { /// M: MEID
            deviceEsn = getDeviceId();
        }

        if (uimSavedEsn != null && uimSavedEsn.equalsIgnoreCase(deviceEsn)) {
            Log.d(SUB_TAG, "ESN/MEID is same.");
            return true;
        } else {
            Log.d(SUB_TAG, "ESN/MEID is not same:" + uimSavedEsn + "--" + deviceEsn);
            return false;
        }
    }
