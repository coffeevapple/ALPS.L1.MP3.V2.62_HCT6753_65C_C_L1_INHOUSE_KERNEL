...
/*        iTelEx = ITelephonyEx.Stub.asInterface(ServiceManager.getService(Context.TELEPHONY_SERVICE_EX));

        if (iTelEx != null)
        {
            try {
                sca = iTelEx.getScAddressUsingSubId(subId[0]);
            } catch (RemoteException ex) {
                Log.e(TAG, "ITelephony api exception:" + ex);
            }
        }*/
		try {
			//SMS related API changes
			sca = TelephonyManagerEx.getDefault().getScAddress(subId[0]);
