  ...
  // Definition for necessary HTTP headers.
	...
	private ConnectivityManager.NetworkCallback mNetworkCallback = new ConnectivityManager.NetworkCallback() {
        ...
        public void onAvailable(Network network) {
            ...
            synchronized (MmsConnection.this) {
				acquireWakeLock();
	...
	private void startConnection(){
		if(NetworkUtil.isGeminiSupport()){
			beginMmsConnectivityGemini(mSimId);
		} else {
 ...
 private static String getHttpAcceptLanguage() {
  ...
  private static void addLocaleToHttpAcceptLanguage(
	...
	public int lookupHost(String hostname) {
     ...
     public void beginMmsConnectivity() {
		...
		NetworkRequest networkRequest = new NetworkRequest.Builder()
                                             ...
                                             .build();
        mConnMgr.requestNetwork(networkRequest, 
		 	                     mNetworkCallback, 
		 	                     mHttpSocketTimeout);
    }
  ...
  // add for gemini
	 public void beginMmsConnectivityGemini(int simId) {
		 // Take a wake lock so we don't fall asleep before the message is downloaded.
		 createWakeLock();
		 ...
		 NetworkRequest networkRequest = new NetworkRequest.Builder()
                                             .addTransportType(NetworkCapabilities.TRANSPORT_CELLULAR)
                                             .addCapability(NetworkCapabilities.NET_CAPABILITY_MMS)
                                             .setNetworkSpecifier(Long.toString(slotId))
                                             .build();
         mConnMgr.requestNetwork(networkRequest, 
		 	                     mNetworkCallback, 
		 	                     mHttpSocketTimeout);
