...
public class NetworkUtil{
	...
	public static boolean isGeminiSupport(){
		return (SystemProperties.get("ro.mtk_gemini_support").equals("1") ||
				SystemProperties.get("ro.mtk_gemini_3sim_support").equals("1") ||
				SystemProperties.get("ro.mtk_gemini_4sim_support").equals("1"));
	}
	
	public static int getNetworkType(){
		return TelephonyManager.getDefault().getSimState(0); 	
	}
	//
	public static int getGeminiNetworkType(int slotId){
		return TelephonyManagerEx.getDefault().getSimState(slotId);
	}
...
//		SIMInfo info = 	SIMInfo.getSIMInfoBySlot(context, slot);
		List<SubInfoRecord> infoList = SubscriptionManager.getSubInfoUsingSlotId(slot);
		
		if (!infoList.isEmpty()){
			SubInfoRecord  info = infoList.get(0);
			simId = info.subId;
		} 
		return simId;
	}
...
//		return SIMInfo.getSlotById(context, simId);
		SubInfoRecord  info = SubscriptionManager.getSubInfoForSubscriber(simId);
		if (info != null){
			return info.slotId;
		} else
		{
	...
	{
		if (SystemProperties.get("ro.mtk_gemini_4sim_support").equals("1")) {
			return SIM4 + 1;
		} else if (SystemProperties.get("ro.mtk_gemini_3sim_support").equals("1")) {
			return SIM3 + 1;
