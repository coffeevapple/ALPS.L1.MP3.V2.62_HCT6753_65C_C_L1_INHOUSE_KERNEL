...
import com.mediatek.bluetooth.BluetoothMap;
import android.bluetooth.BluetoothDevice;
import android.bluetooth.BluetoothAdapter;
import com.mediatek.bluetooth.IBluetoothMap;
...
import com.mediatek.bluetooth.map.cache.FolderListRequest;
import com.mediatek.bluetooth.map.cache.MessageListRequest;
import com.mediatek.bluetooth.map.cache.MessageRequest;
import com.mediatek.bluetooth.map.cache.SetFolderRequest;
import com.mediatek.bluetooth.map.cache.StatusSwitchRequest;
...
public class BluetoothMapServerService extends Service  implements EventChannel{
	...
	private final int MAP_SERVER_SEND_REPORT_REQ    = 102;
	...
	private class ReportEventObj {
		BluetoothDevice mDevice;
		EventReport     mReport;

		public ReportEventObj(BluetoothDevice device, EventReport report)
		{
			mDevice = device;
			mReport = report;
		}
        ...
        public void handleMessage(Message msg) {
        	...
        	switch(msg.what) {
				...
				case MAP_SERVER_SET_NOTIFICATION:					
					...
					if(mMnsConnected == true)
					{
						instance.registerCallback(mEventManager);
					}
					result = MAP.RESULT_OK;
					setNotificationRegResponse((String)msg.obj, msg.arg1, result);
					break;
				case MAP_SERVER_SEND_REPORT_REQ:
					ReportEventObj report = (ReportEventObj)msg.obj;
					if (report == null) {
						log("address is null");
						return;
					}
					if(onSendReportHandler(report) == false)
					{
	...
	public synchronized boolean sendReport(BluetoothDevice device,EventReport report) {
		ReportEventObj obj = new ReportEventObj(device, report);
		String address = device.getAddress();
		log("sendReport(): name is "+ device.getName()+",address="+address);

		Message msg = Message.obtain(mHandler, MAP_SERVER_SEND_REPORT_REQ);
		msg.obj = obj;
		mHandler.sendMessage(msg);
		return true;
	}
	
	public boolean onSendReportHandler(ReportEventObj obj) {
		BluetoothDevice device = obj.getDevice();
		EventReport     report = obj.getReport();
		String address = device.getAddress();
		log("onSendReportHandler(): name is "+ device.getName()+",address="+address);
		...
		if (isEnabled()) {
			if(sendReportNative(address, report) == 0)
			{
				return true;
			}
			else
	...
	private void setState(int state) {
		...
		if (shouldSendIntent) {
			...
			intent.putExtra(BluetoothProfileManager.EXTRA_PROFILE, Profile.MAP_SERVER);
