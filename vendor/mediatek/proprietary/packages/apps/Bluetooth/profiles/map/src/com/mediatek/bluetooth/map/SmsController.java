...
/*
    ...
    public void deregisterListener() {
        ...
        if (!convertFilterToWhere(req, selection)) {
            return null;
        }
...
//              (mailbox == Sms.MESSAGE_TYPE_OUTBOX) ? Sms.MESSAGE_TYPE_SENT:mailbox);
    ...
    //      cv.put(Mms.MESSAGE_SIZE, size);
        ...
        if (mailbox == Sms.MESSAGE_TYPE_OUTBOX && recipient != null) {
			...
			log("pushMessage message text:" + text + ", list size:" + messages.size());
    ...
    public  boolean deleteMessage(long id) {
        ...
        if (cs != null && cs.moveToFirst()) {
            int thread_id = cs.getInt(cs.getColumnIndex(Sms.THREAD_ID));
            if (thread_id == INVALID_VALUE_ID) {
                mContentResolver.delete(uri, null, null);
                mDeleteFolder.remove(Long.valueOf(id));
            } else {
                ContentValues cv = new ContentValues();
                cv.put(Sms.THREAD_ID, INVALID_THREAD_ID);
    ...
    private MessageItem composeMessageItem(Cursor cs, int mailbox) {
		...
		String name = cs.getString(PERSON_COLUMN);
...
/*      if (mSendername == null || mSendername.equals("")){
	    msg.setSenderName(address);
		}
        else {
	    msg.setSenderName(mSendername);
        }*/

        msg.setReplyAddr(null);
        msg.setRecipientName(Receivername);
	...
	private boolean convertFilterToWhere(MessageListRequest req,
                                            StringBuilder where) {
        ...
        if (NetworkUtil.SIM1 == mSimId) {
            ...
            for (index = NetworkUtil.SIM2 ; index < NetworkUtil.getTotalSlotCount(); index ++) {
                ...
                if (simid > 0)
                {
					...
					where.append(Sms.SUB_ID + " <> " + Long.toString(simid));
                }
            }
        } else {
        ...
        if (mailbox != MESSAGE_TYPE_DELETE) {
			where.append(Sms.THREAD_ID + " <> " + Integer.toString(INVALID_THREAD_ID));
        } else {
        	where.append(Sms.THREAD_ID + " = " + Integer.toString(INVALID_THREAD_ID));
        }
        ...
        if (startTimeMillis > 0) {
			...
			where.append(Sms.DATE + " >= " + Long.toString(startTimeMillis));
        } else if (endTimeMillis > 0) {
            if(where.length() != 0) {
				where.append(" AND ");
	    	}
			where.append(Sms.DATE + " <= " + Long.toString(endTimeMillis));
        }
        ...
        if (readStatus != -1) {
			...
			where.append(Sms.READ + "= " + Integer.toString(readStatus));
        }
        return true;
    }
        //MAP -> SMS
