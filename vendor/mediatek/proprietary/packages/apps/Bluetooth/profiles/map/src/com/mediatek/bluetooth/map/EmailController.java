...
import com.mediatek.bluetooth.map.Email.MessageColumns;
import com.mediatek.bluetooth.map.Email.AccountColumns;
...
import com.mediatek.bluetooth.map.mime.EmailMime;
...
//	private ControllerListener mListener;
	...
	public MessageListObject getMessageList(MessageListRequest req) {
		...
		mMessageListObject.setMask(req.getMask());
		...
		if (listCursor == null){
			log("getMessageList listCursor == null");
			return null;
		}
		...
		if (listSize > 0 && listOffset > 0 && !listCursor.move(listOffset)) {
			log("getMessageList listSize " + listSize + "listOffset " + listOffset);
			listCursor.close();
			return null;
		}
		...
		while(listCursor.moveToNext() && (listSize == 0 || mMessageListObject.getCurrentSize() < listSize)) {
			...
			if (recipient != null) {
				...
				log("getMessageList toList " + toList);
			...
			if (orignator!= null) {
                ...
                log("getMessageList fromList " + fromList);
			...
			index++;
			log("getMessageList listSize " + listSize);
			if (listSize > 0) {
		...
		log("getMessageList newMessageFlag " + newMessageFlag);
		if (newMessageFlag) {
			mMessageListObject.setNewMessage();	
		}
		
		mMessageListObject.setCurrentTime();
		log("getmessagelist currenttime " + mMessageListObject.setCurrentTime());
	...
	public boolean pushMessage(BMessageObject message) {
		...
		EmailMime mime = new EmailMime(mResolver);
		MimeParser parser;
		FileInputStream in = null;

		//paser BMessage
		try {
			in = new FileInputStream(message.getFile());
		...
		String   toAddr = mime.getRecipient();
		String   ccAddr = mime.getCCList();
		String   attachFile = new String("");
		ArrayList<Uri> uris = new ArrayList<Uri>();
		String   strBody = new String("");
		Intent intent = null;
		MimeAttachment[] attachments = mime.getAttachment();
		if(attachments.length == 1)
		{
			strBody = new String(attachments[0].mContentBytes);
			intent = new Intent(Intent.ACTION_SEND);
			if(attachments[0].mLocation != null || attachments[0].mFileName != null)
			{
				attachFile = attachments[0].mLocation + attachments[0].mFileName;
		...
		else if((attachments.length > 1))
		{
			...
			{
				if(attachment.mLocation != null || attachment.mFileName != null)
				{
					attachFile = attachment.mLocation + attachment.mFileName;
					uris.add(Uri.fromFile(new File(attachFile)));
				}
			}
			if(uris.size() > 0)
				intent.putExtra(Intent.EXTRA_STREAM, uris);
		}
		log("attachments size: " + attachments.length + ", mMimeType: " + attachments[0].mMimeType +
			", mName: " + attachments[0].mName + ", mCharset: " + attachments[0].mCharset + ", mBody:"
			+ strBody);
		intent.setType("email/*");
		if(toAddr != null)
		{
			String[] toAddrList = toAddr.split(";");
			intent.putExtra(Intent.EXTRA_EMAIL, toAddrList);
		}
		if(ccAddr != null)
		{
			String[] ccAddrList = ccAddr.split(";");
			intent.putExtra(Intent.EXTRA_CC, ccAddrList);
		}
		intent.putExtra(Intent.EXTRA_SUBJECT, mime.getSubject());
	...
	/*	log("composeMessageItem()");
		...
		*/
		...
		log("composeMessageItem readStatus " + readStatus);
		if(readStatus == -1) {
		...
		if (cs.getInt(COLUMN_LOADED) == 1) {
			log("composeMessageItem cs.getInt(COLUMN_LOADED) == 1");
			...
			Cursor cursor =mResolver.query(attachUri, 
									...
									null);
			if (cursor != null) {
				log("composeMessageItem cursor != null");
	...
	public boolean deleteMessage (long messageID) {
		...
		try {
			messageCursor = mResolver.query(uri, message_PROJECTION,
