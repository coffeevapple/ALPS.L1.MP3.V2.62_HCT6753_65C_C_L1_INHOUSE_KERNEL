...
import com.mediatek.bluetooth.map.util.IOUtils;
...
public class EmailMime extends MimeBase {
	...
	private static final int BODY_HTML_REPLY_COLUMN			= 0;
	private static final int BODY_TEXT_REPLY_COLUMN			= 1;
	private static final int BODY_INTRO_COLUMN				= 2;
	...
	public EmailMime(ContentResolver resolver){
	...
	private void loadBody(){
		...
		InputStream in = null;
		try{
			bodyCursor = mContentResolver.query(Email.BODY_URI,
								                BODY_PROJECTION, 
								                MESSAGE_KEY + " = ? ",
        ...
        try {
			uri = ContentUris.withAppendedId(Email.MESSAGE_BODY_HTML_URI, mId);
        	in = mContentResolver.openInputStream(uri);
			try {
	            mBody.mHtmlContent = IOUtils.toString(in);
				log("Html Content: " + mBody.mHtmlContent);
			}finally {
            	in.close();
        	}
        } catch (IOException io) {
         ///
         log("Query Html Content failed: " + io);
		}

        try {
			uri = ContentUris.withAppendedId(Email.MESSAGE_BODY_TEXT_URI, mId);
        	in = mContentResolver.openInputStream(uri);
			try {
	            mBody.mTextContent = IOUtils.toString(in);
				log("Text Content: " + mBody.mTextContent);
			}finally {
            	in.close();
        	}
        } catch (IOException io) {
         ///
         log("Query Text Content failed: " + io);
		}
	}
   ...
   public String getBCCList(){
		return mHeaders.mBcc;
   }

   public String getSubject(){
		return mHeaders.mSubject;
   }
