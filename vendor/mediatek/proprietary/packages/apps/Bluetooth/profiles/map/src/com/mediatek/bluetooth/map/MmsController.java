...
import com.mediatek.bluetooth.map.cache.MessageItem;
import com.mediatek.bluetooth.map.cache.MessageListObject;
import com.mediatek.bluetooth.map.cache.MessageListRequest;
import com.mediatek.bluetooth.map.cache.MessageRequest;
import com.mediatek.bluetooth.map.util.HandleUtil;
import com.mediatek.bluetooth.map.util.NetworkUtil;
import com.mediatek.bluetooth.map.mime.MimeParser;
import com.mediatek.bluetooth.map.mime.MmsMime;
...
class MmsController extends Controller 
	...
	public MessageListObject getMessageList(MessageListRequest req) {
		...
		public boolean pushMessage(BMessageObject obj) {
			...
			EncodedStringValue subject = pdu.getSubject();
			String mSubject = null;
			
			if(subject != null)
			{
				mSubject = PduPersister.toIsoString(subject.getTextString());
			}
			values.put(Mms.MESSAGE_BOX, mail);
	        values.put(Mms.READ, 0);
	        values.put(Mms.SEEN, 0);

			log("Subject:"+mSubject);
			if(mSubject != null) {
	            values.put(Mms.SUBJECT, mSubject);
	        ...
	        values.put("sub_id", sim);
			if(mime.getTextOnly() == true)
            	values.put(Mms.TEXT_ONLY, true);
	        values.put(Mms.MESSAGE_SIZE, mime.getSize());

			// Get thread id
	        Set<String> recipients = new HashSet<String>();
	        recipients.addAll(Arrays.asList(mime.getToField()));
	        ...
	        try {
				...
				for(index = 0; (body != null) && (index < body.getPartsNum()); index++){
					int count = 0;
					PduPart part = body.getPart(index);;
					...
					if(part != null)
					{
						 ...
						 String mContentLocation = null;
						 String mContentDisposition = null;
						 ...
						 if(cache != null)
					 	 {
					 	 	mContentType = new String(cache);
					 	 }

						 cache = part.getFilename();
						 if(cache != null)
					 	 {
					 	 	mPartName = new String(cache);
					 	 }
						 
                         cache = part.getContentId();
						 if(cache != null)
					 	 {
					 	 	mContentId = new String(cache);
					 	 }
						 
                         cache = part.getContentLocation();
						 if(cache != null)
					 	 {
					 	 	mContentLocation = new String(cache);
					 	 }

						 cache = part.getContentDisposition();
						 if(cache != null)
					 	 {
					 	 	mContentDisposition = new String(cache);
					 	 }
						 
		                 count++;
		                 values.clear();
		                 if(mContentType.toUpperCase().contains("TEXT")) {
		                	 values.put(Mms.Part.CONTENT_TYPE, "text/plain");
		                     values.put(Mms.Part.CHARSET, 106);
		                     if(mPartName != null) {
	                             values.put(Mms.Part.FILENAME, mPartName);
	                             values.put(Mms.Part.NAME, mPartName);
	                         } else {
	                         ...
	                         if(mContentId != null) {
	                             values.put(Mms.Part.CONTENT_ID, mContentId);
	                         } else {
	                             if(mPartName != null) {
	                                 values.put(Mms.Part.CONTENT_ID, "<" + mPartName + ">");
	                             } else {
	                         ...
	                         if(mContentLocation != null) {
	                             values.put(Mms.Part.CONTENT_LOCATION, mContentLocation);
	                         } else {
	                             if(mPartName != null) {
	                                 values.put(Mms.Part.CONTENT_LOCATION, mPartName + ".txt");
	                             } else {
	                                 values.put(Mms.Part.CONTENT_LOCATION, "text_" + count + ".txt");
	                             }
	                        }
		                    log("[pushMmsToFolder] Added TEXT part");
	                        if(mContentDisposition != null) {
	                            values.put(Mms.Part.CONTENT_DISPOSITION, mContentDisposition);
	                        }
							cache = part.getData();
							if(cache != null)
						 	{
						 	 	mDataString = new String(cache);
						 	}
							if(mDataString != null)
							{
	                            values.put(Mms.Part.TEXT, mDataString);
							}
	                        uri = Uri.parse(Mms.CONTENT_URI + "/" + handle + "/part");
	                        uri = mContentResolver.insert(uri, values);
	                        log("Added TEXT part");
		                } else if (mContentType.toUpperCase().contains("SMIL")){
							...
							if(cache != null)
						 	{
						 	 	mDataString = new String(cache, "UTF-8");
						 	}
							if(mDataString != null)
							{
	                            values.put(Mms.Part.TEXT, mDataString);
					        ...
					        if(mContentType != null) {
					            values.put(Mms.Part.CONTENT_TYPE, mContentType);
					        } else {
					            log("MMS has no CONTENT_TYPE for part " + count);
					        }
					        if(mContentId != null) {
					            values.put(Mms.Part.CONTENT_ID, mContentId);
					        } else {
					        ...
					        if(mContentLocation != null) {
					            ...
					            if(mPartName != null) {
					                values.put(Mms.Part.CONTENT_LOCATION, mPartName + ".dat");
					            } else {
					        ...
					        Uri res = mContentResolver.insert(partUri, values);

					        // Add data to part
					        OutputStream os = mContentResolver.openOutputStream(res);
					        os.write(part.getData());
					        os.close();
		                    log("Added OTHER part");
		                }
	                    if (uri != null){
	                        log("Added part with content-type: "+ mContentType + " to Uri: " + uri.toString());
	                    }
		            }
				}
			}catch (UnsupportedEncodingException e) {
	            log("[pushMmsToFolder] " + e);
	        } catch (IOException e) {
	            log("[pushMmsToFolder] " + e);
	        }
	        ...
	        values.put(Mms.Addr.TYPE, 0x89);
	        values.put(Mms.Addr.CHARSET, 106);

	        uri = Uri.parse(Mms.CONTENT_URI + "/"  + handle + "/addr");
	        uri = mContentResolver.insert(uri, values);
	        if (uri != null){
	            log("[pushMmsToFolder] NEW URI " + uri.toString());
	        }

	        values.clear();
	        values.put(Mms.Addr.CONTACT_ID, "null");
	        values.put(Mms.Addr.ADDRESS, mime.getToField());
	        values.put(Mms.Addr.TYPE, 0x97);
	        values.put(Mms.Addr.CHARSET, 106);
            ...
            log("broadcasting intent: "+sendIntent.toString());
