...
public class NetworkChangedReceiver extends BroadcastReceiver {
    ...
    /*
     * // Handle roaming notification private void
     * handleRoamingNotification(final NetworkInfo info) { if (info == null) {
     * Logger.d(TAG, "handleRoamingNotification()-info is null"); return; } if
     * (info.getType() != ConnectivityManager.TYPE_MOBILE) { Logger.d(TAG,
     * "handleRoamingNotification()-netwotk type is not TYPE_MOBILE"); return; }
     * new AsyncTask<Void, Void, Void>() {
     *
     * @Override protected Void doInBackground(Void... arg0) { if
     * (info.isRoaming()) { // Display roaming dialog if
     * (!sDeviceLastRoamingStatus) { if
     * (RcsSettings.getInstance().isRoamingAuthorized()) {
     * showRoamingNofitication(mContext, RoamingActivity.ROAMING_WITH_ENABLE); }
     * else { showRoamingNofitication(mContext,
     * RoamingActivity.ROAMING_WITH_DISABLE); } sDeviceLastRoamingStatus = true;
     * } } else { if (sDeviceLastRoamingStatus) {
     * showRoamingNofitication(mContext, RoamingActivity.UNROAMING);
     * sDeviceLastRoamingStatus = false; } } return null; } }.execute(); }
     */
    /*
     * private void showRoamingNofitication(Context context, int type) { Intent
     * intent = new Intent();
    ...
    public static void removeListener(OnNetworkStatusChangedListerner listener) {
        ...
        Logger.d(
                TAG,
                "removeListener() exit,the list size is "
                        + LISTENER_LIST.size());
    }
}
