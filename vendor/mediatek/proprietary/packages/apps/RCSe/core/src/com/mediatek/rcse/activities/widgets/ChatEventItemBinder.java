...
public class ChatEventItemBinder extends AbsItemBinder {
    ...
    public void bindView(View itemView) {
        TextView chatEventInformation = (TextView) itemView
                .findViewById(R.id.file_transfer_terminated);
        Information information = ((ChatEventInformation) mChatEventInformation)
                .getInformation();
        Logger.d(TAG, "bindView the information is " + information);
        switch (information) {
        case LEFT:
            Object leftContact = ((ChatEventInformation) mChatEventInformation)
                    .getRelatedInfo();
            bindChatEventLeft(chatEventInformation, leftContact);
            break;
        case JOIN:
            Object contact = ((ChatEventInformation) mChatEventInformation)
                    .getRelatedInfo();
            bindChatEventJoin(chatEventInformation, contact);
            break;
        default:
            Logger.e(TAG, "bindView the information is not defined "
                    + information);
            break;
        }
    }
        ...
        if (context != null) {
            ...
            if (contact instanceof String) {
                String displayName = ContactsListManager.getInstance()
                        .getDisplayNameByPhoneNumber((String) contact);
                eventInformation = context.getString(R.string.leave_group_chat,
                        displayName);
                Logger.d(TAG, "bindChatEventQui the eventInformation is "
                        + eventInformation);
            } else {
                eventInformation = context.getString(R.string.leave_group_chat,
                        context.getString(R.string.group_chat_stranger));
                Logger.e(TAG,
                        "bindChatEventLeft the contactName is not a String");
            }
    ...
    private void bindChatEventJoin(TextView textView, Object contact) {
        ...
        if (context != null) {
            ...
            if (contact instanceof String) {
                String displayName = ContactsListManager.getInstance()
                        .getDisplayNameByPhoneNumber((String) contact);
                eventInformation = context.getString(R.string.join_group_chat,
                        displayName);
                Logger.d(TAG, "bindChatEventQui the eventInformation is "
                        + eventInformation);
            } else {
