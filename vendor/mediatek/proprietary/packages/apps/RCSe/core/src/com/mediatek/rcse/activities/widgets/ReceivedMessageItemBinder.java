...
public class ReceivedMessageItemBinder extends AbsItemBinder {
    ...
    private void bindReceivedMessage(IReceivedChatMessage message, View view) {
        ...
        TextView remoteNameView = (TextView) view
        ...
        if (message != null) {
            if (message instanceof One2OneChatFragment.ReceivedMessage) {
                bindReceivedOne2OneMessage(
                        (One2OneChatFragment.ReceivedMessage) message, view);
                phoneNumUri = ((One2OneChatFragment.ReceivedMessage) message)
                        .getMessageSender();
            } else {
                bindReceivedGroupMessage(
                        (GroupChatFragment.ReceivedMessage) message, view);
                phoneNumUri = ((GroupChatFragment.ReceivedMessage) message)
                        .getMessageSender();
            }
            if (phoneNumUri != null) {
                phoneNum = PhoneUtils.extractNumberFromUri(phoneNumUri);
                Logger.e(TAG, "bindReceivedMessage the phoneNumUri is "
                        + phoneNum);
                remoteName = phoneNum;
                ...
                if (PhoneUtils.isANumber(phoneNum)) {
                    if (ContactsListManager.getInstance().isLocalContact(
                         ...
                         */
                        // remoteName =
                        // RichMessaging.getInstance().getContactAliasName(phoneNum);
                        String tempName = ContactsListManager.getInstance()
                                .getDisplayNameByPhoneNumber(phoneNum);
                        String aliasName = ((GroupChatFragment.ReceivedMessage) message)
                                .getAliasName();
                        if (aliasName != null
                                && (aliasName.equals("0")
                                        || aliasName.equals("") || aliasName
                                        .equals(tempName))) {
                            remoteName = tempName;
                        } else {
                            remoteName = tempName + " " + "~" + aliasName;
                        }
                        Logger.d(TAG, "remote name is " + remoteName
                                + " alias is =" + aliasName);

                    }
                } else {
                    Logger.e(TAG, "bindReceivedMessage() the remoteContact "
    ...
    private void bindReceivedOne2OneMessage(
            final One2OneChatFragment.ReceivedMessage message, View view) {
    ...
    private void bindReceivedGroupMessage(
            final GroupChatFragment.ReceivedMessage message, View view) {
        TextView textView = (TextView) view
                .findViewById(R.id.chat_text_display);
        // textView.setOnClickListener(this);
