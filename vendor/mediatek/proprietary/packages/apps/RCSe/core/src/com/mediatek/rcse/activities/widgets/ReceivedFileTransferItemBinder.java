 ...
 */
public class ReceivedFileTransferItemBinder extends AbsItemBinder implements
        OnClickListener {
    ...
    private FileTransfer mFile = null;
     ...
     */
    public ReceivedFileTransferItemBinder(FileTransfer file) {
        Logger.v(TAG, "ReceivedFileTransferItemBinder: ReceiveFileTransfer = "
                + file);
        mFile = file;
        if (mFile != null) {
    ...
    public void bindView(View itemView) {
        Logger.v(TAG, "bindView: itemView = " + itemView + ", mFileStruct = "
                + mFileStruct);
    ...
    private View bindReceiveInvitationView(View itemView) {
        AsyncAvatarView avatarImageView = (AsyncAvatarView) itemView
                .findViewById(R.id.peer_avatar);
        TextView displayTextView = (TextView) itemView
                .findViewById(R.id.file_transfer_display);
        ImageView imageType = (ImageView) itemView
                .findViewById(R.id.image_type);
        TextView fileSize = (TextView) itemView
                .findViewById(R.id.file_transfer_size);
        DateView dateTextView = (DateView) itemView
                .findViewById(R.id.file_transfer_time);
        Button acceptBtn = (Button) itemView
                .findViewById(R.id.file_transfer_accept);
        Button rejectBtn = (Button) itemView
                .findViewById(R.id.file_transfer_reject);
        TextView remoteNameView = (TextView) itemView
                .findViewById(R.id.remote_name);
        acceptBtn.setOnClickListener(this);
        rejectBtn.setOnClickListener(this);
        String remoteName = null;
        String phoneNum = mFileStruct.mRemote;

        if (PhoneUtils.isANumber(phoneNum)) {
            remoteName = ContactsListManager.getInstance()
                    .getDisplayNameByPhoneNumber(phoneNum);
        } else {
            Logger.e(TAG, "bindReceiveInvitationView() the remoteContact "
                    + phoneNum + " is not a real number");
        }

        if (remoteNameView != null && phoneNum != null) {
        ...
        if (mFile instanceof One2OneChatFragment.ReceivedFileTransfer) {
            phoneNum = ((One2OneChatFragment.ReceivedFileTransfer) mFile)
                    .getContactNum();
            if (phoneNum != null) {
    ...
    private View bindFailedView(View itemView) {
        TextView failedTextView = (TextView) itemView
    ...
    private View bindAcceptView(View itemView) {
        ...
        Button cancelBtn = (Button) itemView
                .findViewById(R.id.file_transfer_cancel);
        Button playPauseBtn = (Button) itemView
                .findViewById(R.id.file_transfer_play_pause);
    ...
    private View bindFinishedView(View itemView) {
        AsyncAvatarView avatarImageView = (AsyncAvatarView) itemView
                .findViewById(R.id.peer_avatar);
        DateView dateView = (DateView) itemView
                .findViewById(R.id.chat_item_date);
        TextView remoteNameView = (TextView) itemView
                .findViewById(R.id.remote_name);
        Logger.d(TAG, "bindFinishedView: avatarImageView  = " + avatarImageView
                + ", dateView = " + dateView);
        String remoteName = null;
        String phoneNum = mFileStruct.mRemote;
        ...
        if (avatarImageView != null && dateView != null) {
            ...
            Logger.d(TAG, "bindFinishedView: fileTransferView = "
                    + fileTransferView);
    ...
    private void bindCancelView(View itemView) {
        Context context = ApiManager.getInstance().getContext();
        Logger.v(TAG, "bindCancelView entry: itemView = " + itemView
                + ", context = " + context + ", mFileTransfer = " + mFile);
        ...
        stringBuilder.append(WHITE_SPACE);
        stringBuilder
                .append(context.getString(R.string.file_transfer_canceled));
        stringBuilder.append(WHITE_SPACE);
        if (mFile instanceof One2OneChatFragment.ReceivedFileTransfer) {
            stringBuilder
                    .append(((One2OneChatFragment.ReceivedFileTransfer) mFile)
                            .getContactName());
            stringBuilder.append(WHITE_SPACE);
        }
        ...
        if (fileStruct != null) {
            ...
            if (FILEPATH_NO_SPACE.equals(fileStruct.mFilePath)) {
                ...
                Toast toast = Toast.makeText(context, noSpaceText,
                        Toast.LENGTH_LONG);
                toast.show();
            } else {
    ...
    private void bindCanceledView(View itemView) {
        Context context = ApiManager.getInstance().getContext();
        Logger.v(TAG, "bindCanceledView entry: itemView = " + itemView
                + ", context = " + context + ", mFile = " + mFile);
    ...
    private void handleReceiverReject() {
        ...
        if (null != controller) {
            Message controllerMessage = controller.obtainMessage(
                    ...
                    mFile.getTag(), mFileStruct.mFileTransferTag);
    ...
    private void onUserAccept(Context context) {
        ...
        if (fileSize >= warningFileSize && warningFileSize != 0) {
            ...
            if (isRemind) {
                ...
                if (mFile instanceof One2OneChatFragment.ReceivedFileTransfer) {
                    intent.putExtra(RcsNotification.CONTACT,
                            ((One2OneChatFragment.ReceivedFileTransfer) mFile)
                                    .getContactNum());
                }
        ...
        if (fileStruct != null) {
            ...
            if (null != controller) {
                Message message = controller.obtainMessage(
                        ...
                        mFile.getTag(), fileStruct.mFileTransferTag);
