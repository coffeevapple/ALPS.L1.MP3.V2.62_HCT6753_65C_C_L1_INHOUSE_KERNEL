...
public class ImsConnectionManager implements Runnable {
	...
	private boolean volte_debug = false;
...
/**
    ...
    private static final boolean GEMINI_PHONE =
...
/**
    ...
    */
    ...
    private synchronized void connectionEvent(Intent intent) {
		...
		/*
		if(RcsUaAdapter.getInstance().isSingleRegistrationSupported() && volte_debug){
			...
			currentNetworkInterface = getVolteNetworkInterface();
			loadUserProfile();
			String localIpadd_volte = RcsUaAdapter.getInstance().getLocalIPAddress();
					//"192.168.43.1";//NetworkFactory.getFactory().getLocalIpAddress(onlyApnNetworkInfo.getType());
