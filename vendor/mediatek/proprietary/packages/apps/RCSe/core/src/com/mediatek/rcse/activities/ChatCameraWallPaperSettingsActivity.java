...
public class ChatCameraWallPaperSettingsActivity extends Activity {
    ...
    private void getWallPaperFromCamera() {
        ...
        mCameraWallPaperUri = Uri.fromFile(new File(
                ChatFragment.RCSE_TEMP_FILE_DIR,
    ...
    private void onCameraResult() {
        ...
        Logger.d(TAG, "onCameraResult(), Camera return, fileNameString = "
                + fileNameString);
        if (fileNameString != null
                && fileNameString.startsWith(One2OneChatFragment.FILE_SCHEMA)) {
            String fileFullPath = fileNameString.substring(
                    One2OneChatFragment.FILE_SCHEMA.length(),
                    fileNameString.length());
            AppSettings.getInstance().setChatWallpaper(fileFullPath);
        } else {
            Logger.e(TAG, "fileNameString = " + fileNameString
                    + ",is not start with file://");
        }
        finish();
    }
