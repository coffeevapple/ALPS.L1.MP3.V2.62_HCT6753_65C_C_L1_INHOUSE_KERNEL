...
public class IpMessageContactManager extends ContactManager {
            ...
            if (number
                ...
                if (name.equals("")) {
                    if (name != null && name.equals(ChatUtils.ANOMYNOUS_URI)) {
                        Logger.e(TAG, "getNameByNumber() number is not present"
                                + " in CONTACT_LIST ANOMYNOUS_URI = " + name);
                        name = number;
                    } else if (name == null) {
                        name = number;
                    }
    ...
    private boolean markThreadAsSpam(int[] contactIds, boolean isSpam) {
        ...
        for (int contactId : contactIds) {
            ...
            ContactsManager.createInstance(AndroidFactory
                    .getApplicationContext());
            if (isSpam) {
