...
public class PluginApiManager {
    ...
    public int getContactPresence(final String number) {
        ...
        if (info != null) {
            Logger.d(TAG, "getContactPresence number " + number
    ...
    private void queryContactsPresence() {
        ...
        String[] projection = { Data.MIMETYPE, Data.DATA1, Data.DATA2,
                Data.DATA3, Data.DATA5, Data.DATA6, Data.DATA7, Data.DATA8,
                Data.DATA9 };
        // Filter the mime types
        String selection = Data.MIMETYPE + "=? AND " + "(" + Data.DATA4
                + "=? OR " + Data.DATA4 + "=? )";
        String[] selectionArgs = { MIMETYPE_RCSE_CAPABILITIES,
        ...
        try {
            if (cursor != null) {
                while (cursor.moveToNext()) {
                    ...
                    ContactInformation info = new ContactInformation();
                    info.isRcsContact = 1;
                    info.isImSupported = cursor.getInt(imIndex) == 1 ? true
                            : false;
                    info.isFtSupported = cursor.getInt(ftIndex) == 1 ? true
                            : false;
                    info.isImageShareSupported = cursor
                            .getInt(imageSharingIndex) == 1 ? true : false;
                    info.isVideoShareSupported = cursor
                            .getInt(videoSharingIndex) == 1 ? true : false;
                    info.isCsCallSupported = cursor.getInt(csVideoIndex) == 1 ? true
                            : false;
    ...
    private PluginApiManager(Context context) {
        ...
        intent.putExtra("action",IRemoteBlockingRequest.class.getName());
