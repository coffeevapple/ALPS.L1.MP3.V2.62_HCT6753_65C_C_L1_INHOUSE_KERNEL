		...
		if (outputBufferIndex >= 0) {
			while (outputBufferIndex >= 0) {
				...
				if (sps != null && pps != null) {
					...
					/*
					 * for (int index = 0; index < frameOut.length; index++){
					 * Log.v("ENCODED BY MANIK", String.format("0x%20x",
					 * frameOut[index])); }
					 */
					result = 0;
					return frameOut;
				...
				// SPS PPS
					 ...
					 * Log.v("ENCODED BY MANIK SPS", String.format("0x%20x",
					 * sps[index])); }
					 */
					 ...
					 * Log.v("ENCODED BY MANIK PPS", String.format("0x%20x",
					 * pps[index])); }
					 */
					System.arraycopy(outData, ppsIndex, pps, 0, pps.length);
