...
public final class ContactsListManager {
    ...
    private static final String[] SELECTION_ARGS = { MIMETYPE_RCS_CONTACT,
    ...
    public String getDisplayNameByPhoneNumber(String matchString) {
        Logger.d(TAG,
                "getDisplayNameByPhoneNumber() entry,with matchString is "
                        + matchString + " and CONTACTS_LIST " + CONTACTS_LIST);
    ...
    public short getContactIdByNumber(String number) {
        Logger.d(TAG, "getContactIdByNumber() entry with the phone number "
                + number + " and CONTACTS_LIST " + CONTACTS_LIST);
        if (number == null) {
    ...
    public long[] getPhoneIdTobeShow(List<Participant> participantList) {
        Logger.d(TAG, "getPhoneIdTobeShow() entry the participantlist is "
                + participantList + " CONTACTS_LIST is" + CONTACTS_LIST);
    ...
    public boolean isLocalContact(String contact) {
        Logger.d(TAG, "isLocalContact() entry with contact is" + contact
                + " and CONTACTS_LIST " + CONTACTS_LIST);
        if (contact != null) {
            for (RcsContact rcsContact : CONTACTS_LIST) {
    ...
    /**
     * Gets the contact list vodafone.
     *
     * @param contacts the contacts
     * @return the contact list vodafone
     */
    private void getContactListVodafone(List<RcsContact> contacts) {
        ...
        if (contacts != null && contacts.size() != 0) {
            for (RcsContact contact : contacts) {
                ...
                Logger.d(TAG,
                        "getContactListVodafone, a new contact, need to add"
                                + contact.mNumber);
                if (!CONTACTS_LIST.contains(contact)) {
                    CONTACTS_LIST.add(contact);
                }
                Logger.d(TAG, "getContactListVodafone, CONTACTS_LIST is "
                        + CONTACTS_LIST);
    ...
    public void onContactsDbChange() {
        ...
        if (cursor != null && cursor.getCount() != 0) {
            ...
            do {
                ...
                Logger.v(TAG, "onLoadComplete(), number  = " + number
                        + " phoneId = " + phoneId + " contactId = " + contactId);
                Logger.v(TAG, "onLoadComplete(), sortKeyString = "
                        + sortKeyString);
