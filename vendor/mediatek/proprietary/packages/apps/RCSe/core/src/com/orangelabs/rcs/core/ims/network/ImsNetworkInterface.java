...
import com.mediatek.rcse.ims.rcsua.RcsProxyRegistrationHandler;
...
public abstract class ImsNetworkInterface {
    ...
    public boolean volteRegister(int currentPCscfAddress){
    	...
    	if(!RcsUaAdapter.getInstance().isSingleRegistrationSupported()){
    	 return false;
    	}
		...
		try {
			...
			regStatus =  ((RcsProxyRegistrationHandler)RcsUaAdapter.getInstance().getRegistrationEventDispatcher()).register();	
