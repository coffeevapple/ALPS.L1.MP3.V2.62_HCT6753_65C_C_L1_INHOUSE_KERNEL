...
public class VoiceCommandSelect extends PreferenceActivity implements OnClickListener {
    ...
    protected boolean isValidFragment(String fragmentName) {
        if (VoiceCommandSelectFragment.class.getName().equals(fragmentName)) {
            return true;
        }
        return false;
    }
    ...
    public static class VoiceCommandSelectFragment extends SettingsPreferenceFragment implements
        ...
        private void initDefaultApp() {
            ...
            if (addPhoneApps) {
                TelephonyManager telephony = (TelephonyManager) getActivity().getSystemService(
        ...
        class VoiceCommandAdapter extends ArrayAdapter<CommandApp> {
            ...
            public View getView(int position, View convertView, ViewGroup parent) {
                ...
                if (convertView == null) {
                    ...
                    holder.mPrimary = (TextView) convertView.findViewById(R.id.primary);
                    holder.mSecondary = (TextView) convertView.findViewById(R.id.secondary);
                    holder.mRadio = (RadioButton) convertView.findViewById(R.id.radio);
                ...
                if (app.mName != null) {
                    String primary = mContext.getResources().getString(
                            R.string.voice_command_primary, app.mName);
                    String secondary = mContext.getResources().getString(
                            R.string.voice_command_secondary, app.mName);
                   ...
                   * if (mCustomApp != null) { String secondary =
                   * mContext.getResources
                   * ().getString(R.string.voice_command_secondary,
                   * mCustomApp.mName);
                   * holder.mSecondary.setVisibility(View.VISIBLE);
                   * holder.mSecondary.setText(secondary); } else {
                   * holder.mSecondary.setVisibility(View.GONE); } }
                   */

                if (mChecked != -1) {
                    log("mChecked=" + mChecked + " position=" + " position + getItemId(position)="
                            + getItemId(position));
                    if (getItemId(position) == mChecked) {
                        holder.mRadio.setChecked(true);
                        // if (mCustomApp != null) {
                        // mCommandVaule =
                        // mCustomApp.mComponentName.flattenToShortString();
                        // mCustomApp = null;
                        // } else {
                        log("getItem(position)=" + getItem(position)
                                + " getItem(position).mComponentName = "
                                + getItem(position).mComponentName);
        ...
        // if (requestCode == RESULT_LAUNCH_APP && resultCode ==
        // Activity.RESULT_OK) {
        // ComponentName cn =
        // ComponentName.unflattenFromString(data.getAction());
        // mCustomApp = new CommandApp(getAppLabel(cn), cn);
        // mChecked = mAdapter.getCount() -1;
        // log("after choose app mChecked = " + mChecked + "mCustomApp = " +
        // mCustomApp.toString());
        // mAdapter.notifyDataSetChanged();
        // }
        // }
