...
//        setContentView(R.layout.main);
    ...
    protected void onResume() {
        ...
        if (true == SystemProperties.get("ro.mtk_gemini_4sim_support").equals("1")) {
        ...
        } else if (false == SystemProperties.get("ro.mtk_gemini_support").equals("1")) { //single card
            bSIM1Inserted = hasIccCard(0);
            ...
            if (SystemProperties.get("ro.evdo_dt_via_support").equals("1")) {
                int i = 0;
                for (i = 0; i < 2; i++)
                {
    ...
    public static boolean hasIccCard(int slot) {
        ...
        if (true == SystemProperties.get("ro.mtk_gemini_support").equals("1")) {
            try {
                final ITelephonyEx iTelephony = ITelephonyEx.Stub.asInterface(ServiceManager.getService(Context.TELEPHONY_SERVICE_EX));
                if (null != iTelephony) {
