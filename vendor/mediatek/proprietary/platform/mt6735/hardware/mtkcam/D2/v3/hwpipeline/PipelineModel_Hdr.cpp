...
public:     ////                    Operations.
    ...
    virtual MERROR                  createHdrCaptureParams() const;
...
//    MERROR                          createSubRequest(AppRequest& request, Vector< sp<HdrRequest> > &vSubRequest);
...
{
    MY_LOGD("register inflight request!");
...
#endif
    //
    return PipelineModelBaseHdr::submitRequest(request);
...
MERROR
PipelineHdrImp::
createSubRequest(AppRequest& request, Vector< sp<HdrRequest> > &vSubRequest)
{
    MUINT32 iSubReqCnt = 3;
    for ( int i=0; i<iSubReqCnt; i++ ) {
        vSubRequest[i]->frameNo                 = request.frameNo;
        //vSubRequest[i]->PipelineModelCallback   = this;
...
)   const
{
    ...
    switch  (nodeId)
    {
        case eNODEID_P1Node:
            ...
            break;
            //
        case eNODEID_P2Node://eNODEID_HdrNode:
            err = evaluateIOStreamsLocked_p2node(rParams);
            //err = evaluateIOStreamsLocked_hdrnode(rParams);
            break;
            //
...
)
{
    ...
    mConfigProfile.mbConfigHdr   = MFALSE;
...
#define addBufPlane(planes, height, stride)                                      \
    ...
    switch( imgFormat ) {
        case eImgFmt_BAYER10:
            addBufPlane(bufPlanes , imgSize.h      , ALIGN_2((imgSize.w * 10 + 7)>> 3));
            break;
        case eImgFmt_YV12:
