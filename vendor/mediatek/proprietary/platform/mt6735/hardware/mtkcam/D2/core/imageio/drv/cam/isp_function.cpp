...
//TEST 0418               ISP_WRITE_REG(m_pPhyIspReg, CAM_IMGI_CON2, 0x00000000);// TEST_MDP by m_pPhyIspReg
            ...
            ISP_FUNC_INF("mdpCropXReg(0x%08x),mdpCropyReg(0x%08x)",mdpCropXReg,mdpCropYReg);
            if(m_pIspDrv->IsReadOnlyMode())
            {
...
int CAM_TDRI_PIPE::_config( void )
{
    ...
    if ( this->enTdri )
    {
        ...
        else // Add
        {
            ...
            ISP_FUNC_INF("g_MdpMgrCqIdx(%d),top.mdp_crop_en/rsp_en(%d/%d),tpipe_irq_mode(%d)",g_MdpMgrCqIdx,\
