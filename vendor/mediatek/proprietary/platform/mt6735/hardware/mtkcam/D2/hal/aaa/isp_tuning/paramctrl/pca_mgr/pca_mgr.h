    ...
    loadLut(MBOOL const fgForceToLoad = MFALSE)
    {
        if  (isChanged() || fgForceToLoad)
        {
            ...
            case LUT_HIGH:      //color temperature: high
                ...
                break;
            case LUT_LOW_3:       //color temperature: low
                ISP_MGR_PCA_T::getInstance(m_eSensorDev).loadLut(reinterpret_cast<MUINT32*>(&m_rIspPcaLUTs.lut_lo3[0]));
                break;
            case LUT_MIDDLE_3:    //color temperature: middle
                ISP_MGR_PCA_T::getInstance(m_eSensorDev).loadLut(reinterpret_cast<MUINT32*>(&m_rIspPcaLUTs.lut_md3[0]));
                break;
            case LUT_HIGH_3:      //color temperature: high
                ISP_MGR_PCA_T::getInstance(m_eSensorDev).loadLut(reinterpret_cast<MUINT32*>(&m_rIspPcaLUTs.lut_hi3[0]));
                break;                
            default:
                break;
            }
