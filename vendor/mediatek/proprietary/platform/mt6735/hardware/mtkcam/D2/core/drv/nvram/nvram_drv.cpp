	                      ...
	                      int version)
{
    ...
    if(a_eNvramDataType==CAMERA_DATA_AE_PLINETABLE || a_eNvramDataType==CAMERA_DATA_TSF_TABLE)
    {
...
//    static bool bCustomInit = 0; //[ALPS00424402] [CCT6589] Len shading page --> Save to NVRAM --> CCT reboot failed
    ...
    case CAMERA_NVRAM_DATA_STROBE:
        int sz;
        int ret;
		ret = cust_getDefaultStrobeNVRam_V2(a_eSensorType, a_pNvramData, &sz);
        break;
    ...
    case CAMERA_OEM_COMMON:
        NVRAM_DRV_LOG("CAMERA_OEM_COMMON %d",__LINE__);
        if (0 != fillDefaultOemComm((OEM_CAMERA_COMMON*)a_pNvramData))
        {
            return NVRAM_DEFAULT_DATA_READ_ERROR;
        }
        break;
    case CAMERA_OEM_SENSOR_LAYOUT:
        NVRAM_DRV_LOG("CAMERA_OEM_SENSOR_LAYOUT %d",__LINE__);
        if (0 != fillDefaultOemCamLayout(a_eSensorType, (OEM_CAMERA_SENSOR_LAYOUT*)a_pNvramData))
        {
            return NVRAM_DEFAULT_DATA_READ_ERROR;
        }
        break;
    case CAMERA_OEM_PARTS_LIST:
        NVRAM_DRV_LOG("CAMERA_OEM_PARTS_LIST %d",__LINE__);
        if (0 != fillDefaultOemCameraPartsList((OEM_CAMERA_PARTS_LIST*)a_pNvramData))
        {
            return NVRAM_DEFAULT_DATA_READ_ERROR;
        }
        break;
    case CAMERA_OEM_FEATURE:
        NVRAM_DRV_LOG("CAMERA_OEM_FEATURE %d",__LINE__);
        if (0 != fillDefaultOemFeature(a_eSensorType, (OEM_CAMERA_FEATURE*)a_pNvramData))
        {
            return NVRAM_DEFAULT_DATA_READ_ERROR;
        }
        break;
