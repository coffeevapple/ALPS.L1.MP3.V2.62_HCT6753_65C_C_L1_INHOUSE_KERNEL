...
sendIntent(intent2type<eIntent_AFUpdate>)
{
    ...
    MY_LOG_IF(sm_3APvLogEnable,"[StateRecording::sendIntent<eIntent_AFUpdate>] deQHwBufAFO done\n");
    ...
    MY_LOG_IF(sm_3APvLogEnable,"[StateRecording::sendIntent<eIntent_AFUpdate>] doAF done\n");

    // Enqueue AFO DMA buffer
    BufMgr::getInstance().enqueueHwBuf(ECamDMA_AFO, rBufInfo);
    MY_LOG_IF(sm_3APvLogEnable,"[StateRecording::sendIntent<eIntent_AFUpdate>] enQHwBuf done\n");
