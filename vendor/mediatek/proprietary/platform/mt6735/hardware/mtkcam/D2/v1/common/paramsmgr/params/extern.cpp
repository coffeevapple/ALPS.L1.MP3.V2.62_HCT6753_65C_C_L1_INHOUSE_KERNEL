...
querySensorInfo(int32_t const i4OpenId, String8& rs8SensorName, uint32_t& ru4SensorType, int32_t& ri4SensorFacing)
{
    ...
    bool ret = false;
    uint32_t halSensorDev = DevMetaInfo::queryHalSensorDev(i4OpenId);
    ...
    pSensorHal = pHalSensorList->createSensor(
                                    LOG_TAG,
                                    i4OpenId);
    ri4SensorFacing = pHalSensorList->queryFacingDirection(i4OpenId);
    if(pSensorHal == NULL)
    {
    ...
    //
    if  ( 0 != pSensorHal->sendCommand(halSensorDev, NSCam::SENSOR_CMD_GET_SENSOR_FEATURE_INFO, (MUINTPTR)&pSensorInfo, 0, 0) || ! pSensorInfo )
    {
