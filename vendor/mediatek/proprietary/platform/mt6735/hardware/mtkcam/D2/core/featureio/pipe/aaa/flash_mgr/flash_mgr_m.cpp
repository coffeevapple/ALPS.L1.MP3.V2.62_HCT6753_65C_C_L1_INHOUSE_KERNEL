...
#define logI(fmt, arg...) ALOGD(LOG_TAG " "fmt, ##arg) //XLOGD(fmt, ##arg)
...
*/
    ...
    rAEInfo2ISP.u4IspGain = isp;
    rAEInfo2ISP.u4RealISOValue = iso;
    IspTuningMgr::getInstance().setAEInfo(sensorType, rAEInfo2ISP);
...
int FlashMgrM::init(int sensorId)
{
	...
	pthread_create(&mThread, NULL, onSetTorchThread, this);
     ...
     logI("FlashMgrM::init(int sensorId) bCheckCaliResult=%d",bCheckCaliResult);
