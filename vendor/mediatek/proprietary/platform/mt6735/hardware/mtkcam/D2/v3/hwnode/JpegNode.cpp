...
)
{
        ...
        if( ret == NAME_NOT_FOUND ) {
            ...
            if( pEncodeFrame ) {
                ...
                if( err != OK ) {
                    ...
                    errorMetaHandle(pEncodeFrame, pInMetaStream_Request, pInMeta_Request);
                    return;
                }
    ...
    while( pEncodeFrame->mpJpeg_Main.get() == NULL ) {
        //[++]get out buffer and lock
        {
            ...
            if( err == NAME_NOT_FOUND ) {
                ...
                //errorHandle(pEncodeFrame, pOutImgStreamBuffer, pOutImageBuffer);
                return;
            }
            //
    ...
    while( pEncodeFrame->mbHasThumbnail && pEncodeFrame->mpJpeg_Thumbnail.get() == NULL ) {
        ...
        if( err == NAME_NOT_FOUND ) {
            ...
            //if(pInImageStreamBuffer.get())
            //    pInImageStreamBuffer->unlock(getNodeName(), pInImageBuffer->getImageBufferHeap());
            //errorHandle(pEncodeFrame, pEncodeFrame->mpOutImgStreamBuffer, pEncodeFrame->mpJpeg_Thumbnail);
            return;
        }
        //
...
#endif
    //
    ret = rpEncodeFrame->exif.make(rOutExifSize);
    //
lbExit:
    returnMetadataAndUnlock(
...
{   FUNC_START;
    ...
    // in metadata
    {
        /*IMetadata* pInMeta_Result = NULL;
        sp<IMetaStreamBuffer> pInMetaStream_Result = NULL;

        MERROR const err = getMetadataAndLock(
                rpEncodeFrame->mpFrame,
                mpInAppMeta->getStreamId(),
                pInMetaStream_Result,
                pInMeta_Result
                );
        ...
        if(pInMetaStream_Result.get())
            pInMetaStream_Result->markStatus(STREAM_BUFFER_STATUS::ERROR);*/
    ...
    {//in
        ...
        if(pInImageStreamBuffer.get())
        {
            ...
            pInImageStreamBuffer->markStatus(STREAM_BUFFER_STATUS::ERROR);
        }
    ...
    {
        rpStreamBuffer->unlock(getNodeName(), rpEncodeFrame->mpOutImgBufferHeap.get());
        rpStreamBuffer->markStatus(STREAM_BUFFER_STATUS::ERROR);
        streamBufferSet.markUserStatus(
