...
private:
    ...
    enum
    {
        ERegInfo_CAM_CTL_IHDR,        
        ERegInfo_NUM
    };

    enum
    {        
        ERegInfo_CAM_CTL_IHDR_D,        
    };
...
protected:
    ISP_MGR_RMG(ESensorDev_T const eSensorDev, ESensorTG_T const eSensorTG)
        ...
        , m_u4StartAddr((eSensorTG == ESensorTG_1) ? REG_ADDR(CAM_CTL_IHDR) : REG_ADDR(CAM_CTL_IHDR_D))
        , m_bEnable(MFALSE)
        , mLeFirst(0)
        , mAeExpRatio(1.0)        
    {
        ...
        {
            INIT_REG_INFO_ADDR(CAM_CTL_IHDR);        // 0x4104
            INIT_REG_INFO_VALUE(CAM_CTL_IHDR,0x40);
        }
        else
        {
            INIT_REG_INFO_ADDR(CAM_CTL_IHDR_D);      // 0x4108
            INIT_REG_INFO_VALUE(CAM_CTL_IHDR_D,0x40);
        }
    }
