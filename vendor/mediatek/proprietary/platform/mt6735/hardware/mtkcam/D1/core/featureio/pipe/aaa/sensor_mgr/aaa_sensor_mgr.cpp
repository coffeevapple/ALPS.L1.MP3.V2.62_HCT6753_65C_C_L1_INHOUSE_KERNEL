...
MRESULT AAASensorMgr::setSensorIso(MINT32 i4SensorDev, int sensorMode, int iso)
{
    MINT32 ret = S_AAA_SENSOR_MGR_OK;
    ...
    logI("[setSensorIso] ln=%d Id:%d iso=%d\n", __LINE__, i4SensorDev,  iso);

    MY_LOG("[setSensorIso] Id:%d iso=%d\n", i4SensorDev,
        iso);
    ...
    if(i4SensorDev == ESensorDev_Main)
    {
        ret = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_MAIN, SENSOR_CMD_SET_SENSOR_ISO, (MINTPTR)data, 0, 0);
    }
    else if(i4SensorDev == ESensorDev_Sub)
    {
        ret = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_SUB, SENSOR_CMD_SET_SENSOR_ISO, (MINTPTR)data, 0, 0);
    }
    else if(i4SensorDev == ESensorDev_MainSecond)
    {
        ret = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_MAIN_2, SENSOR_CMD_SET_SENSOR_ISO, (MINTPTR)data, 0, 0);
    }
    else
    {
        MY_ERR("[%s()] Sensor type error:%d", __FUNCTION__, i4SensorDev);
    ...
    if(ret)
    {
        MY_ERR("Err setSensorIso, ln=%d dev=%d mode=%d iso=%d\n", __LINE__, i4SensorDev,
        sensorMode, iso);
    }
    return (ret);    
}
