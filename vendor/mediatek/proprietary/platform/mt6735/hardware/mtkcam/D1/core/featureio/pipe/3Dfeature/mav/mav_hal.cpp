...
halMAV::mHal3dfDoMotion(void* InputData,MUINT32* MotionResult, MUINT32 u4SrcImgWidth, MUINT32 u4SrcImgHeight)
{
    ...
    CAM_LOGD("[mHalMAVDoMotion] ImgAddr %p\n", MotionInfo.ImgAddr);
...
)
{
    ...
    else        
    {
        ...
        for(int i=0;i<ImgNum;i++)
        {
            ...
            CAM_LOGD("[mHalMavWarp] ImgAddr %p ImgNum %d Width %d Height %d ClipWidth %d ClipHeight %d",MyImageInfo.ImgAddr[i],MyImageInfo.ImgNum,MyImageInfo.Width,MyImageInfo.Height,MyImageInfo.ClipWidth,MyImageInfo.ClipHeight);    
