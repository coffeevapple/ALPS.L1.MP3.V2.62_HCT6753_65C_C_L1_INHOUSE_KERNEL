...
#include <mtkcam/D1/core/featureio/pipe/aaa/aaa_hal.h>
#include <mtkcam/D1/core/featureio/pipe/aaa/state_mgr/aaa_state.h>
#include <mtkcam/D1/core/featureio/pipe/aaa/state_mgr/aaa_state_mgr.h>
...
sendPrecapIntent(intent2type<eIntent_VsyncUpdate>, state2type<ePrecap_AF_BeforeFlash>)
{
    ...
    if(CUST_ENABLE_PRECAPTURE_AF() && !CUST_PRECAPTURE_AF_AFTER_PREFLASH())
    {
        ...
        {
            XLOGD("[sendPrecapIntent]isDoAF=0 || no flash hw || isSupportAF=0");
        }
    }
    else
    {
