...
Hal3A::sendCommand(ECmd_T const r3ACmd, MINTPTR const i4Arg)
{
    ...
    if (r3ACmd == ECmd_Update)
    {
        ...
        {
            Mutex::Autolock lock(m_DropLock);
            m_i4DropFrameCnt = 2;
        }
    }
            ...
            case ECmd_Update:
                     ...
                     frameIdx++)
                {
                    ...
                    {   
                        ...
                        if (/*frameIdx != 1*/!bIsFirstOfCmdSet || _this->mbInCapturing) 
                        {
...
Hal3A::AESensorThreadLoop(MVOID *arg)
{
    ...
    while(1)
	{
        ...
        // m_i4DropFrameCnt > 0 : drop 1 Vsync, because 3A handles cmd before Vsync.
        {
            MY_LOG_IF(_this->m_3ALogEnable & EN_3A_FLOW_LOG, "m_i4DropFrameCnt = %d", _this->m_i4DropFrameCnt);
            Mutex::Autolock lock(_this->m_DropLock);
            if (--_this->m_i4DropFrameCnt < 0) { _this->m_i4DropFrameCnt = 0; }
        }
        
        if(_this->m_i4DropFrameCnt == 0 || _this->mbInCapturing)
        {
        ...
        if (!bRet) 
        {
            ...
            MY_LOG("Timeout : sem_wait_relativeTime(semAEI2CSen).");
            continue;
        }
