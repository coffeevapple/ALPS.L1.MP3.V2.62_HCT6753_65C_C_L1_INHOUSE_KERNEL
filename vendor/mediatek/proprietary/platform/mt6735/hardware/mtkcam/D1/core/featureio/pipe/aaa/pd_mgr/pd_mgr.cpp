...
//
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MRESULT PDMgr::SetCaliData2PDCore(MVOID* ipCaliData)
{
	MY_LOG("[Core] set calibration data flow");

	MRESULT ret=E_3A_ERR;

	PD_NVRAM_T *ptrCaliData = reinterpret_cast<PD_NVRAM_T *>(ipCaliData);
	...
	if( ret==S_3A_OK)
	{
	   ...
	   if(pIHalSensor) pIHalSensor->destroyInstance("pd_mgr");
		...
		{
			memset( m_i4PDInfo2HybirdAF, 0, sizeof(MINT32)*10);
			m_pIPdAlgo->getInfoForHybridAF( m_i4PDInfo2HybirdAF);
			MY_LOG("[Core] configure PD algo done %d %d\n", m_i4PDInfo2HybirdAF[0], m_i4PDInfo2HybirdAF[1]);
		}		
	}
	else
	{
...
//
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MRESULT PDMgr::SetCaliData2PDOpenCore(MVOID* ipCaliData)
{
	MY_LOG("[Open] Set calibration data flow");

	MUINT8 *ptrData = reinterpret_cast<MUINT8 *>(ipCaliData);
...
MRESULT PDMgr::PDOpenCoreFlow()
{
	...
	m_PD_mgr_open->SetDataBuf( m_databuf_size, m_databuf);
...
//
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MRESULT PDMgr::setPDCaliData(MVOID *ipCaliData)
{	
	MRESULT ret=E_3A_ERR;
	...
	if( m_bEnPDBufThd)
	{
		...
		switch( m_profile.BufType)
		{
			case EPDBuf_VC :
			case EPDBuf_Raw :
				ret = SetCaliData2PDCore( ipCaliData);
				break;
				
			case EPDBuf_VC_Open :
			case EPDBuf_Raw_Open :
				ret = SetCaliData2PDOpenCore( ipCaliData);
				break;
