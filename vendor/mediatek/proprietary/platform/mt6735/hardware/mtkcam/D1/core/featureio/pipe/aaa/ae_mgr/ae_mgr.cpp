...
#include "ae_mgr_if.h"
#include "ae_mgr.h"
#include <mtkcam/D1/core/featureio/pipe/aaa/aaa_scheduler.h>
...
//    MY_LOG("%s() : AEMode:%d AEState:%d i4ActiveAEItem:%d u4AAOUpdate:%d\n", __FUNCTION__, m_eAEMode, m_eAEState, i4ActiveAEItem, u4AAOUpdate);
    ...
    if(m_bAdbAEPerFrameCtrl) {
        ...
        MY_LOG("[m_bAdbAEPerFrameCtrl] willupdate = %d", bWillUpdateSensorbyI2C );
        return S_AE_OK;
    }
...
void AeMgr::setAfe(int afe)
{
	...
	AAASensorMgr::getInstance().setSensorIso((ESensorDev_T)m_eSensorDev, m_eSensorMode, m_rAEOutput.rPreviewMode.u4RealISO);
}
...
AE_EASYTUNING_PLINE_INFO_T rEasytuningTable;
    ...
    if(m_pIAeAlgo != NULL) {
        ...
        if((m_eZoomWinInfo.u4XOffset != 0) && (m_eZoomWinInfo.u4YOffset != 0)){
            AEMeteringArea_T sAEMeterAreaTemp;
            memset(&sAEMeterAreaTemp, 0, sizeof(AEMeteringArea_T));
            sAEMeterAreaTemp.u4Count = 1;
            m_pIAeAlgo->setAEMeteringArea(&m_eAEMeterAreaForDPwhileZoom);
            m_pIAeAlgo->getDebugInfo(rAEDebugInfo);
            m_pIAeAlgo->setAEMeteringArea(&sAEMeterAreaTemp);
        } else {
            m_pIAeAlgo->getDebugInfo(rAEDebugInfo);
        }
...
//                        g_rAEOutput.rPreviewMode.u4IspGain);
        ...
        case AE_CAPTURE_STATE:
            ...
            if( (m_bEnableAE == MTRUE) && (m_bMultiCap == MFALSE) &&  (mCaptureMode.u4Eposuretime > m_rAEInitInput.rAEPARAM.strAEParasetting.u4LongCaptureThres) && (m_rAEInitInput.rAEPARAM.strAEParasetting.bEnableLongCaptureCtrl == MTRUE)) {
                ...
                AAASensorMgr::getInstance().setSensorIso((ESensorDev_T)m_eSensorDev, m_eSensorMode, m_rAEOutput.rCaptureMode[0].u4RealISO);
