...
#if TEST_ALGORITHM
#include <mtkcam/D1/core/featureio/pipe/mfll/mfll_hal.h>
...
flowRawToYuy2()
{
    ...
    for(MUINT32 i=0; i < mReferenceFrameLength+1; i++) {
        ...
        MY_LOGD("enqueBuf[%d]:%p to CapBufMgr:", i, (void *)mMemoryCapBufRaw[i]); 
        mpCapBufMgr->enqueBuf(mMemoryCapBufRaw[i]);
        mMemoryCapBufRaw[i] = NULL;
...
doMixing()
{
    ...
    minput_imgi.mPortID.type = NSCam::NSIoPipe::EPortType_Memory;
    params.mvIn.push_back(minput_imgi);
    MY_LOGD("minput_imgi.mBuffer = {VA=%p, size=%zu}", (void*)mMemoryYuv->getBufVA(0), mMemoryYuv->getBufSizeInBytes(0));
    ...
    minput_vipi.mPortID.type = NSCam::NSIoPipe::EPortType_Memory;
    params.mvIn.push_back(minput_vipi);
    MY_LOGD("minput_vipi.mBuffer = {VA=%p, size=%zu}", (void*)mMemoryYuvB->getBufVA(0), mMemoryYuvB->getBufSizeInBytes(0));
