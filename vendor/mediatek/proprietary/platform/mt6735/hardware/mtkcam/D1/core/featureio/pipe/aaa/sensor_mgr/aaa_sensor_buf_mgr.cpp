...
#include <mtkcam/D1/core/iopipe/CamIO/PortMap.h>
...
dequeueHwBuf(BufInfo_T& rBufInfo, MINT32 i4PDSel, MUINT32 i4FrmNum)
{
	...
	if( m_bDebugEnable)
	{
		...
		if( i4PDSel==_PD_BUF_SEL_ && m_EnPDAF)
		{
			...
			MY_LOG_IF(m_bDebugEnable, "(0x980C) CAMSV_CAMSV2_INT_STATUS : 0x%0x\n", ISP_REG(pisp, CAMSV_CAMSV2_INT_STATUS));
...
AEBufMgr::newBuf()
{
	...
	MY_LOG("SensorID 0x%x, PDAF_Support mode %d, Senraio %d", m_i4CurrSensorId, rSensorStaticInfo.PDAF_Support, i4SensorSenraio);
	...
	switch( m_i4CurrSensorId)
	{
		...
		case IMX230_SENSOR_ID :
			m_EnPDAF = MTRUE;
 			PDXSize = rSensorVCInfo.VC2_SIZEH-1;
 			PDYSize = rSensorVCInfo.VC2_SIZEV-1;
 
			m_EnMVHDR = MTRUE;
 			MVHDRXSize = rSensorVCInfo.VC1_SIZEH-1;
 			MVHDRYSize = rSensorVCInfo.VC1_SIZEV-1;
		break;
