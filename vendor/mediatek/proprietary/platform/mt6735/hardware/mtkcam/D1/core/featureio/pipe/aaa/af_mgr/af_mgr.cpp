...
MRESULT AfMgr::init(MINT32 i4SensorIdx, MINT32 isInitMCU)
{
    ...
    { 
        ...
        if(pIHalSensor) pIHalSensor->destroyInstance("PDAF-af_mgr");
			...
			ret = IPDMgr::getInstance().setPDCaliData( m_i4CurrSensorDev, reinterpret_cast<MVOID *>(&initPdData));
		...
		else if( m_PDBuf_Type==EPDBuf_VC_Open)
		{
			...
			ret = IPDMgr::getInstance().setPDCaliData( m_i4CurrSensorDev, reinterpret_cast<MVOID *>(m_DccData));
		}
	...
	if( m_PDAF_Sensor_Support_Mode==2 && m_PDBuf_Type==EPDBuf_VC_Open)
        {
		SPDLibVersion_t PdLibVersion;
		//if(m_PD_mgr!=NULL) m_PD_mgr->GetVersionOfPdafLibrary( PdLibVersion);
        }
...
MRESULT AfMgr::doAF(MVOID *pAFStatBuf)
{
	...
	if( 0<=m_sAFInput.sLensInfo.i4CurrentPos && m_sAFInput.sLensInfo.i4CurrentPos<=1023)
	{
		m_pdaf_pure_lens_pos_pre = m_pdaf_pure_lens_pos;
    	m_pdaf_pure_lens_pos = m_sAFInput.sLensInfo.i4CurrentPos;
	}
...
MRESULT AfMgr::doPDTask()
{
...

	{
		...
		if( m_pdaf_rawbuf)
		{
			...
			if( m_pdaf_raw_frmnum!=0)
			{
				...
				{  
					...
					{  
						...
						m_pdaf_pure_lens_pos_pre = itor->i4LensPos;
						seachDone = TRUE;
						break;
					}  
					itor--;  
			...
			PDInput.curLensPos	 = m_pdaf_pure_lens_pos_pre;
...
MVOID AfMgr::VsyncSignal()
{
	...
	static MINT32 pure_pos=0;
	static MINT32 cur_pos=0;	
	 ...
	 MY_LOG_IF( m_bDebugEnable, "[DAF] DafIO in :[%d] [W]%d [H]%d [X]%d [Y]%d\n", 
		...
		if( m_AAOPDBuf)
		{
			...
			if( ret==MTRUE)
			{
				...
				PDInput.frmNum = m_cur_frm_num;
				PDInput.curLensPos	 = cur_pos;
				PDInput.XSizeOfImage = m_i4AF_in_Hsize;
				PDInput.YSizeOfImage = m_i4AF_in_Vsize;
				for( MUINT32 i=0; i<PDInput.numROI; i++)
				{
