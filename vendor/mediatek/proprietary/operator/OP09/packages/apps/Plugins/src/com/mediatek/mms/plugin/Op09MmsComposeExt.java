...
/**
 * M: OP09 Mms compose implemention.
 */
@PluginImpl(interfaceName = "com.mediatek.mms.ext.IMmsComposeExt")
public class Op09MmsComposeExt extends DefaultMmsComposeExt {
    ...
    private boolean mForSlideshow = false;
    private boolean mIsShowDualButtonPanel = true;
    private boolean mIsSubjectHide = false;
    ...
    @Override
    public boolean deleteMassTextMsg(AsyncQueryHandler backQueryHandler, long msgId,
            long timeStamp) {
        Xlog.d(TAG, "deleteMassTextMag: msgId:" + msgId + " timeStamp:" + timeStamp);
        if (timeStamp >= 0) {
            return false;
        }
    ...
    public boolean showMassTextMsgDetail(final Context context, long timeStamp) {
        Xlog.d(TAG, "showMassTextMsgDetail:" + timeStamp);
        ...
        try {
            ...
            while (cursor.moveToNext()) {
                ...
                switch (type) {
                    ...
                    case Sms.MESSAGE_TYPE_QUEUED:
                        status = status
                            + this.getResources().getString(R.string.msg_status_sending);
                        break;
                    default:
                        break;
                }
                ...
                public final void onClick(DialogInterface dialog, int which) {
                    ...
                    if (msgIDS != null && msgIDS.length > 0) {
                        for (int index = 0; index < msgIDS.length; index++) {
                            Xlog.d(TAG, "msgIDS:" + msgIDS[index][0]);
                            if (msgIDS[index][0] > 0) {
                                Uri resendUri = ContentUris.withAppendedId(Sms.CONTENT_URI,
                                    msgIDS[index][0]);
                                Xlog.d(TAG, "Resend SMS Uri:" + resendUri + " SMs MsgID:"
                                    + msgIDS[index][1]);
            ...
            if (hasFailed) {
                showMassTextMsgDetails(context, datas, clickListner, sendButtonStr, hasFailed);
            } else {
                showMassTextMsgDetails(context, datas, null, sendButtonStr, hasFailed);
            }
        } catch (SQLiteException e) {
            Xlog.e(TAG, "query mass text message's details failed.", e);
        } finally {
            if (cursor != null) {
                cursor.close();
            }
        ...
        public boolean onMenuItemClick(MenuItem item) {
            switch (item.getItemId()) {
                case MENU_SPLIT_MESSAGE:
                    Xlog.d(TAG,
                        "ContextMenuClickListener: click MENU_SPLIT_MESSAGE, messageGroupId:"
                            + mMessageGroupId);
    ...
    public void splitSingleMessage(final Context context, final long messageGroupId) {
        if (messageGroupId >= 0) {
            Xlog.d(TAG, "splitSingleMessage failed: messageGroupId >= 0; messageGroupId = "
                + messageGroupId);
            return;
        }
        if (context == null) {
            Xlog.d(TAG, "splitSingleMessage failed: context == null;");
            return;
        }
        new Thread(new Runnable() {
            @Override
            public void run() {
                ...
                Cursor cursor = SqliteWrapper.query(context, context.getContentResolver(),
                    Sms.CONTENT_URI, new String[] {Sms._ID, Sms.ADDRESS, Sms.THREAD_ID,
                        Sms.IPMSG_ID}, Sms.IPMSG_ID + " = ? ", new String[] {messageGroupId + ""},
                    null);
                long threadId = 0;
                if (cursor != null) {
                    try {
                        ...
                        while (cursor.moveToNext()) {
                            ...
                            SqliteWrapper.update(context, context.getContentResolver(), ContentUris
                                    .withAppendedId(Sms.CONTENT_URI, smsId), cv, null, null);
                        }
    ...
    private void closeActivity(Context context, long threadId) {
        ...
        try {
            if (conCursor != null) {
                ...
                Xlog.d(TAG, "closeActivity, msgCount: " + msgCount);
    ...
    private void splitThreadMassingTextMessage(final Context context, final long threadId) {
        if (threadId <= 0) {
            Xlog.d(TAG, "splitThreadMassingTextMessage failed: threadId <= 0; threadId = "
                + threadId);
            return;
        }
        if (context == null) {
            Xlog.d(TAG, "splitThreadMassingTextMessage failed: context == null;");
            return;
        }
        new Thread(new Runnable() {
            @Override
            public void run() {
                ...
                Cursor cursor = SqliteWrapper.query(context, context.getContentResolver(),
                    Sms.CONTENT_URI, new String[] {Sms._ID, Sms.ADDRESS, Sms.THREAD_ID,
                        Sms.IPMSG_ID}, Sms.THREAD_ID + " = ? ", new String[] {threadId + ""},
                        null);
                if (cursor != null) {
                    try {
                        while (cursor.moveToNext()) {
    ...
    @Override
    public void addSplitMessageContextMenu(ContextMenu menu, View v, ContextMenuInfo menuInfo,
            Activity activity, long messageGroupId, int messagesCount) {
        Xlog.i(TAG, "add Context enu");
        this.mMessageGroupId = messageGroupId;
        this.mContext = activity.getApplicationContext();
        this.mActivity = activity;

        menu.add(0, MENU_SPLIT_MESSAGE, 0, getString(R.string.split_message_apart))
    ...
    public void addSplitThreadOptionMenu(Menu menu, Activity activity, long threadId) {
        ...
        this.mActivity = activity;
        menu.add(0, MENU_SPLIT_THREAD, 0, this.getResources()
                .getString(R.string.split_thread_apart));
    }
    ...
    public void showDisableDRDialog(final Activity activity, final int subId) {
        Xlog.d(TAG, "showDisableDRDialog() subId = " + subId);
        ...
        final View contents = View.inflate(mPluginContext, R.layout.disable_delivery_report_dialog,
            null);
    ...
    @Override
    public int getSmsMessageAndSaveToSim(String[] numbers, String scAddress,
            ArrayList<String> messages, int smsStatus, long timeStamp, int subId,
            final int srcResult) {
        Xlog.d(TAG, "save sms message to sim");
        ...
        for (String number : numbers) {
            ...
            if (result == 0 || result == 7) {
                Xlog.d(TAG, "save sms message to sim successed: number:" + number);
                return result;
            }
        }
        return srcResult;
...
/// M: ---------------new OP09Plugin Refactory-------------------------
    ...
    @Override
    public void initDualSendButtonLayout(Activity activity, LinearLayout buttonWithCounter,
    ...
    @Override
    public void initDualSendBtnForDialogMode(Context context, LinearLayout buttonWithCounter,
            TextView newTextCounter) {
        mContext = context;
        if (buttonWithCounter == null) {
            return;
    ...
    @Override
    public void initDualSendBtnForAttachment(Context context, LinearLayout attachmentBtnLayout,
            Button sendButton, ViewOnClickListener listener) {
        Xlog.d(TAG, "initDualSendBtnForAttachment");
        mContext = context;
        ...
        int simCardSize = SubscriptionManager.from(context).getActiveSubscriptionInfoCount();
        Xlog.d(TAG, "initDualSendBtnForAttachment, simCardSize:" + simCardSize);
        if (simCardSize == 2) {
            mButtonSmall.setVisibility(View.VISIBLE);
    ...
    public void showDualButtonPanel() {
        if (!mIsShowDualButtonPanel) {
            setIsShowDualButtonPanel(true);
            return;
        }
        if (mButtonBig != null) {
            mButtonBig.setVisibility(View.VISIBLE);
    ...
    public void setDualSendButtonType(final ViewOnClickListener btnListener) {
        ...
        simCardSize = SubscriptionManager.from(mContext)
                .getActiveSubscriptionInfoCount();
        Xlog.d(TAG, "[setDualSendButtonType]-simCardSize:" + simCardSize);
        ...
        if (simCardSize == 0) {
            Xlog.d(TAG, "setDualSendButtonType Failed, as simCardSize = 0;");
            return;
        } else if (simCardSize == 1) {
            int defaultSubId = SubscriptionManager.from(mContext)
                    .getActiveSubscriptionIdList()[0];
            bigSendButtonBitmap = mmsUtilExt.createSendButtonBitMap(this,
                    bigSendButtonBackgroundRes, defaultSubId,
                    mButtonBig.isEnabled());
            ...
            if (!SubscriptionManager.isValidSubscriptionId(defaultSubId) ||
                    ...
                    defaultSubId) == null) {
                defaultSubId = SubscriptionManager.from(mContext).getActiveSubscriptionIdList()[0];
            }
            Xlog.d(TAG, "default SIM Sub Id:" + defaultSubId);
            bigSendButtonBitmap = mmsUtilExt.createSendButtonBitMap(this,
                    bigSendButtonBackgroundRes, defaultSubId,
                    mButtonBig.isEnabled());
            SubscriptionInfo defaultSimInfo = SubscriptionManager
                    .from(mContext).getActiveSubscriptionInfo(defaultSubId);
            int defaultSlotId = defaultSimInfo.getSimSlotIndex();
            if (defaultSlotId == 0) {
                secondSimInfo = SubscriptionManager.from(mContext)
            ...
            smallSendButtonBitmap = mmsUtilExt.createSendButtonBitMap(this,
                    smallSendButtonBackgroundRes, secondSubId,
                    mButtonBig.isEnabled());
        }
        View.OnClickListener buttonClickListener = new View.OnClickListener() {
            public void onClick(View v) {
                int sendSubIdForDualBtn = -1;
                int simCount = 0;
                int defaultSubId = -1;
                int secondSubId = -1;
                SubscriptionInfo secondSubInfo = null;
                simCount = SubscriptionManager.from(mContext)
                        .getActiveSubscriptionInfoCount();
                if (simCount == 0) {
                    return;
                } else if (simCount == 1) {
                    defaultSubId = SubscriptionManager.from(mContext)
                            .getActiveSubscriptionIdList()[0];
                } else if (simCount == 2) {
                    defaultSubId = SubscriptionManager.getDefaultSmsSubId();
                    if (!SubscriptionManager.isValidSubscriptionId(defaultSubId) ||
                            SubscriptionManager.from(mContext).getActiveSubscriptionInfo(
                                defaultSubId) == null) {
                        defaultSubId = SubscriptionManager.from(mContext)
                    ...
                    SubscriptionInfo defaultSimInfo = SubscriptionManager.from(
                        mContext).getActiveSubscriptionInfo(defaultSubId);
                    int defaultSlotId = defaultSimInfo.getSimSlotIndex();
                    if (defaultSlotId == 0) {
                        secondSubInfo = SubscriptionManager.from(mContext)
                                .getActiveSubscriptionInfoForSimSlotIndex(1);
                    } else {
                        secondSubInfo = SubscriptionManager.from(mContext)
                                .getActiveSubscriptionInfoForSimSlotIndex(0);
                    }
                    secondSubId = secondSubInfo.getSubscriptionId();
                }
                if (v == mButtonBig) {
                    sendSubIdForDualBtn = defaultSubId;
                } else if (v == mButtonSmall) {
                    sendSubIdForDualBtn = secondSubId;
                }
                btnListener.setSelectedSubId(sendSubIdForDualBtn);
                btnListener.onClick(v);
            }
        };
        mButtonBig.setImageBitmap(bigSendButtonBitmap);
        mButtonSmall.setImageBitmap(smallSendButtonBitmap);
        mButtonBig.setOnClickListener(buttonClickListener);
        mButtonSmall.setOnClickListener(buttonClickListener);
    }

    @Override
    public void updateNewTextCounter(int textLineCount, boolean isMms,
            int remainingInCurrentMessage, int msgCount) {
    ...
    /**
     * M: hide all the old button which the defult for mms.
     */
    ...
    public void updateDualSendButtonStatue(boolean enable, boolean isMms) {
        Xlog.d(TAG, "updateDualSendButtonStatue  enable: " + enable + " isMms:"
                + isMms);
        ...
        int simCount = SubscriptionManager.from(mContext)
                .getActiveSubscriptionInfoCount();
        Xlog.d(TAG, "updateDualSendButtonStatue, simCount:" + simCount);
        List<SubscriptionInfo> listSubInfo = SubscriptionManager.from(mContext)
        ...
        if (bigEnable != enable) {
            Bitmap bitmap = null;
            if (enable) {
                bitmap = mmsUtilsExt.createSendButtonBitMap(this,
                        R.drawable.ct_send_big, defaultSubId, true);
            } else {
                bitmap = mmsUtilsExt.createSendButtonBitMap(this,
                        R.drawable.ct_send_big_disable, defaultSubId, false);
            }
            mButtonBig.setImageBitmap(bitmap);
        }
        if (smallEnable != enable) {
            Bitmap bitmap = null;
            int secondSubId = -1;
            SubscriptionInfo secondSimInfo = null;
            SubscriptionInfo defaultSimInfo = SubscriptionManager
            ...
            if (simCount == 2) {
                ...
                secondSubId = secondSimInfo.getSubscriptionId();
            } else {
                secondSubId = defaultSubId;
            }
            if (enable) {
                bitmap = mmsUtilsExt.createSendButtonBitMap(this,
                        R.drawable.ct_send_small, secondSubId, true);
            } else {
                bitmap = mmsUtilsExt.createSendButtonBitMap(this,
                        R.drawable.ct_send_small_disable, secondSubId, false);
            }
            mButtonSmall.setImageBitmap(bitmap);
        }
        ...
        Xlog.d(TAG, "updateDualSendButtonStatue END");
    }
     ...
     */
    private void showMassTextMsgDetails(Context context, String[] items,
    ...
    public void setIsShowDualButtonPanel(boolean ifShowDualButtonPanel) {
        mIsShowDualButtonPanel = ifShowDualButtonPanel;
    }

    @Override
    public void hideOrShowSubjectForCC(boolean screenIsFull, EditText subjectTextEditor) {
        if (screenIsFull) {
            subjectTextEditor.setVisibility(View.GONE);
            mIsSubjectHide = true;
            return;
        }
        if (mIsSubjectHide && subjectTextEditor != null) {
            subjectTextEditor.setVisibility(View.VISIBLE);
            mIsSubjectHide = false;
        }
    }

    @Override
    public boolean getIsSubjectHide() {
        return mIsSubjectHide;
    }
}
