...
public class ManualNetworkSelection extends PreferenceActivity implements
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (subId != null) {
            ...
            if (mPhone == null) {
                Xlog.e(TAG, "onCreate: mSubId is invalid, finsh");
                mIsAborting = true;
                finish();
            }
        } else {
            Xlog.e(TAG, "onCreate: getSubId = null,finsh");
            mIsAborting = true;
            finish();
        }
 ...
 private boolean targetSlotRadioOn() {
        ...
        try {
            if (mTelephony != null) {
                isRadioInOn = mTelephony.isRadioOnForSubscriber(mSubId);
                Xlog.d(TAG, "Slot " + mTargetSlot + " is in radion state " + isRadioInOn);
            }
        ...
        private void handleNetworkSelectionDone(final Message msg) {
            ...
            Runnable runnable = new Runnable() {
                ...
                public void run() {
                    ...
                    removeDialog(DIALOG_NETWORK_SELECTION);
                    if (TelephonyManager.NETWORK_TYPE_UNKNOWN != mServiceState.getDataNetworkType()) {
                        networkType_t = getServiceNetworkRat(mServiceState.getDataNetworkType());   
                    } else if (TelephonyManager.NETWORK_TYPE_UNKNOWN != mServiceState.getVoiceNetworkType()){
                        networkType_t = getServiceNetworkRat(mServiceState.getVoiceNetworkType());
                    }
                    log("handleNetworkSelectionDone mSelectedNetworkRate=" + mSelectedNetworkRate);
                    log("handleNetworkSelectionDone networkType_t=" + networkType_t);
                    
                    if (mSelectedNetworkRate == networkType_t) {
    ...
    private void networkQuery() {
        log("networkQuery, mTargetSlot = " + mTargetSlot);
        try {
        ...
        if (status != QUERY_OK) {
            ...
            if (result != null) {
                ...
                for (OperatorInfo ni : result) {
                    ...
                    log("gsmNetworksListLoaded isCurrentNetwork return: " + isCurrentNetwork(isDataNet, ni));
        ...
        if (mPhoneStateListener == null) {
            mPhoneStateListener = new PhoneStateListener(mSubId) {
                ...
                public void onServiceStateChanged(ServiceState serviceState) {
                    Xlog.i(TAG, "onServiceStateChanged, serviceState: " + serviceState);
