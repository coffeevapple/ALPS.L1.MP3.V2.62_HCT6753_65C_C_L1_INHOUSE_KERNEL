 ...
 */
@PluginImpl(interfaceName = "com.mediatek.mms.ext.IMmsTextSizeAdjustExt")
public class Op09MmsTextSizeAdjustExt extends DefaultMmsTextSizeAdjustExt {
    ...
    public void init(IMmsTextSizeAdjustHost host, Activity activity) {
        Xlog.d(TAG, "init(): init IMmsTextSizeAdjustHost.");
     ...
     */
    public void refresh() {
        Xlog.d(TAG, "refresh(): mHostContext = " + mHostContext);
        //get the lasted font size and set.
        if (mHostContext != null) {
            float size = Op09MmsUtilsExt.getTextSize(mHostContext);
            Xlog.d(TAG, "refresh(): Current textSize = " + mTextsize + ", new textSize = " + size);
            if (mTextsize != size) {
                mTextsize = size;
                mListener.setTextSize(mTextsize);
    ...
    public class OP09OnScaleListener extends ScaleDetector.SimpleOnScaleListener {
        ...
        protected void performChangeText(float size) {
            Xlog.d(TAG, "performChangeText(): Current textSize = " + mTextsize
                + ", new textSize = " + size);
            if (mTextsize != size) {
                mTextsize = size;
                IMmsTextSizeAdjustHost host = getHost();
                if (host != null) {
                    Xlog.d(TAG, "performChangeText(): Calling host.setTextSize()");
