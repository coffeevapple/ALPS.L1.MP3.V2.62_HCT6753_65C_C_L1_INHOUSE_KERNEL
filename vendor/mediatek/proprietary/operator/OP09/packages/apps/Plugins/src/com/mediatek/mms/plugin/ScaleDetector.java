...
/**
 * only first and second finger can pinch. if finger num > 2, the gesture stop. if only one finger
 * stay, you can start with another finger down.
 */
public class ScaleDetector {
     ...
     * combined pressure. We will only fire an onScale event if the computed ratio between the
     * current and previous event pressures is greater than this value. When pressure decreases
     * rapidly between events the position values can often be imprecise, as it usually indicates
     * that the user is in the process of lifting a pointer off of the device. Its value was tuned
     * experimentally.
     */
    ...
    public interface OnScaleListener {
        ...
        /**
         * when the ScaleGesture start, onScaleStart will be called. if return false,
         * the onScale()and onScaleEnd will not be called.
         * @param detector the scaleDetector.
         * @return boolean false: the onScale()and onScaleEnd will not be called.
         */
        boolean onScaleStart(ScaleDetector detector);

       /**
        * when the ScaleGesture end, onScaleEnd will be called.
        * @param detector the ScaleDetector.
        */
         ...
         *
         * @param detector
         *            is the rate that the length of current two touch point and length of
         *            two touch point when MotionDown.
         * @return boolean true: scale; false: no scale.
         */
     ...
     * @return GestureInProgress. true: on Continue handle. false: end.
     */
                ...
                if (count == 2) {
                    ...
                    if (mListener != null) {
                        ...
                        if (mGestureInProgress) {
                            // send ACTION_CANCEL to cancel previous actions before
                            // ACTION_POINTER_DOWN
     ...
     * This value is defined as ({@link #getCurrentSpan()} / {@link #getPreviousSpan()}).
     *
    ...
    public abstract static class SimpleOnScaleListener implements OnScaleListener {
        ...
        public interface OnTextSizeChanged {
            /**
             * when text size has changed , on changed will be called.
             *
             * @param size
             *            the text size.
             */
         ...
         * @param initTextSize the text size.
         */
