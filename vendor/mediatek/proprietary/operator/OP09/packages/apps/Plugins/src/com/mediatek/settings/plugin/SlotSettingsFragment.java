...
public class SlotSettingsFragment extends PreferenceFragment {
    ...
    private BroadcastReceiver mReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(Intent.ACTION_AIRPLANE_MODE_CHANGED)
                ...
                updateScreen();
                Xlog.d(TAG, "ACTION_CDMA_CARD_TYPE: cardType = " + cardType
    ...
    public void onResume() {
        ...
        if (subId != null) {
            ...
            Xlog.d(TAG, "onResume: mTargetSlot = " + mTargetSlot + ", mSubId = " +
                       mSubId + ", phoneID = " + phoneID);
        } else {
            Xlog.e(TAG, "onResume: getSubId = null");
        }
    ...
    private void phoneStateChangeListener() {
        Xlog.i(TAG, "listenPhoneStateListener, mTargetSlot: "
                  + mTargetSlot + ", mSubId: " + mSubId);
        if (mPhoneStateListener == null) {
            mPhoneStateListener = new PhoneStateListener(mSubId) {
                @Override
                public void onServiceStateChanged(ServiceState serviceState) {
                    Xlog.i(TAG, "listenPhoneStateListener, status: " + serviceState);
                    if (null == SlotSettingsFragment.this.getActivity()) {
                        Xlog.i(TAG, "fragment is removed from activity, return");
                        return;
                    }
                    ...
                    if (mIsSIMRadioSwitching) {
                        ...
                        Xlog.i(TAG, "soltId: " + mTargetSlot + ", radio is off : " + isOff);
                        if (isOff) {
    ...
    private void switchSimRadioState(int slot, boolean isChecked) {
        if (!SubscriptionManager.isValidSlotId(slot)) {
            Xlog.i(TAG, "switchSimRadioState, slot id is invalid");
            return;
        }
    ...
    private boolean getRadioState() {
        ...
        try {
            if (mTelephony != null) {
                isRadioStateOn = mTelephony.isRadioOnForSubscriber(mSubId);
            }
        ...
        if (!isAirplaneOn) {
            ...
            if (targetSlotRadioOn) {
                ...
                if (getPreferenceScreen().findPreference(KEY_MANUAL_NETWORK_CDMA_SELECTION) !=
                    ...
                    Xlog.d(TAG, "updateScreen: externalSlotInRoaming = " + externalSlotInRoaming);
                }
   ...
   /**
    ...
    public static boolean externalSlotInRoamingService(ServiceState serviceState) {
        ...
        Xlog.d(TAG, "externalSlotInRoamingService, isRoamingMode : " + isRoamingMode);
    ...
    private boolean isTargetSubRadioOn(int subId) {
        boolean isRadioOn = false;
        try {
            isRadioOn = mTelephony.isRadioOnForSubscriber(subId);
        } catch (RemoteException e) {
            isRadioOn = false;
