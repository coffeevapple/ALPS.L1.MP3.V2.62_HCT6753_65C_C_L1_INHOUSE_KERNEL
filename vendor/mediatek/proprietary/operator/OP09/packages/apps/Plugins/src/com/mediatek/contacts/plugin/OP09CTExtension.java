...
public class OP09CTExtension extends DefaultCtExtension {
    ...
    public String formatPhoneNumber(String phoneNumber) {
        return super.formatPhoneNumber(phoneNumber);
    }
    ...
    public Bitmap getOperatorIconBitmapForPhotoId(long mPhotoId,
        ...
        Log.i(TAG, "getOperatorIconBitmapForPhotoId mPhotoId : "
                + mPhotoId + " | resultBitmap : " + resultBitmap);
        if (mPhotoId == -16 && null != sCtSim1Resourceid) {
            return sCtSim1Resourceid;
        } else if (mPhotoId == -17 && null != sCtSim2Resourceid) {
            return sCtSim2Resourceid;
        } else {
            Log.i(TAG, "getOperatorIconBitmapForPhotoId return defualt value");
        }
        return resultBitmap;
    }
...
//                .getSubInfoForSubscriber(subId).simIconRes[0]);
    ...
    public long getPhotoIdBySub(int subId, int sdnFlag, long photoId) {
        // TODO Auto-generated method stub
        Log.i(TAG, "getPhotoIdBySub");
        int slotId = getSlotIdBySubId(subId);
    ...
    public String getString(String stringName) {
        return super.getString(stringName);
    }

    @Override
    public boolean isOperatorSimPhotoId(long mPhotoId) {
        Log.i(TAG, "isOperatorSimPhotoId mPhotoId : " + mPhotoId);
        if (mPhotoId == -16 || mPhotoId == -17) {
            return true;
        }
        return false;
    }
