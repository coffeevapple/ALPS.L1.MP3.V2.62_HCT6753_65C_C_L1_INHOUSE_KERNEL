...
public class PhoneNumberUtils {
    ...
    private static String getGeoDescription(Context context, String number) {
        Xlog.d(TAG, "getGeoDescription(" + number + ")");
        ...
        if (SystemProperties.get("ro.mtk_phone_number_geo").equals("1")) {
            ...
            Xlog.d(TAG, "[GeoCodingQuery] cityName = " + cityName);
        ...
        try {
            ...
            Xlog.d(TAG, "parsing '" + number + "' for countryIso '" + countryIso
                + "',parsed number '" + pn + "', used locale '" + locale + "'...");
        } catch (NumberParseException e) {
            Xlog.e(TAG, "getGeoDescription: NumberParseException for incoming number '" + number
                + "'");
        }
        if (pn != null) {
            String description = geocoder.getDescriptionForNumber(pn, locale);
            Xlog.d(TAG, "- got geoDescription : '" + description + "'");
            return description;
        }
        return null;
    }
    ...
    private static String getCurrentCountryIso(Context context, Locale locale) {
        ...
        if (countryIso == null) {
            ...
            Xlog.w(TAG, "No CountryDetector; falling back to countryIso based on locale: "
                + countryIso);
        }
        return countryIso;
    }
