 ...
 */
@PluginImpl(interfaceName = "com.mediatek.mms.ext.IMissedSmsReceiverExt")
public class Op09MissedSmsReceiverExt extends DefaultMissedSmsReceiverExt {
    ...
    private Uri findMissedSms(int refId, int newPduCount) {
        ...
        if (cursor == null || cursor.getCount() != 1) {
            Xlog.e(TAG, "cursor == " + (cursor == null ? "NULL" : cursor.getCount()));
            return null;
        }
        ...
        try {
            if (cursor.moveToFirst()) {
                ...
                Xlog.d(TAG, "totalCount: " + totalCount + " newPduCount: " + newPduCount);

                if (newPduCount > totalCount) {
                    Xlog.e(TAG, "Wrong Pdu Count!");
                    return null;
                }

                if (newPduCount < totalCount) {
                    mIsWholeSms = false;
                    Xlog.d(TAG, "Not whole SMS! totalCount should be: " + totalCount);
                }
