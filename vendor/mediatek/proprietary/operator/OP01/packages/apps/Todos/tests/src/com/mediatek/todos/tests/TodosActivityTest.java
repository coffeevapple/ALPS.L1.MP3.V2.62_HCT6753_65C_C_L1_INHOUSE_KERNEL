...
public class TodosActivityTest extends ActivityInstrumentationTestCase2<TodosActivity> {
    ...
    private static final int DB_OPEARION_TIMEOUT = 5; // in seconds
    private QueryListenerForTest mQueryListener;
    ...
    protected void setUp() throws Exception {
        ...
        mQueryListener = new QueryListenerForTest();
        try {
            Thread.sleep(500);
...
//                assertEquals(1, (mAdapter.getSeletedTodosNumber() + mAdapter.getSeletedDonesNumber()));
    ...
    public void test01Query() {
        ...
        preCondition(mAdapter);
        mQueryListener.startQuery(null);
        waitUntilOperationFinished("testQuery");
        assertEquals(QueryListenerForTest.QUERY, mQueryListener.getQueryListenerData());
    }
    ...
    public void test02Insert() {
        ...
        mQueryListener.startInsert(info);
        waitUntilOperationFinished("testInsert");
        assertEquals(QueryListenerForTest.INSERT, mQueryListener.getQueryListenerData());
    }
    ...
    public void test03Update() {
        ...
        mQueryListener.startUpdate(info);
        waitUntilOperationFinished("testUpdate");
        assertEquals(QueryListenerForTest.UPDATE, mQueryListener.getQueryListenerData());
    }
    ...
    public void test04Delete() {
        ...
        int todosNumber = mAdapter.getTodosDataSource().size();
        info = mAdapter.getTodosDataSource().get(todosNumber - 1);
        mQueryListener.startDelete(info);
        waitUntilOperationFinished("testDelete");
        assertEquals(QueryListenerForTest.DELETE, mQueryListener.getQueryListenerData());
    }
    ...
    public void testConstructure() {
        ...
        Cursor cursor = mDataBase.query(Tables.TODOS, null, null, null, null, null, null);
        try {
            if (cursor != null) {
                int count = cursor.getCount();
                int mAdapterNumber = mAdapter.getDonesDataSource().size() + mAdapter.getTodosDataSource().size();
     ...
     * Looping and check the condition in @sleeptime.
     */
    ...
    /**
     * M:Wait until any of the {@link QueryListenerForTest} method is called.@{
     */
    private void waitUntilOperationFinished(String message) {
        waitUntil(message, new Condition() {
            @Override
            public boolean isMet() {
                return mQueryListener.isOperationFinished();
            }
        }, DB_OPEARION_TIMEOUT, 1000);
    }
}
