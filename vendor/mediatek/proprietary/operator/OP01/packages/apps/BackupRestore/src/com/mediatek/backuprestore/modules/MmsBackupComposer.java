            ...
            if (mMmsCursorArray[i] != null && !mMmsCursorArray[i].isAfterLast()) {
                ...
                try {
                    ...
                    if (pduMid != null) {
                        ...
                        int subId = mMmsCursorArray[i].getInt(mMmsCursorArray[i].getColumnIndex(COLUMN_NAME_SUBID));
                        //long subId = mMmsCursorArray[i].getLong(mMmsCursorArray[i].getColumnIndex(COLUMN_NAME_SUBID));
                        MyLogger.logD(CLASS_TAG, "Get subId From Mms_db :" + subId);
                        String slotId = "0";
                        if(SystemProperties.get("ro.mtk_gemini_support").equals("1") && subId >= 0) {
                            SubscriptionInfo simInfo = new SubscriptionManager(mContext).getActiveSubscriptionInfo(subId);
                            if (simInfo != null) {
                                int slot = simInfo.getSimSlotIndex();
                                slotId = String.valueOf(slot + 1);
                            } else {
                                MyLogger.logD(CLASS_TAG, "WARMING : simInfo is null and stop backup MMS");
                            }
