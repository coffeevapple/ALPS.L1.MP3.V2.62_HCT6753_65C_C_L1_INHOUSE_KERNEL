    ...
    public boolean onEnd() {
     ...
     private String getCondition() {
         if (mParams != null) {
             ...
             List<SubscriptionInfo> simInfoList = new SubscriptionManager(mContext).getActiveSubscriptionInfoList();
             if(simInfoList != null && simInfoList.size() > 0) {
                 for(SubscriptionInfo simInfo : simInfoList){
                     if(mParams.contains(String.valueOf(simInfo.getSimSlotIndex()))) {
                         conditionList.add(Integer.toString(simInfo.getSubscriptionId()));
                     }
                 }
             }
