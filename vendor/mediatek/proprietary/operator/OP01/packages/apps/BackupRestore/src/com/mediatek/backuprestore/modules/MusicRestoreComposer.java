        ...
        if (folder.exists() && folder.isDirectory()) {
            try {
                ...
                if( mFileNameList == null ) {
                    MyLogger.logD(CLASS_TAG, "init(): zipFile is error Maybe the FIle is the error Zip ! ");
                    return result;
                }
                //String tmppath = (new File(mParentFolderPath)).getParent();
                if(SDCardUtils.getSDCardDataPath() != null ) {
                    tmppath = SDCardUtils.getSdCardMountPath();
                } else {
                    tmppath = SDCardUtils.getPhoneStoragePath();
                }
                if (tmppath != null) {
