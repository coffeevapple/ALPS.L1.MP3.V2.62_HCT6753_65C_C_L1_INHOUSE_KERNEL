...
public class BackupFileScanner {
    ...
    //private HashMap<String,List<?>> mDataResult = new HashMap<String, List<?>>();
    ...
    private class ScanThread extends Thread {
        ...
        private String filterFile(File file, int index) {
            ...
            /*String fileIndex[] = file.getAbsolutePath().split(File.separator);
            if (fileIndex.length > 26) {
                MyLogger.logV(CLASS_TAG, "scanner warning , the folder is more than 20 !!" + file.getAbsolutePath()
                        + ", fileIndex.length = " + fileIndex.length);
                return "WARNNING";
            }*/
            if(index > 23) {
                MyLogger.logV(CLASS_TAG, "scanner warning , the folder is more than 20 !!");
                return "WARNNING";
            }
            ...
            if(files != null && files.length > 0) {
                for (File currentFile : files) {
                    ...
                    if (currentFile.exists() && currentFile.isDirectory()) {
                        ...
                        if(result != null && result.length() > currentFile.getAbsolutePath().length()) {
                            MyLogger.logV(CLASS_TAG, "warning : return result = "+ result);
                            return result;
                        }
                    } else if (currentFile.exists() && currentFile.isFile()
                            && checkEndsWithInStringArray(currentFile.getAbsolutePath(),
                                    (mContext.getResources().getStringArray(R.array.apk_ending)))) {
                        /*MyLogger.logV(CLASS_TAG, "APP 1 : file getAbsolutePath = "+ currentFile.getAbsolutePath());
                        MyLogger.logV(CLASS_TAG, "APP 3 : file name  = "+ currentFile.getName());*/
                        synchronized (mObject) {
                            ...
                            && checkEndsWithInStringArray(currentFile.getAbsolutePath(),
                                    (mContext.getResources().getStringArray(R.array.data_ending)))) {
                        /*MyLogger.logV(CLASS_TAG, "DATA 1 : file getAbsolutePath() = "+ currentFile.getAbsolutePath());
                        MyLogger.logV(CLASS_TAG, "DATA 3 : file name  = "+ currentFile.getName());*/
                        synchronized (mObject) {
                            if(checkDataForAdd(currentFile.getParentFile().getParentFile().getAbsolutePath(),"PERSON_DATA")) {
                                BackupFilePreview mPersonData = new BackupFilePreview(currentFile.getParentFile().getParentFile());
                                mPersonData.setDataType("PERSON_DATA");
                                mDataResult.add(mPersonData);
                                handleMessageData(mDataResult,MessageID.SCANNER_ONE_ITEM);
                            }
                        }
                        return currentFile.getParentFile().getParentFile().getAbsolutePath();
                    } else if (currentFile.exists() && currentFile.isFile()
                            && checkEndsWithInStringArray(currentFile.getAbsolutePath(),
                                    (mContext.getResources().getStringArray(R.array.old_data_ending)))) {
                        /*MyLogger.logV(CLASS_TAG, "OLD_DATA 1 : file getAbsolutePath() = "+ currentFile.getAbsolutePath());
                        MyLogger.logV(CLASS_TAG, "OLD_DATA 3 : file name  = "+ currentFile.getName());*/
                        synchronized (mObject) {
                            if(checkDataForAdd(currentFile.getAbsolutePath(),"OLD_DATA")) {
                                OldBackupFilePreview backupFile = new OldBackupFilePreview(currentFile);
                        ...
                        MyLogger.logV(CLASS_TAG, "currentFile is no data path = " + currentFile.getAbsolutePath());*/
                    }
                }
            }
                ...
                for (int i = 0; i < mDataResult.size(); i++) {
                    ...
                    if (obj instanceof BackupFilePreview && type.equals("PERSON_DATA")) {
                        ...
                        if (backupFilePreview.getFile().getAbsolutePath().equals(path)) {
                            MyLogger.logD(CLASS_TAG, "checkDataForAdd : PERSON_DATA false path = " + path);
                            return false;
                        }
                        ...
                        if (appData.getAppData().contains(appData)) {
                            MyLogger.logD(CLASS_TAG, "checkDataForAdd : APP false path = " + path);
                            return false;
                        }
                    } else if (obj instanceof OldBackupFilePreview && type.equals("OLD_DATA")) {
                        OldBackupFilePreview oldData = (OldBackupFilePreview) obj;
                        if (oldData.getFilePath().equals(path)) {
                            MyLogger.logD(CLASS_TAG, "checkDataForAdd : OLD_DATA false path = " + path);
                            return false;
                        }
                    }
                }
...
/*        private String scanPersonalBackupFiles() {
        ...
        private void checkDataNeedUpdata() {
            String sdcard = SDCardUtils.getSDCardDataPath();
            String phone = SDCardUtils.getPhoneStoragePath();
            if (mDataResult != null && mDataResult.size() > 0) {
                for (int i = mDataResult.size() - 1; i >= 0; i--) {
                    ...
                    if (obj instanceof BackupFilePreview) {
                        ...
                        MyLogger.logE(CLASS_TAG, " 0 checkDataNeedUpdata : path = " + path);
                        if (sdcard == null && !path.startsWith(phone)) {
                            MyLogger.logE(CLASS_TAG,
                                    " 1 checkDataNeedUpdata : checkFile = " + checkFile.getAbsolutePath());
                            mDataResult.remove(i);
                        } else if (!checkFile.exists()) {
                            MyLogger.logE(CLASS_TAG,
                                    " 2 checkDataNeedUpdata : checkFile = " + checkFile.getAbsolutePath());
                            mDataResult.remove(i);
                        } else {
                          ///M : modify for the file is deleted and path is recreate,so setSize again.
        ...
        private void settingsListPath() {
            mListPath.clear();
            if(mIsFullScan) {
                if(SDCardUtils.getPathList() != null) {
                    mListPath = SDCardUtils.getPathList();
                }
            } else {
                String oldBackupPath = SDCardUtils.getSDCardDataPath();
                String sdcardPath = SDCardUtils.getSDCardDataPath();
                String phonePath = SDCardUtils.getPhoneDataPath();
