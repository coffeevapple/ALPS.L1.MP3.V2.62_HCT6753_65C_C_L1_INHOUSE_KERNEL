            ...
            for (int j = 0; (mPduList != null) && (j < mPduList.size()); ++j) {
                ...
                if (pduMid != null) {
                    ...
                    if (msgUri == Mms.Inbox.CONTENT_URI) {
                        try {
                            retrieveConf = (RetrieveConf) new PduParser(pduMid, false).parse();
                        } catch (Exception e) {
                            e.printStackTrace();
                        } finally {
                            if (retrieveConf == null) {
                                indConf = (NotificationInd) new PduParser(pduMid, false).parse();
                            }
                        }
                    } else {
                        sendConf = (SendReq) new PduParser(pduMid, false).parse();
                    }
