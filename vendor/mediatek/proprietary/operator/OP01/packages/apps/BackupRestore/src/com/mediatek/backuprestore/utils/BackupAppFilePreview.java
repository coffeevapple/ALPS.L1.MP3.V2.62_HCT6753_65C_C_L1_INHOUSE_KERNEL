...
public class BackupAppFilePreview {
    private static final String CLASS_TAG = MyLogger.LOG_TAG + "/BackupAppFilePreview";
    ...
    private CopyOnWriteArraySet<String> mApkFile = new CopyOnWriteArraySet<String>();
    private HashMap<Integer, Integer> mNumberMap = new HashMap<Integer, Integer>();

    public BackupAppFilePreview() {
        mNumberMap.clear();
        mApkFile.clear();
    }

    public BackupAppFilePreview(File file, String absolutePath) {
        if (file == null) {
            MyLogger.logE(CLASS_TAG, "apkFile error! file is null");
            return;
        }
        mNumberMap.clear();
        mFolderName = file.getParentFile();
        //computeSize();
    }
    
    public void isAdded(boolean result) {
        mAdded = result;
    }
    ...
    public long computeSize() {
        ...
        long mSize = 0;
        if (mApkFile.size() > 0 ){
            for(String mFile : mApkFile) {
                //MyLogger.logE(CLASS_TAG, "BackupAppFilePreview : computeSize ! mFile "+ mFile);
                file = new File(mFile);
                if (file != null && file.isFile()) {
                    mSize = mSize + file.length();
                }
            }
            mFolderName = file.getParentFile();
        }
    ...
    public void putData(String filePath) {
        mApkFile.add(filePath);
        //Log.v(CLASS_TAG, "putData: filepath= " + filePath + ",mApkFile size = " + mApkFile.size());
    }
    
    public int getAppDataSize() {
        return mApkFile.size();
    }
    
    public void checkAppData() {
        if(mApkFile.size() >0 ) {
            for(String file : mApkFile) {
                File checkFile = new File(file);
                if(!checkFile.exists()) {
                    mApkFile.remove(file);
                }
            }
        }
    }
    
    public CopyOnWriteArraySet<String> getAppData() {
        return mApkFile;
    }
