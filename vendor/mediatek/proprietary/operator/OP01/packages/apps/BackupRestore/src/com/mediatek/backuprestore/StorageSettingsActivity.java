...
public class StorageSettingsActivity extends PreferenceActivity {
    ...
    private void addRadioButtonPreferences(PreferenceScreen ps, SettingData mSettingData) {
        ...
        if (mSettingData != null) {
            ...
            if(mSettingData.getType().equals(Constants.SDCARD_STOTAGE) && SDCardUtils.getSDCardDataPath() == null) {
    ...
    private List<SettingData> initData(Bundle savedInstanceState) {
        ...
        for(int i = 0; i< storageData.length; i++) {
            if(!SDCardUtils.isSupprotSDcard() && storageData[i].equals(Constants.SDCARD_STOTAGE)){
                continue;
            }
        ...
        for (SettingData mData : mSettingDataList) {
            if(!SDCardUtils.isSupprotSDcard() && mData.getType().equals(Constants.SDCARD_STOTAGE)) {
        ...
        Log.d(CLASS_TAG, "~initData path = " +mStoragePath +" SDCardUtils.isSupprotSDcard() == "+SDCardUtils.isSupprotSDcard());
    ...
    public boolean onPreferenceTreeClick(PreferenceScreen preferenceScreen, Preference preference) {
        if (preference != null && preference instanceof AppBackupRadioButtonPreference) {
            ...
            case Constants.PHONE_STOTAGE:
                mStoragePath = SDCardUtils.getPhoneDataPath();
            ...
            case Constants.SDCARD_STOTAGE:
                mStoragePath = SDCardUtils.getSDCardDataPath();
                if( mStoragePath != null) {
                    setCurrentPath(this, mStoragePath);
                    setPathIndexKey(this,key);
                }
    ...
    private void registerSDCardListener() {
        SDCardReceiver.getInstance().registerOnSDCardChangedListener(mSDCardStatusChangedListener);
    }
    private void unRegisterSDCardListener() {
        if (SDCardReceiver.getInstance() != null) {
            SDCardReceiver.getInstance().unRegisterOnSDCardChangedListener(mSDCardStatusChangedListener);
        }
    }
    class SDCardStatusChangedListener implements SDCardReceiver.OnSDCardStatusChangedListener {
        ...
        public void onSDCardStatusChanged(boolean mount) {
            ...
            if(!mount) {
                if(mStoragePath.contains(SDCardUtils.getSdCardMountPath())){
                    mStoragePath = SDCardUtils.getPhoneDataPath() + File.separator + ModulePath.FOLDER_BACKUP;
                    mSettingData = new Bundle();
                    mSettingData.putString(Constants.DATE, mStoragePath);
                }
            }
            MyLogger.logD(CLASS_TAG, "onSDCardStatusChanged - mount = " + mount +" mStoragePath = "+mStoragePath);
