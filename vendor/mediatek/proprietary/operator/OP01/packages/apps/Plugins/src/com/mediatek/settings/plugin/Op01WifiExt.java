...
public class Op01WifiExt extends DefaultWifiExt implements AdapterView.OnItemSelectedListener {
    ...
    private static final String KEY_WIFI_GATEWAY = "wif_gateway";
    private static final String KEY_WIFI_NETMASK = "wif_netmask";
    ...
    public void addDisconnectButton(AlertDialog dialog, boolean edit, DetailedState state, final int networkId) {
        Xlog.d(TAG, "addDisconnectButton, edit = " + edit + ", state = " + state + ", networkId = " + networkId);
        if (!edit && state != null && shouldSetDisconnectButton()) {
            //set disconnect button
            dialog.setButton(DialogInterface.BUTTON_POSITIVE, mContext.getString(R.string.wifi_disconnect),
                    new DialogInterface.OnClickListener() {
                        public void onClick(DialogInterface dialog, int which) {
                            Xlog.d(TAG, "addDisconnectButton, onClick, networkId = " + networkId);
    ...
    public void initConnectView(Activity activity, PreferenceScreen screen) {
        if (screen.findPreference(KEY_CONNECT_TYPE) != null) {
            return;
        }
    ...
    public void initNetworkInfoView(PreferenceScreen screen) {
        if (screen.findPreference(KEY_WIFI_GATEWAY) != null) {
            screen.removePreference(mGatewayPref);
        }
        if (screen.findPreference(KEY_WIFI_NETMASK) != null) {
            screen.removePreference(mNetmaskPref);
        }
        mGatewayPref = new Preference(mContext);
        mGatewayPref.setTitle(mContext.getString(R.string.wifi_gateway));
        mGatewayPref.setKey(KEY_WIFI_GATEWAY);
        ...
        mNetmaskPref.setKey(KEY_WIFI_NETMASK);
    ...
    private int getSecurity(WifiConfiguration config) {
     ...
     public void setEapMethodArray(ArrayAdapter adapter, String ssid, int security) {
        if (SystemProperties.get("ro.mtk_eap_sim_aka").equals("1")) {
