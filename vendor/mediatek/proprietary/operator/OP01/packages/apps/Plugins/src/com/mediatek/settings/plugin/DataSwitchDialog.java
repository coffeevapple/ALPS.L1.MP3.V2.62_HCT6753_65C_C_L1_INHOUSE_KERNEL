...
public class DataSwitchDialog extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mContext = this;
        mActivity = this;
        Xlog.d(TAG, "Switch data: " + getCallingActivity());
        Xlog.d(TAG, "Switch data: " + getCallingPackage());

        mToOpenSubId =
            getIntent().getIntExtra("subId", SubscriptionManager.INVALID_SUBSCRIPTION_ID);
        Xlog.d(TAG, "mToOpenSubId =" + mToOpenSubId);
        ...
        builder.setPositiveButton(android.R.string.yes,
               new DialogInterface.OnClickListener() {
                      public void onClick(DialogInterface dialog, int whichButton) {
                           ...
                           Xlog.d(TAG, "Switch data to subId = " + mToOpenSubId);
                           Xlog.d(TAG, "Switch data: " + mActivity.getCallingActivity());
                           Xlog.d(TAG, "Switch data: " + mActivity.getCallingPackage());
                           Xlog.d(TAG, "Switch data = " + getCallingActivity());
                           Xlog.d(TAG, "Switch data = " + getCallingPackage());
                           setDefaultDataSubId(context, mToOpenSubId);
                           finish();
                           Xlog.d(TAG, "finish avtivity");
                       }
                   });
    ...
    private void dealDataConnectionChanged(int subId) {
        Xlog.i(TAG, "dealDataConnectionChanged: subId is " + subId +
                ", mToOpenSubId is " + mToOpenSubId);
        if (mToOpenSubId == subId) {
            Xlog.d(TAG, "dealDataConnectionChanged finish ");
            finish();
        }
    }
    ...
    private void setDefaultDataSubId(final Context context, final int subId) {
        if (subId != SubscriptionManager.INVALID_SUBSCRIPTION_ID) {
            ...
            Xlog.i(TAG, "setDefaultDataSubId: subId is " + subId +"curConSubId is"+ curConSubId );
            if (curConSubId == subId){
                return;
            }
            boolean enable = false;
            if(tm.getDataEnabled(curConSubId) || tm.getDataEnabled(subId)){
                Xlog.i(TAG, "tm.setDataEnabled(curConSubId, false) ");
                tm.setDataEnabled(curConSubId, false);
                enable = true;
            }
            subscriptionManager.setDefaultDataSubId(subId);
            if(enable){
                Xlog.i(TAG, "tm.setDataEnabled(subId, true) ");
