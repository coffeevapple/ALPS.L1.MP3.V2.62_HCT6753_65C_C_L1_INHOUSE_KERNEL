...
public class Op01WifiSettingsExt extends DefaultWifiSettingsExt {
    ...
    public  void updateContextMenu(ContextMenu menu, int menuId, DetailedState state) {
        Xlog.d(TAG, "updateContextMenu state = " + state);

        if (state != null
                && mWifiManager.getConnectionInfo().getSupplicantState() == SupplicantState.COMPLETED) {
            Xlog.d(TAG, "updateContextMenu string = " + mContext.getString(R.string.wifi_menu_disconnect));
    ...
    public Preference getPreference(PreferenceScreen preferenceScreen, int index) {
        int configedCount = mCmccConfigedAP.getPreferenceCount();
        if (index < configedCount) {
            return mCmccConfigedAP.getPreference(index);
        } else {
            return mCmccNewAP.getPreference(index - configedCount);
        }
    }
