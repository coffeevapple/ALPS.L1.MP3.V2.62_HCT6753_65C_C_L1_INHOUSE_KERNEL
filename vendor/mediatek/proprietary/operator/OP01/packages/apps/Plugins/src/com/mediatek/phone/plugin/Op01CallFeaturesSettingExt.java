...
public class Op01CallFeaturesSettingExt extends DefaultCallFeaturesSettingExt {
    ...
    public void initOtherCallFeaturesSetting(PreferenceFragment fragment) {
        ...
        try {
            cont = fragment.getActivity().getApplicationContext().createPackageContext(
                    "com.mediatek.op01.plugin",
                    Context.CONTEXT_INCLUDE_CODE | Context.CONTEXT_IGNORE_SECURITY);
        } catch (NameNotFoundException e) {
            log("catch no found com.mediatek.op01.plugin");
        }
        Preference callrejectPreference = new Preference(fragment.getActivity());
        callrejectPreference.setKey(BUTTON_CALL_REJECTION_KEY);
        if (cont != null) {
            callrejectPreference.setTitle(
                    cont.getResources().getString(R.string.call_reject_title));
            callrejectPreference.setSummary(
                    cont.getResources().getString(R.string.call_reject_sum));
        }
        Intent intent = new Intent();
        intent.setClassName("com.mediatek.op01.plugin",
                "com.mediatek.phone.callrejection.CallRejectSettingActivity");
        callrejectPreference.setIntent(intent);

        ListPreference buttonTTYPreference =
                (ListPreference) fragment.getPreferenceScreen().findPreference(BUTTON_TTY_KEY);
        int callrejectOrder = 0;
        if (buttonTTYPreference != null) {
            callrejectOrder = buttonTTYPreference.getOrder();
        }
        callrejectPreference.setOrder(callrejectOrder);
        int count = fragment.getPreferenceScreen().getPreferenceCount();
        //reset order
        for (int index = 0; index < count; index++) {
            Preference preference = fragment.getPreferenceScreen().getPreference(index);
            if (preference != null) {
                int pOrder = preference.getOrder();
                if (pOrder >= callrejectOrder) {
                    preference.setOrder(pOrder + 1);
                }
            }
        }

        fragment.getPreferenceScreen().addPreference(callrejectPreference);
        if (buttonTTYPreference != null) {
    ...
    public void initOtherCallFeaturesSetting(PreferenceActivity activity) {
        ...
        Context cont = null;
        log("initOtherCallFeaturesSettings PreferenceActivity");
        try {
            cont = activity.getApplicationContext().createPackageContext(
                    "com.mediatek.op01.plugin",
                    Context.CONTEXT_INCLUDE_CODE | Context.CONTEXT_IGNORE_SECURITY);
        } catch (NameNotFoundException e) {
            log("catch no found com.mediatek.op01.plugin");
        }
        Preference callrejectPreference = new Preference(activity);
        callrejectPreference.setKey(BUTTON_CALL_REJECTION_KEY);
        if (cont != null) {
            callrejectPreference.setTitle(
                    cont.getResources().getString(R.string.call_reject_title));
        ...
        callrejectPreference.setOrder(callrejectOrder);
        int count = activity.getPreferenceScreen().getPreferenceCount();
        //reset order
        for (int index = 0; index < count; index++) {
            Preference preference = activity.getPreferenceScreen().getPreference(index);
            if (preference != null) {
                int pOrder = preference.getOrder();
                if (pOrder >= callrejectOrder) {
                    preference.setOrder(pOrder + 1);
        ...
        activity.getPreferenceScreen().addPreference(callrejectPreference);
        if (buttonTTYPreference != null) {
