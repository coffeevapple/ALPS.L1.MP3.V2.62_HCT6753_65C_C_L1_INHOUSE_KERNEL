...
@PluginImpl(interfaceName="com.mediatek.mms.ext.ISmsReceiverExt")
        ...
        if (hasMissedSegments) {
            ...
            if (pduCount == 1) {
                ...
                for (int i = 0; i < pduCount; i++) {
                    ...
                    int encodingType = msg.getEncodingType();
                    Xlog.d(TAG, "thiss encodingType = " + encodingType);
                    if (encodingType != SmsMessage.ENCODING_16BIT) {
                        needCombinByByte = false;
                    }
                }
                if (needCombinByByte) {
                    Xlog.d(TAG, "thiss length = "+length);
                    byte newbuf[] = new byte[length];
                    int pos = 0;
                    for (int j = 0; j < pduCount; j++) {
                        SmsMessage msg;
                        msg = msgs[j];
                        byte[] userData = msg.getUserData();
                        int l = userData.length;
                        Xlog.d(TAG, "thiss l = "+l + "pos ="+pos);
                    ...
                    String sBody = null;
    				try {
                        sBody = new String(newbuf,"utf-16");
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                    values.put(Inbox.BODY, sBody);
                } else {
