    ...
    private int get34GCapabilitySubId() {
        ...
        if (iTelephony != null) {
            for (int i = 0; i < telephonyManager.getPhoneCount(); i++) {
                try {
                    log("get34GCapabilitySubId, getRadioAccessFamily(" + i + "): "
                    ...
                    log("get34GCapabilitySubId FAIL to getPhoneRat i" + i +
                            " error:" + e.getMessage());
                }
            }
        }
    ...
    */
    ...
    private final BroadcastReceiver mReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            log("action: " + action);
            if (action.equals(TelephonyIntents.ACTION_SIM_STATE_CHANGED) && mListPreference != null) {
    ...
    public boolean isNotPrimarySIM(int slotId) {
        if (isGeminiSupport() && SubscriptionManager.from(mContext).
                getActiveSubscriptionInfoList().size() > 1 && slotId != mCurrentTab) {
            return true;
        }
