...
public class ServiceStateExtOP01 extends DefaultServiceStateExt {
    ...
    public int mapGsmSignalLevel(int asu, int gsmRscpQdbm) {
        int level;
        //[ALPS02111968] only apply TDD's mapping rule in this function
        ...
        if (modemType == ModemSwitchHandler.MD_TYPE_TG ||
            ...
            Log.i(TAG, "using TDD's mapping rule, modemType=" + modemType);
            if (asu <= 2 || asu == 99) {
            ...
            return SignalStrength.SIGNAL_STRENGTH_NONE_OR_UNKNOWN - 1;
        }
    }
    ...
    public int needAutoSwitchRatMode(int phoneId,String nwPlmn){
        ...
        if ((phoneId == (switchStatus - 1)) && SubscriptionManager.isValidPhoneId(phoneId)){
            ...
            if (r != null) {
                ...
                if (imsi != null && !imsi.equals("")) {
                    ...
                    log("get simOperator from IMSI = "+simOperator);
                }
            }
            ...
            if((phoneId >= 0 && phoneId <= 3) && SystemProperties.getInt(PROPERTY_RIL_TEST_SIM[phoneId], 0) == 1){
                isTestIccCard = true;
            }
            ...
            log("isTdd: "+isTdd+",isWorldPhone:"+isWorldPhoneSupport()+",isLte: "+isLTESupport()+
                ",nwPlmn: "+nwPlmn +",testMode: "+testMode +
                ",isTestIccCard: "+isTestIccCard+",userType: "+userType+",isSimType: "+isSimType);
            ...
            if((miscFeatureConfig & PhoneConstants.MISC_FEATURE_CONFIG_MASK_AUTO_SWITCH_RAT) == 1) {
                if (userType == TYPE3_USER){//type3
                    if (!SystemProperties.get("ro.mtk_world_phone_policy").equals("1")) {
                        ...
                        log("needSwitchRatMode: set Rat to 2G only when inserting non-CMCC China operator Card");                        
                        return Phone.NT_MODE_GSM_ONLY;
                    }
                }
                //type1/2
                if (!isWorldPhoneSupport() && (isTdd == true)) {//3M
                    if (nwPlmn != null && !(isInDesignateRegion("460", nwPlmn)) && !(isLabTestPlmn(nwPlmn))) {
        ...
        log("currentNetworkMode = "+currentNetworkMode);
        return currentNetworkMode;
    }
